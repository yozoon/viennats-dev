...
unsigned find_error_line(Iter first_iter, Iter error_iter) {
...
template <typename Iter>
void report_input_error(Iter first_iter, Iter last_iter, Iter error_iter,
  ...
  auto error_line =
      ((first_pos == std::string::npos) ? first
  ...
  std::cerr << "In line " << find_error_line(first_iter, error_iter)
            << ", column " << error_pos << std::endl
            << error_line << std::endl
            << std::setw(error_pos) << '^' << "--- here" << std::endl;
}
...
struct ReportError {
  ...
  template <typename Iter>
  void operator()(Iter first_iter, Iter last_iter, Iter error_iter,
                  const qi::info &what) const {
    ...
    std::cerr << "In line " << client::find_error_line(first_iter, error_iter)
              << ", column " << error_pos << std::endl
              << error_line << std::endl
              << std::setw(error_pos) << '^' << "--- here" << std::endl;
  }
...
struct boolean_ : qi::symbols<char, bool> {
  boolean_() { add("true", true)("false", false); }
} boolean_symbols;

// symbol table for true/false
struct boundary_condition_ : qi::symbols<char, bnc::boundary_condition_type> {
  boundary_condition_() {
    add("INFINITE", bnc::INFINITE_BOUNDARY)("REFLECTIVE",
                                            bnc::REFLECTIVE_BOUNDARY)(
        "PERIODIC", bnc::PERIODIC_BOUNDARY)("EXTENDED", bnc::EXTENDED_BOUNDARY);
  }
...
struct cartesian_dir_ : qi::symbols<char, int> {
  cartesian_dir_() {
    add("-z", -3)("-y", -2)("-x", -1)("+x", 1)("x", 1)("+y", 2)("y", 2)(
        "+z", 3)("z", 3);
  }
} cartesian_dir_symbols;

// symbols for finite difference scheme
struct finite_difference_
    : qi::symbols<char, proc::FiniteDifferenceSchemeType> {
  finite_difference_() {
    add("ENGQUIST_OSHER_1ST_ORDER", proc::ENGQUIST_OSHER_1ST_ORDER)(
...
struct partition_data_structure_
    : qi::symbols<char, partition::DataStructureType> {
  partition_data_structure_() {
    add("NEIGHBOR_LINKS_ARRAYS", partition::NEIGHBOR_LINKS_ARRAYS)(
        "FULL_GRID", partition::FULL_GRID)("UP_DOWN_LINKED_TREE",
                                           partition::UP_DOWN_LINKED_TREE);
  }
} partition_data_structure_symbols;

// symbols for partition splitting strategy
struct partition_splitting_strategy_
    : qi::symbols<char, partition::SplittingType> {
  partition_splitting_strategy_() {
    add("SPATIAL_MEDIAN", partition::SPATIAL_MEDIAN)("OBJECT_MEDIAN",
                                                     partition::OBJECT_MEDIAN)(
...
struct Parameters {
  ...
  struct ProcessParameterType {
    ...
    int AddLayer; // the number of level set layers added to the geometry before
    ...
    std::string ModelParameters; // the model parameters, which are passed to
                                 // the model given by "ModelName"
    int IterationCycles; // the number of iterations (process calculation)
                         // before advancing to the next time step,
    int StartIterationCycles; // the number of iterations at the process
                              // simulation start, might be necessary to
                              // calculate initial values for coverages
    double MaxTimeStep;             // maximal allowed time step
    double smoothing_max_curvature; // parameter for smoothing surface level set
                                    // every time step
    double smoothing_min_curvature; // parameter for smoothing surface level set
                                    // every time step
    ...
    partition::SurfaceAreaHeuristicLambdaType
...
private:
  ...
  std::string make_error(std::string name) {
    ...
    oss << "INPUT ERROR: Required parameter \"" << name << "\" not set!"
        << std::endl;
    return oss.str();
  }
  ...
  bool validate() {
    std::string error;
    if (geometry_files.empty())
      error += make_error("geometry_file");
    // if(output_path.empty()) error+=make_error("output_path");
    if (cfl_condition == -1.)
      error += make_error("cfl_condition");
    if (grid_delta == -1.)
      error += make_error("grid_delta");
    if (default_disc_orientation.empty())
      error += make_error("default_disc_orientation");
    if (num_dimensions == -1)
      error += make_error("num_dimensions");
    if (boundary_condition.empty())
      error += make_error("boundary_conditions");
...
// ADD STRUCT MEMBERS THAT SHOULD BE PARSED HERE SO THEY CAN BE ACCESSED BY THE
// PARSER
BOOST_FUSION_ADAPT_STRUCT(
    client::Parameters,
    (std::vector<std::string>,
     geometry_files)(std::string, output_path)(bool, surface_geometry)(
        bool, report_import_errors)(double, cfl_condition)(double, input_scale)(
        double, grid_delta)(std::vector<int>, input_transformation)(
        std::vector<double>, input_shift)(std::vector<double>,
                                          default_disc_orientation)(
        std::vector<int>,
        ignore_materials)(bool, change_input_parity)(int, random_seed)(
        int, num_dimensions)(int, omp_threads)(double, domain_extention)(
        double, receptor_radius)(double, further_tracking_distance)(
        int, bits_per_distance)(bool, output_volume_extract_single_materials)(
        bool, print_vtk)(bool, print_vtp)(bool, print_dx)(bool, print_lvst)(
        bool, print_explicit_lvst)(bool, print_volume_tetra)(
        bool, print_volume_hull)(bool, print_velocities)(bool, print_coverages)(
        bool, print_rates)(bool, print_materials)(bool, print_statistics)(
        int, max_extended_starting_position)(int, open_boundary)(
        bool, remove_bottom)(double, snap_to_boundary_eps)(int, process_cycles)(
        std::vector<int>, material_mapping)(int, add_layer)(
        std::vector<bnc::boundary_condition_type>,
        boundary_condition)(std::list<client::Parameters::ProcessParameterType>,
                            process_parameters))

BOOST_FUSION_ADAPT_STRUCT(
    client::Parameters::ProcessParameterType,
    (double, ProcessDistance)(std::vector<double>,
                              AFMStartPosition)(std::vector<double>,
                                                AFMEndPosition)(int, AddLayer)(
        double, ProcessTime)(unsigned int, ALDStep)(std::string, ModelName)(
        std::string,
        ModelParameters)(int, IterationCycles)(int, StartIterationCycles)(
        double, MaxTimeStep)(double, smoothing_max_curvature)(
        double, smoothing_min_curvature)(int, smoothing_material_level)(
        int, smoothing_max_iterations)(bool, print_coverages)(
        bool, print_rates)(bool, print_velocities)(bool, print_materials)(
        proc::FiniteDifferenceSchemeType,
        FiniteDifferenceScheme)(double, LaxFriedrichsDissipationCoefficient)(
        partition::DataStructureType, partition_data_structure)(
        partition::SplittingType,
        partition_splitting_strategy)(partition::SurfaceAreaHeuristicLambdaType,
                                      partition_surface_area_heuristic_lambda)(
        std::vector<double>, output_times)(std::vector<double>, output_volume)(
        int, output_times_periodicity)(double, output_times_period_length)(
        bool, initial_output)(bool, final_output)(bool,
                                                  MaskLayer)(bool,
                                                             GrowNewOxide))
...
template <typename Iterator> struct skipper_grammar : qi::grammar<Iterator> {
  skipper_grammar() : skipper_grammar::base_type(skip_on) {
    using ascii::char_;
    using ascii::space;
    using qi::eol;

    skip_on = space | eol | ("/*" >> *(char_ - "*/") >> "*/") |
              ("//" >> *(char_ - eol) >> eol);
  }
...
struct par_grammar : qi::grammar<Iterator, Parameters(), Skipper> {
  ...
  par_grammar() : par_grammar::base_type(definition, "Definition") {
    ...
    boundaryvec %=
        (('{' > +(lexeme[boundary_condition_symbols] % ',') > '}') % ',');
    model_param_end = lit(';') >> lit('}') >> lit(';');

    // ADD CUSTOM RULES HERE
    // rules have to be in the same order in which the variables are declared in
    // the struct
    geometry_file %=
        (lit("geometry_files") | lit("GeometryFiles") | lit("geometry_file") |
         lit("GeometryFile")) > "=" > listed > ";";
    output_path %=
        (lit("output_path") | lit("OutputPath")) > "=" > quotes > ";";
    ...
    default_disc_orientation %=
        (lit("default_disc_orientation") | lit("default_disk_orientation")) >
        '=' > doublevec > ';';
    ...
    further_tracking_distance %=
        lit("further_tracking_distance") > '=' > lexeme[double_] > ';';
    bits_per_distance %= lit("bits_per_distance") > '=' > lexeme[int_] > ';';
    output_volume_extract_single_materials %=
        lit("output_volume_extract_single_materials") > '=' > boolean > ';';
    ...
    max_extended_starting_position %=
        lit("max_extended_starting_postion") > '=' > lexeme[int_] > ';';
    open_boundary %= lit("open_boundary") > '=' > '"' >
                     lexeme[cartesian_dir_symbols] > '"' > ';';
    remove_bottom %= lit("remove_bottom") > '=' > boolean > ';';
    snap_to_boundary_eps %=
        lit("snap_to_boundary_eps") > '=' > lexeme[double_] > ';';
    ...
    boundary_conditions %=
        lit("boundary_conditions") > '=' > '{' > boundaryvec > '}' > ';';

    // PROCESS PARSING
    process_distance %= lit("process_distance") > '=' > lexeme[double_] > ';';
    afm_start_position %=
        lit("afm_start_position") > '=' > lexeme[double_] > ';';
    ...
    model_parameters %= lit("parameters") > '=' > '{' >
                        +(char_ - (model_param_end)) >
                        model_param_end; // do not use lexeme to allow skipping
                                         // for end detection
    iteration_cycles %= lit("iteration_cycles") > '=' > lexeme[int_] > ';';
    start_iteration_cycles %=
        lit("start_iteration_cycles") > '=' > lexeme[int_] > ';';
    max_time_step %= lit("max_time_step") > '=' > lexeme[double_] > ';';
    smoothing_max_curvature %=
        lit("smoothing_max_curvature") > '=' > lexeme[double_] > ';';
    smoothing_min_curvature %=
        lit("smoothing_min_curvature") > '=' > lexeme[double_] > ';';
    smoothing_material_level %=
        lit("smoothing_material_level") > '=' > lexeme[int_] > ';';
    smoothing_max_iterations %=
        lit("smoothing_max_iterations") > '=' > lexeme[int_] > ';';
    finite_difference_scheme %= lit("finite_difference_scheme") > '=' >
                                lexeme[finite_difference_symbols] > ';';
    dissipation_coefficient %=
        lit("dissipation_coefficient") > '=' > lexeme[double_] > ';';
    partition_data_structure %= lit("partition_data_structure") > '=' >
                                lexeme[partition_data_structure_symbols] > ';';
    partition_splitting_strategy %=
        lit("partition_splitting_strategy") > '=' >
        lexeme[partition_splitting_strategy_symbols] > ';';
    partition_surface_area_heuristic_lambda %=
        lit("partition_surface_area_heuristic_lambda") > '=' > lexeme[double_] >
        ';';
    output_times %= (lit("output_times") - lit("output_times_periodicity") -
                     lit("output_times_period_length")) > '=' > doublevec > ';';
    output_volume %= lit("output_volume") > '=' > doublevec > ';';
    output_times_periodicity %=
        lit("output_times_periodicity") > '=' > lexeme[int_] > ';';
    output_times_period_length %=
        lit("output_times_period_length") > '=' > lexeme[double_] > ';';
    ...
    process_params %=
        process_distance ^ afm_start_position ^ afm_end_position ^ add_layer ^
        process_time ^ ald_step ^ model_name ^ model_parameters ^
        iteration_cycles ^ start_iteration_cycles ^ max_time_step ^
        smoothing_max_curvature ^ smoothing_min_curvature ^
        smoothing_material_level ^ smoothing_max_iterations ^ print_coverages ^
        print_rates ^ print_velocities ^ print_materials ^
        finite_difference_scheme ^ dissipation_coefficient ^
        partition_data_structure ^ partition_splitting_strategy ^
        partition_surface_area_heuristic_lambda ^ output_times ^ output_volume ^
        output_times_periodicity ^ output_times_period_length ^ initial_output ^
        final_output ^ mask_layer ^ grow_new_oxide;

    process %= lit('{') > process_params > lit('}');
    processes %= lit("processes") > lit('=') > lit('{') > (process % ',') >
                 lit('}') > lit(';');

    // ORDER OF RULES MUST MATCH VARIABLE DECLARATION IN STRUCT
    definition %=
        geometry_file ^ output_path ^ surface_geometry ^ report_import_errors ^
        cfl_condition ^ input_scale ^ grid_delta ^ input_transformation ^
        input_shift ^ default_disc_orientation ^ ignore_materials ^
        change_input_parity ^ random_seed ^ num_dimensions ^ omp_threads ^
        domain_extension ^ receptor_radius ^ further_tracking_distance ^
        bits_per_distance ^ output_volume_extract_single_materials ^ print_vtk ^
        print_vtp ^ print_dx ^ print_lvst ^ print_explicit_lvst ^
        print_volume_tetra ^ print_volume_hull ^ print_velocities ^
        print_coverages ^ print_rates ^ print_materials ^ print_statistics ^
        max_extended_starting_position ^ open_boundary ^ remove_bottom ^
        snap_to_boundary_eps ^ process_cycles ^ material_mapping ^ add_layer ^
        boundary_conditions ^ processes;

    // what to do when parsing fails
    on_error<fail>(definition, report_error(_1, _2, _3, _4));
  }
...
private:
  ...
  qi::rule<Iterator, Parameters::ProcessParameterType(), Skipper>
      process_params;
  qi::rule<Iterator, std::list<Parameters::ProcessParameterType>(), Skipper>
      processes;
  ...
  qi::rule<Iterator, std::vector<bnc::boundary_condition_type>(), Skipper>
      boundaryvec;
  qi::rule<Iterator, std::vector<bnc::boundary_condition_type>(), Skipper>
      boundary_conditions;
  qi::rule<Iterator, std::vector<int>(), Skipper> input_transformation,
      ignore_materials, material_mapping;
  qi::rule<Iterator, std::vector<double>(), Skipper> input_shift,
      default_disc_orientation, output_times, output_volume;
  qi::rule<Iterator, std::string(), Skipper> output_path, model_name,
      model_parameters;
  qi::rule<Iterator, bool(), Skipper> surface_geometry, report_import_errors,
      change_input_parity, output_volume_extract_single_materials, print_vtk,
      print_vtp, print_dx, print_lvst, print_explicit_lvst, print_volume_hull,
      print_volume_tetra, print_velocities, print_coverages, print_rates,
      print_materials, print_statistics, remove_bottom, initial_output,
      final_output, mask_layer, grow_new_oxide;
  qi::rule<Iterator, double(), Skipper> cfl_condition, input_scale, grid_delta,
      domain_extension, receptor_radius, further_tracking_distance,
      snap_to_boundary_eps, process_distance, afm_start_position,
      afm_end_position, process_time, max_time_step, smoothing_max_curvature,
      smoothing_min_curvature, dissipation_coefficient,
      output_times_period_length;
  qi::rule<Iterator, int(), Skipper> random_seed, num_dimensions, omp_threads,
      bits_per_distance, max_extended_starting_position, open_boundary,
      process_cycles, add_layer, ald_step, iteration_cycles,
      start_iteration_cycles, smoothing_material_level,
      smoothing_max_iterations, output_times_periodicity,
      partition_data_structure, partition_splitting_strategy,
      partition_surface_area_heuristic_lambda, finite_difference_scheme;
};
} // namespace client
...
client::Parameters::Parameters(std::string fileName) {
  ...
  if (!r) {
    report_input_error(input.begin(), end, start,
                       "INPUT ERROR: Illegal parameter name or other "
                       "unexpected name specified.");
  ...
  if (start != end) {
    report_input_error(input.begin(), end, start,
                       "INPUT ERROR: Illegal parameter name or duplicate "
  ...
  // IMPORTANT: MODEL PARAMETERS ARE MISSING }; AT THE END, NEED TO ADD BEFORE
  // ANYTHING ELSE
  //  SelectiveDeposition process requires that add_layer is at least 1
  for (std::list<client::Parameters::ProcessParameterType>::iterator it =
           process_parameters.begin();
       it != process_parameters.end(); ++it) {
  ...
  for (size_t i = 0; i < input_transformation.size(); ++i)
    input_transformation_signs[i] = input_transformation[i] >= 0;
  ...
  // fix bits_per_distance so there are no overflow bits when writing to lvst
  // file meaning that bits_per_distance are mapped to 1, 2, 4, 8, 16, 24, 32,
  // 48, 56, 64
  int bpd = 1;
  while (bpd < bits_per_distance ||
         (bpd < CHAR_BIT ? CHAR_BIT % bpd : bpd % CHAR_BIT)) {
    if (bpd < CHAR_BIT)
      bpd++;
    else
      bpd +=
          CHAR_BIT; // once bpd (bits per distance) reaches the amount of bits
                    // per char, the increase will be CHAR_BIT instead of 1
  }
  if (bits_per_distance == 0)
    bits_per_distance = CHAR_BIT;
  else
    bits_per_distance = bpd > 64 ? 64 : bpd;

  // Create directory, if it does not exist; make it relative to the parameter
  // file's path
  std::string relative_path, relative_name;

  std::size_t last_slash = fileName.find_last_of('/');
  if (last_slash !=
      std::string::npos) { // if parameter file is in a different directory
    relative_path = fileName.substr(0, last_slash + 1); // path of parameter
                                                        // file
  ...
  if (output_path.front() != '/') {
    if (output_path.size() ==
        0) { // default output path; example: par_trench.txt -> output_trench/
      output_path = relative_name.substr(
          0, relative_name.find_last_of('.')); // exclude file ending
  ...
  for (unsigned int i = 0; i < geometry_files.size(); i++) {
    if (geometry_files[i].front() != '/')
      geometry_files[i] = relative_path + geometry_files[i];
  }
  ...
  // add _bits_per_distance/ to output path
  {
    ...
    if (!boost::filesystem::exists(output_path)) {
      msg::print_message(
          "Output directory not found! Creating new directory: " + output_path +
          "\n");
      boost::filesystem::path dir(output_path);
      if (!boost::filesystem::create_directory(dir))
        msg::print_error("Could not create directory!");
    }
  }

  // test boundary condtions for periodicity, and check conformity
  for (unsigned int h = 0; h < boundary_conditions.size(); h++) {
    if (boundary_conditions[h].min == bnc::PERIODIC_BOUNDARY)
      assert(boundary_conditions[h].max == bnc::PERIODIC_BOUNDARY);
    if (boundary_conditions[h].max == bnc::PERIODIC_BOUNDARY)
      assert(boundary_conditions[h].min == bnc::PERIODIC_BOUNDARY);
    if (h == static_cast<unsigned int>(open_boundary))
      assert(boundary_conditions[h].min == bnc::INFINITE_BOUNDARY);
    if (h == static_cast<unsigned int>(open_boundary))
      assert(boundary_conditions[h].max == bnc::INFINITE_BOUNDARY);
  }

  // add OutputTimes
  for (std::list<ProcessParameterType>::iterator pIter =
           process_parameters.begin();
       pIter != process_parameters.end(); ++pIter) {
    // add output times for initial and final output
    if (pIter->initial_output)
      pIter->output_times.push_back(0);
    if (pIter->final_output)
      pIter->output_times.push_back(pIter->ProcessTime);

    // Create output times from period length and periodicity
    if (pIter->output_times_period_length > 0 &&
        pIter->output_times_periodicity >= 0) {
      if (pIter->output_times.empty())
        pIter->output_times.push_back(0.); // if no output times, use 0 as start
      ...
      for (int h = 1; h < pIter->output_times_periodicity; ++h) {
        for (unsigned int k = 0; k < tmp; k++) {
          pIter->output_times[tmp * h + k] =
              pIter->output_times[k] + h * pIter->output_times_period_length;
        }
      }
    }

    // sort volume output and add it to surface output, so that a surface is
    // output each time a volume is output. this is required for the advection
    // to stop at each volume output. It is also necessary for a downsampling of
    // the volume being output Sort Volume Output
    std::sort(pIter->output_volume.begin(), pIter->output_volume.end());
    std::vector<double>::iterator it =
        std::unique(pIter->output_volume.begin(), pIter->output_volume.end());
    pIter->output_volume.resize(it - pIter->output_volume.begin());
    // add volume output times to surface output times
    pIter->output_times.insert(pIter->output_times.end(),
                               pIter->output_volume.begin(),
                               pIter->output_volume.end());
