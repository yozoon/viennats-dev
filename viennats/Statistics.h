...
inline double RandomNumber() { return distribution(generator); }
...
inline void PickRandomPointOnUnitSphere(double &x, double &y,
                                        double &z) { // better
  double x2, y2, x2py2;
  do {
    x = 2 * RandomNumber() - 1.;
...
inline double ConeCosineSineDistributionReturnTheta(const double cone_angle) {
  ...
  } while (RandomNumber() * angle * u >
           std::cos(Pi1_2 * sqrt_1m_u) * std::sin(angle));
  return angle;
}

inline double
ConeCosineSineDistributionReturnCosTheta(const double cone_angle) {
...
inline void Rotate(const VecType &AverageDirection, VecType2 &RandomDirection,
                   const double sinphi, const double cosphi, double costheta,
                   const double r2 = 1.) {
  ...
  RandomDirection[1] =
      a1 * costheta_p_a0_tmp_sinphi + AverageDirection[2] * tmp_cosphi;
  RandomDirection[2] =
      AverageDirection[2] * costheta_p_a0_tmp_sinphi - a1 * tmp_cosphi;

  if (a0 != AverageDirection[0])
    std::swap(RandomDirection[0], RandomDirection[1]);
}

template <class VecType, class VecType2>
inline void RandomAzimuthalRotation3(const VecType &AverageDirection,
                                     VecType2 &RandomDirection,
...
template <class VecType, class VecType2>
inline void RandomAzimuthalRotation(const VecType &AverageDirection,
                                    VecType2 &RandomDirection,
                                    const double costheta) {
...
template <class VecType, class VecType2>
inline void RandomAzimuthalRotation2(const VecType &AverageDirection,
                                     VecType2 &RandomDirection,
                                     const double costheta) {
  ...
  do {
    ...
    dot = AverageDirection[0] * tmp[0] + AverageDirection[1] * tmp[1] +
          AverageDirection[2] * tmp[2];
  } while (dot >= 1.);

  double r = std::sqrt((1 - costheta * costheta) / (1 - dot * dot));

  RandomDirection[0] =
      AverageDirection[0] * costheta + (tmp[0] - AverageDirection[0] * dot) * r;
  RandomDirection[1] =
      AverageDirection[1] * costheta + (tmp[1] - AverageDirection[1] * dot) * r;
  RandomDirection[2] =
      AverageDirection[2] * costheta + (tmp[2] - AverageDirection[2] * dot) * r;
}

template <class VecType, class VecType2>
inline void CosineNDistributedRandomDirection(double N,
                                              const VecType &AverageDirection,
                                              VecType2 &RandomDirection) {
...
template <class VecType, class VecType2>
inline void
Cosine1DistributedRandomDirection(const VecType &AverageDirection,
                                  VecType2 &RandomDirection,
                                  double twice_cos_cutoff_angle = 0.) {
  double tmp;
  do {
    PickRandomPointOnUnitSphere(RandomDirection[0], RandomDirection[1],
                                RandomDirection[2]);
    RandomDirection[0] += AverageDirection[0];
    RandomDirection[1] += AverageDirection[1];
    RandomDirection[2] += AverageDirection[2];
    tmp = std::sqrt(RandomDirection[0] * RandomDirection[0] +
                    RandomDirection[1] * RandomDirection[1] +
                    RandomDirection[2] * RandomDirection[2]);
...
template <class VecType, class VecType2>
inline void CosAngleDistributedRandomDirection(double Angle,
                                               const VecType &AverageDirection,
                                               VecType2 &RandomDirection) {
...
inline void NormalDistributedStartPosition(const VecType1 &center,
                                           const VecType2 &dir, ValType FWHM,
                                           VecType3 &position) {
...
template <class VecType1, class VecType2, class VecType3, class ValType,
          class ParameterType>
inline void NormalDistributedStartPosition2(const VecType1 &center,
                                            const VecType2 &dir, ValType FWHM,
                                            VecType3 &position,
...
template <class VecType1, class VecType2, class VecType3, class ValType,
          class ParameterType>
inline void LorentzDistributedStartPosition(const VecType1 &center,
                                            const VecType2 &dir, ValType FWHM,
                                            VecType3 &position,
                                            const ParameterType &Parameter) {
...
template <class VecType1, class VecType2, class VecType3, class ValType,
          class ParameterType>
inline void SurfaceChargeDensityDistributedStartPosition(
    const VecType1 &center, const VecType2 &dir, ValType FWHM,
    VecType3 &position, const ParameterType &Parameter, double distance) {
  ...
  radius =
      2 * FWHM * std::sqrt((1 / (1 - randnum)) * (1 / (1 - randnum)) - 1) / 3;
...
template <class VecType1, class VecType2, class VecType3, class ValType,
          class ParameterType>
inline double SurfaceChargeDensityDistributedStartPosition(
    const VecType1 &center, const VecType2 &dir, ValType FWHM,
    VecType3 &position, const ParameterType &Parameter) {
...
template <class VecType1, class VecType2, class VecType3, class ValType,
          class ParameterType, class PartitionType>
inline void
SurfaceChargeDensityDistribution(const VecType1 &center, const VecType2 &dir,
                                 ValType voltage, VecType3 &position,
                                 const ParameterType &Parameter,
  ...
  double d;
  for (int i = 0; i < 3; i++)
    d = (Parameter.open_boundary == i)
            ? (center[i] - (Partition.Max(i) - 1) * Parameter.grid_delta)
            : d;

  do {
    for (int i = 0; i < 3; i++)
      position[i] =
          (Parameter.open_boundary == i)
              ? center[i]
              : RandomNumber() * Partition.Extension(i) * Parameter.grid_delta;

    double PositionSumSquares = 0;
    for (int i = 0; i < 3; i++)
      PositionSumSquares +=
          (Parameter.open_boundary != i)
              ? (position[i] - center[i]) * (position[i] - center[i])
              : 0;

    double scd;
    scd = (d * 1.6e-19) / (2 * (my::math::Pi) * (PositionSumSquares + d * d) *
                           sqrt(PositionSumSquares + d * d));
...
template <class VecType1, class VecType2, class ValType, class ParameterType,
          class PartitionType>
inline void SurfaceChargeDensityDistribution(
    const VecType1 &dir, ValType voltage, VecType2 &position,
    const ParameterType &Parameter, const PartitionType &Partition,
  ...
  do {
    ...
    for (int i = 0; i < 3; i++)
      position[i] =
          (Parameter.open_boundary == i)
              ? ((Partition.Max(i) - 1) * Parameter.grid_delta)
              : RandomNumber() * Partition.Extension(i) * Parameter.grid_delta;
    ...
    for (unsigned int i = 0; i < charges.size(); ++i) {
      ...
      double PositionSumSquares = 0;
      for (int j = 0; j < 3; j++)
        PositionSumSquares += (Parameter.open_boundary != j)
                                  ? (position[j] - positions[3 * i + j]) *
                                        (position[j] - positions[3 * i + j])
                                  : 0;

      double PositionSumSquaresMax = 0;
      for (int j = 0; j < 3; j++)
        PositionSumSquaresMax +=
            (Parameter.open_boundary != j)
                ? (position_max[j] - positions[3 * i + j]) *
                      (position_max[j] - positions[3 * i + j])
                : 0;

      scd += (d * charges[i] * 1.6e-19) /
             (2 * (my::math::Pi) * (PositionSumSquares + d * d) *
              sqrt(PositionSumSquares + d * d));
      scd_max += (d * charges[i] * 1.6e-19) /
                 (2 * (my::math::Pi) * (PositionSumSquaresMax + d * d) *
                  sqrt(PositionSumSquaresMax + d * d));
    }
...
template <class VecType1, class VecType2, class VecType3, class VecType4,
          class ValType, class ParameterType>
inline double
NanowireSurfaceCharge(const VecType1 &StartPosition, const VecType2 EndPosition,
                      const VecType3 &dir, ValType FWHM, ValType Length,
                      ValType Height, ValType Angle, VecType4 &position,
                      const ParameterType &Parameter) {
  ...
  if (line_or_sides <= volume_line) { // generate a particle in the line
    ...
    //          double radius;
    radius =
        FWHM * std::sqrt((1 / (1 - randnum)) * (1 / (1 - randnum)) - 1) / 3;
...
inline void Junctionless(VecType1 &position, const ParameterType &Parameter,
                         const PartitionType &Partition) {
...
template <class VecType1, class VecType2, class VecType3, class VecType4,
          class ValType, class ParameterType, class PartitionType>
inline void NanowireSurfaceChargeDistribution(const VecType1 &StartPosition,
                                              const VecType2 EndPosition,
                                              const VecType3 &dir, ValType FWHM,
                                              ValType Length, ValType Angle,
                                              VecType4 &position,
                                              const ParameterType &Parameter,
                                              const PartitionType &Partition) {
  ...
  for (int i = 0; i < 3; i++)
    d = (Parameter.open_boundary == i)
            ? (StartPosition[i] - (Partition.Max(i) - 1) * Parameter.grid_delta)
            : d;
  ...
  do {
    ...
    double
        alpha; // apha is the angle the line makes with the horizontal direction
    if ((EndPosition[x] - StartPosition[x]) == 0) {
      ...
      alpha = std::atan((EndPosition[y] - StartPosition[y]) /
    ...
    if ((position[x] - StartPosition[x]) == 0) {
      if ((position[y] - StartPosition[y]) > 0) {
        ...
        beta = -my::math::Pi / 2;
      } else { //(position[0]-StartPosition[0]) and
               //(position[2]-StartPosition[2])
        beta = alpha;
      }
    } else {
      beta = std::atan((position[y] - StartPosition[y]) /
    ...
    if (beta < (alpha - my::math::Pi / 2) ||
        beta > (alpha + my::math::Pi / 2)) {
...
template <class VecType1, class VecType2, class VecType3, class VecType4,
          class ValType, class ParameterType, class PartitionType>
inline void
NanowireLorentzDistribution(const VecType1 &StartPosition,
                            const VecType2 EndPosition, const VecType3 &dir,
                            ValType FWHM, ValType Length, ValType Angle,
                            VecType4 &position, const ParameterType &Parameter,
                            const PartitionType &Partition) {
  ...
  do {
    ...
    if ((position[x] - StartPosition[x]) == 0) {
      if ((position[y] - StartPosition[y]) > 0) {
        ...
        beta = -my::math::Pi / 2;
      } else { //(position[0]-StartPosition[0])==0 and
               //(position[2]-StartPosition[2])==0
        beta = alpha;
      }
    } else {
      beta = std::atan((position[y] - StartPosition[y]) /
    ...
    if (beta < (alpha - my::math::Pi / 2) ||
      ...
      if (DirectionalLength > Length) {
        rc = (position[y] - EndPosition[y]) * (position[y] - EndPosition[y]) +
             (position[x] - EndPosition[x]) * (position[x] - EndPosition[x]);
      } else {
...
template <class VecType1, class VecType2, class VecType3, class VecType4,
          class ValType, class ParameterType>
inline void NanowireLorentzDistribution2(const VecType1 &StartPosition,
                                         const VecType2 EndPosition,
                                         const VecType3 &dir, ValType FWHM,
                                         ValType Length, ValType Height,
                                         ValType Angle, VecType4 &position,
                                         const ParameterType &Parameter) {

  double volume_sides, volume_line;
  volume_sides =
      2 * Height * (FWHM / 2) * (FWHM / 2) * (my::math::Pi) * (my::math::Pi);
  ...
  if (line_or_sides <= volume_line) { // generate a particle in the line
    ...
    radius = FWHM / 2 *
             std::sqrt(std::abs(exp(2 * my::math::Pi * RandomNumber())) - 1);
...
template <class DropletType, class VecType1, class VecType2,
          class ParameterType, class PartitionType>
inline void ESDDistribution(const DropletType &d, const VecType1 &StartPosition,
                            VecType2 &Position, double &r, double &q,
  ...
  double r_min_inv = 1 / 2.5e-6;
  double r_max_inv = 1 / 55e-6;
  double r_max_inv_third = exp(log(r_max_inv) / 3);
  double r_min_inv_third = exp(log(r_min_inv) / 3);
  double radius =
      1 / pow(volume_fraction * (r_max_inv_third - r_min_inv_third) +
                  r_min_inv_third,
              3.);
  ...
  //----------------Calculate the charge given
  //r----------------------------------
  double gamma_d = 0.022;
  double permittivity = 8.854187817e-12;
  double qd = 0.58 * 8 * my::math::Pi *
              sqrt(gamma_d * permittivity * radius * radius * radius);
  //------------------------------------------------------------------------------

  //----------------Find the initial droplet position
  //(cylidrical)----------------
  long double theta_i =
      0.5 * my::math::Pi * RandomNumber() * 0.5; // 45 degree spray cone
  ...
  //---FIND THE INITIAL AND THERMALDROPLET ELECTRICAL FORCES AND
  //ACCELLERATIONS---
  ...
  //-----------Find expected initial electric force when E-field acts
  //alone--------
  ...
  //-----------Find expected final electric force where E-field acts
  //alone---------
  double t_heat = 10e-3;
  ...
  //-----------Use the first and second to come up with linear
  //dependence----------
  double E_v = (E_v1 - Eth_v2) / (H - t_heat);
  double E_r =
      (theta < 1e-20) ? 0 : (E_r1 - Eth_r2) / ((H - t_heat) * tan(theta));
  ...
  //------------Calculate the required constant component of electric
  //force--------
  double Fe_v1 = qd * E_v1;
  double Fe_r1 = qd * E_r1;
  double ae_v1 = Fe_v1 / mass; // Initial constant component
  double ae_r1 =
      (theta < 1e-20) ? 0 : Fe_r1 / mass; // Initial constant component

  //-------------------------------------------------------------------------------

  //------------------------------------------------------------------------------

  //--------------Find all forces acting on the
  //droplet---------------------------
  ...
  //--------------Find new position of droplet after
  //t---------------------------- First, separate the forces due to
  // velocity/displacement dependences

  double a_v = g + ae_v1; //+2*a_e;  // independent acceleration (mainly
                          //gravity) - vertical only
  double b =
      s_f; // velocity dependent acceleration  - Stokes force - vert and rad
  ...
  if (b * b - 4 * c_v < 0) {
    ...
    while (true) {
      d_test_v = (A1_v + A2_v + A3_v) * exp(r1_v * t_check) +
                 (B1_v + B2_v + B3_v) * exp(r2_v * t_check) + C_v;
      if (t_high - t_low < 1e-100)
        break;
    ...
    t_drop = t_check;
    v0th_v = (A1_v + A2_v + A3_v) * (exp(r1_v * t_drop) - 1) / r1_v +
             (B1_v + B2_v + B3_v) * (exp(r2_v * t_drop) - 1) / r2_v +
             C_v * t_drop + v0_v;

    //---------Now time required to reach heat zone is
    //known------------------------

    //---------Can now calculate the radial
    //displacement----------------------------
    ...
    if (b * b - 4 * c_r < 0) {
      ...
      d_test_r = (A1_r + A2_r + A3_r) * exp(r1_r * t_drop) +
                 (B1_r + B2_r + B3_r) * exp(r2_r * t_drop) + C_r;
      v0th_r = (A1_r + A2_r + A3_r) * (exp(r1_r * t_drop) - 1) / r1_r +
               (B1_r + B2_r + B3_r) * (exp(r2_r * t_drop) - 1) / r2_r +
               C_r * t_drop + v0_r;
    }
  }
  //------------------------------------------------------------------------------
  ...
  //------------------------------------------------------------------------------
  //------------------------------------------------------------------------------
  //      //---------Now we know z and r on the cusp of the thermal
  //      zone------------------
  //      //---------Reset parameters to include thermal components and re-run
  //      -----------

  //--------CALCULATE THERMAL EFFECTS IN DROPLET SIZE
  //REDUCTION-------------------
  double dth_test_v;
  double dth_test_r;
  ...
  r = r_new;
  double qd_new = 0.58 * 8 * my::math::Pi *
                  sqrt(gamma_d * permittivity * r_new * r_new * r_new);
  double mass_new = 4 * my::math::Pi * rho_d * r_new * r_new * r_new / 3;
  //
  //      //---------CALCULATE THE ENERGIES ASSUMING LINEAR REDUCTION TO
  //      FINAL------------
  //      //---------Electric force F_e is constant in this region and
  //      small--------------
  //
  ...
  double Eth_r =
      E_e * (thplusz / throotplusz - thminusz / throotminusz) / rth_star;
  ...
  //--------------Find all other forces acting on the
  //droplet---------------------
  // Find the droplet mass - knowns: radius, rho_d
  ...
  double F_th = 3 * my::math::Pi * eta_a * eta_a * r_new * 3 * kappa_a *
                grad_T / (rho_a * T * (2 * kappa_a + kappa_d));
  double a_th = F_th / mass_new;

  // Electric force component acceleration - knowns: q_d, r_star, z_star;

  double ath_v = g + aeth_v - a_th; //+2*a_e;  // independent acceleration
                                    //(gravity, initial e-force, thermal force)
  double bth = sth_f; // velocity dependent acceleration    - Stokes force
  double cth_v =
      aeth_v /
      t_heat; // displacement dependent acceleration   - vertical E-force
  double ath_r =
      (theta_th < 1e-20)
          ? 0
          : aeth_r; // independent acceleration - Initial electric force
  double cth_r =
      (theta_th < 1e-20)
          ? 0
          : aeth_r / (t_heat * tan(theta_th)); // displacement dependent
                                               // acceleration - Radial E-force
  ...
  if (bth * bth - 4 * cth_v < 0) {
    ...
    while (true) {
      dth_test_v = (A1th_v + A2th_v + A3th_v) * exp(r1th_v * tth_check) +
                   (B1th_v + B2th_v + B3th_v) * exp(r2th_v * tth_check) + Cth_v;
      if (tth_high - tth_low < 1e-10)
        break;
      ...
      if (iteration_stopth == 0)
        break;
      iteration_stopth--;
    }
    tth_drop = tth_check;
    v_final_v =
        (A1th_v + A2th_v + A3th_v) * (exp(r1th_v * tth_drop) - 1) / r1th_v +
        (B1th_v + B2th_v + B3th_v) * (exp(r2th_v * tth_drop) - 1) / r2th_v +
        Cth_v * tth_drop + v0th_v;

    //---------Now time required to reach surface is
    //known--------------------------
    ...
    if (bth * bth - 4 * cth_r < 0) {
      ...
      dth_test_r = (A1th_r + A2th_r + A3th_r) * exp(r1th_r * tth_drop) +
                   (B1th_r + B2th_r + B3th_r) * exp(r2th_r * tth_drop) + Cth_r;
      v_final_r =
          (A1th_r + A2th_r + A3th_r) * (exp(r1th_r * tth_drop) - 1) / r1th_r +
          (B1th_r + B2th_r + B3th_r) * (exp(r2th_r * tth_drop) - 1) / r2th_r +
          Cth_r * tth_drop + v0th_r;
    }
  }
...
template <class DropletType, class VecType1, class VecType2,
          class ParameterType, class PartitionType>
inline void EvenlyDistributed(const DropletType &d,
                              const VecType1 &StartPosition, VecType2 &Position,
                              double &r, double &q, long double *Velocity,
                              const ParameterType &Parameter,
                              const PartitionType &Partition) {
  ...
  Position[0] = RandomNumber() * 4 * (Partition.Max(0) * Parameter.grid_delta) -
                2 * (Partition.Max(0) * Parameter.grid_delta);
  Position[1] = 0;
  Position[2] = RandomNumber() * 4 * (Partition.Max(2) * Parameter.grid_delta) -
                2 * (Partition.Max(2) * Parameter.grid_delta);
...
template <class VecType1, class VecType2, class ParameterType,
          class PartitionType>
inline void EvenlyDistributed(const VecType1 &StartPosition, VecType2 &Position,
                              const ParameterType &Parameter,
                              const PartitionType &Partition) {

  Position[0] = RandomNumber() * (Partition.Max(0) * Parameter.grid_delta -
                                  Partition.Min(0) * Parameter.grid_delta) +
                Partition.Min(0) * Parameter.grid_delta;
  Position[1] = 0;
  Position[2] = RandomNumber() * (Partition.Max(2) * Parameter.grid_delta -
                                  Partition.Min(2) * Parameter.grid_delta) +
                Partition.Min(2) * Parameter.grid_delta;
}
...
bool AnyElement(typename std::vector<DataType> vec, DataType check) {
  typename std::vector<DataType>::iterator first = vec.begin(),
                                           last = vec.end();
  while (first != last) {
    if (*first == check)
      return true;
