...
namespace my {
/// Includes mathematical functions like equation solvers, root finders and
/// vector arithmetic.
namespace math {
...
double FindFirstRoot(double IntervalStart, double IntervalEnd,
                     const Polynom<double, 2> &p);
double FindFirstRoot(double IntervalStart, double IntervalEnd,
                     const Polynom<double, 3> &p);
...
template <int D>
void DetermineCoefficientsForImplicitRayTracing(const double *Position,
                                                const double *Direction,
                                                const double *Rho,
                                                double *Coefficients);
template <int D>
void CalculateNormal(double *n, const double *v, const double *A);

/*template <int D, template <class, int> class V > inline V<int,D> round(const
V<double,D>& vd) { V<int,D> vi; for (int i=0;i<D;i++)
vi[i]=static_cast<int>(nearbyint(vd[i])); return vi;
    }

template <int D, template <class, int> class V > inline V<int,D> floor(const
V<double,D>& vd) { V<int,D> vi; for (int i=0;i<D;i++)
vi[i]=static_cast<int>(std::floor(vd[i])); return vi;
}

template <int D, template <class, int> class V > inline V<int,D> ceil(const
V<double,D>& vd) { V<int,D> vi; for (int i=0;i<D;i++)
vi[i]=static_cast<int>(std::ceil(vd[i])); return vi;
}*/

template <class T> inline T Interpolate(const T *X, const T *Y, int N, T v) {
...
template <int D> struct TransformRho2 {
...
template <> struct TransformRho2<0> {
  static void exec(double Rho[]) {}
};
} // namespace math
} // namespace my
...
template <class T> inline T my::math::pow2(T x) { return x * x; }
...
public:
  const CoefT *Coefficients() const { return coefficients; }

  CoefT *Coefficients() { return coefficients; }

  CoefT Coefficients(int order) const { return coefficients[order]; }
...
template <class T>
T FindFirstTransitionFromPosToNegOfPolynomBisect(T t0, T t1,
                                                 const Polynom<T, 3> &p,
                                                 T eps) {
  ...
  T Discriminant = p.Coefficients()[2] * p.Coefficients()[2] -
                   3. * p.Coefficients()[1] * p.Coefficients()[3];
  if (Discriminant >= 0) { // p' has real roots

    // calculate roots of p'
    T Q = -(p.Coefficients()[2] +
            my::math::SignPos(p.Coefficients()[2]) * std::sqrt(Discriminant));
...
template <class T>
T FindFirstTransitionFromPosToNegOfPolynomNewton(T t0, T t1,
                                                 const Polynom<T, 3> &p,
                                                 T eps) {
  ...
  while (true) {
    ...
    if ((((t - t0) * dft - ft) * ((t - t1) * dft - ft) > 0.) ||
        (std::fabs(ft + ft) > std::fabs(dxold * dft))) {
      dxold = dx;
      dx = 0.5 * (t1 - t0);
      t = t0 + dx;
      if ((t0 == t) || (dx < eps))
      ...
      T temp = t;
      t -= dx;
      if ((temp == t) || (std::fabs(dx) < eps))
        return std::max(t - eps, t0);
    }
...
T FindFirstTransitionFromPosToNegOfPolynomNewton(T t0, T t1,
  ...
  T e = -0.5 *
        (p.Coefficients(1) / p.Coefficients(2)); // determine inflection point
...
double FindFirstRoot(double t0, double t1, const Polynom<double, 3> &p) {
  ...
  double Discriminant = p.Coefficients()[2] * p.Coefficients()[2] -
                        3. * p.Coefficients()[1] * p.Coefficients()[3];
  if (Discriminant >= 0) { // p' has real roots

    // calculate roots of p'
    double Q = -(p.Coefficients()[2] + my::math::SignPos(p.Coefficients()[2]) *
...
double FindFirstRoot(double t0, double t1, const Polynom<double, 2> &p) {
  ...
  double Discriminant = p.Coefficients()[1] * p.Coefficients()[1] -
                        4. * p.Coefficients()[0] * p.Coefficients()[2];
  if (Discriminant >= 0) { // p has real roots

    // calculate roots of p
    double Q = -(p.Coefficients()[1] + my::math::SignPos(p.Coefficients()[1]) *
                                           std::sqrt(Discriminant)) *
               0.5;
...
template <>
void DetermineCoefficientsForImplicitRayTracing<2>(const double *Position,
                                                   const double *Direction,
                                                   const double *Rho,
                                                   double *Coefficients) {
...
template <>
void DetermineCoefficientsForImplicitRayTracing<3>(const double *Position,
                                                   const double *Direction,
                                                   const double *Rho,
                                                   double *Coefficients) {
  ...
  Coefficients[1] +=
      ((Position[1] * Rho[3] + Rho[1]) * Direction[0] + Direction[1] * tmp);
  Coefficients[0] += (Position[1] * tmp + Rho[1] * Position[0] + Rho[0]);
}

template <>
void CalculateNormal<3>(double *n, const double *v, const double *Rho) {
  n[0] = v[1] * Rho[7] + Rho[5];
  n[2] = n[0];
...
template <>
void CalculateNormal<2>(double *n, const double *v, const double *Rho) {
  n[0] = Rho[1] + v[1] * Rho[3];
  n[1] = Rho[2] + v[0] * Rho[3];
...
// Four rate Hubbart Interpolation. Algorithm works only in 3D, input 2D vector
// (vx,vy) is treated as (vx,vy,0)
template <class T, int D>
T fourRateInterpolation(lvlset::vec<T, D> nv, lvlset::vec<T, 3> direction100,
                        lvlset::vec<T, 3> direction010, T r100, T r110, T r111,
                        T r311) {
  ...
  directions[1] = Normalize(directions[1] -
                            directions[0] * dot(directions[0], directions[1]));
  ...
  for (int i = 0; i < 3; ++i)
    assert(dot(directions[i], directions[(i + 1) % 3]) < 1e-6);

  lvlset::vec<T, 3> N;

  for (int i = 0; i < 3; i++)
    N[i] = std::fabs(directions[i][0] * NormalVector[0] +
                     directions[i][1] * NormalVector[1] +
                     directions[i][2] * NormalVector[2]);
  N.reverse_sort();
...
template <class T>
inline T weno3(T x1, T x2, T x3, T x4, T x5, T dx, bool plus = true,
               T eps = 1e-6) {
...
// plus == true => right-sided
template <class T>
inline T weno5(T x1, T x2, T x3, T x4, T x5, T x6, T x7, T dx, bool plus = true,
               T eps = 1e-6) {
  ...
  if (plus == false) {
    ...
    T s1 = 13 / 12.0 * pow2(v1 - 2 * v2 + v3) +
           1 / 4.0 * pow2(v1 - 4 * v2 + 3 * v3);
    T s2 = 13 / 12.0 * pow2(v2 - 2 * v3 + v4) + 1 / 4.0 * pow2(v2 - v4);
    T s3 = 13 / 12.0 * pow2(v3 - 2 * v4 + v5) +
           1 / 4.0 * pow2(3 * v3 - 4 * v4 + v5);
