#ifndef PARTITION3_DownLinkTreeWithBoxInfoH_
#define PARTITION3_DownLinkTreeWithBoxInfoH_
...
template <class C> class DownLinkTreeWithBoxInfo {
...
public:
  class subbox {
    ...
    friend class DownLinkTreeWithBoxInfo;
  ...
  public:
    IntCoordType Extension(int dim) const {
      return NodeArray[ParentNodes.back()].Extension(dim);
    }

    IntCoordType Max(int dim) const {
      return NodeArray[ParentNodes.back()].Max(dim);
    }

    IntCoordType Min(int dim) const {
      return NodeArray[ParentNodes.back()].Min(dim);
    }

    CellRefType Cell() const { return NodeArray[ParentNodes.back()].Cell; }

    bool ContainsCell() const {
      return ((NodeArray[ParentNodes.back()].IsLeaf()) &&
  ...
  IntLinkType CountLeaves() const {
    ...
    for (typename C::IntLinkType i = 0; i < NodeList.size(); i++) {
      if (NodeList[i].SplitDirection == -1)
        counter++;
    }
  ...
  IntLinkType CountNodes() const { return NodeList.size(); }

  IntLinkType UndefinedLink() const { return C::UndefinedLink(); }

  CellRefType UndefinedCellRef() const { return C::UndefinedCellRef(); }
...
template <class C> void DownLinkTreeWithBoxInfo<C>::ClearPartition() {
  NodeList.clear();
}

template <class C>
template <class CoordsType>
void DownLinkTreeWithBoxInfo<C>::Setup(const CellRefType &start,
    ...
    default: {
      ...
      for (int dir = 0; dir < D; dir++) {
        ...
        for (IntCoordType l = 1; l < ext; l++) {
          ...
          if ((NA == 0) || (NB == 0))
            cost *= C::SurfaceAreaHeuristicLambda;
          if (cost < best_cost) {
            best_cost = cost;
            SplitDirection = dir;
            SplitIndex = min + l;
...
template <class C>
template <class V>
typename DownLinkTreeWithBoxInfo<C>::subbox
DownLinkTreeWithBoxInfo<C>::Access(V &pos, int Direction,
                                   bool DirectionSign) const {
...
template <class C>
template <class V>
bool DownLinkTreeWithBoxInfo<C>::GoToNeighborBox(subbox &sb, V &pos,
                                                 int Direction,
                                                 bool DirectionSign) const {
...
#endif /*PARTITION3_DownLinkTreeWithBoxInfoH_*/
