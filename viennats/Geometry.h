...
public:
  ...
  void ReadSTR(
    ...
    std::cout << "number of tetrahedrons: " << number_of_tetrahedrons
              << std::endl;

    if (D != dim_geometry)
      throw std::runtime_error(
          "Geometry dimension of reader does not fit input file - Aborting!");

    // read one more dummy line, won't need this one, this is just to advance
    // the file pointer
    std::getline(reader, line);
    ...
    if (D == 3) {
      ...
      // provided in the input file. Therefore we have to advance the file
      // pointer by the number of triangles - the '+1' is required to move the
      // file poiniter into the first 'tetrahedron' line
      ...
      for (int i = 0; i < number_of_tetrahedrons; i++) {
        ...
        if (std::find(ignore_materials.begin(), ignore_materials.end(),
                      region_id) != ignore_materials.end()) {
          ignore = true;
        } else
          ignore = false;
        ...
        if (!ignore) {
          if (change_input_parity)
            std::swap(cell_indices[0], cell_indices[1]);
          Elements.push_back(cell_indices);
        }

        // advance file pointer, omitting the additional meta data for each
        // tetrahedron
        for (int j = 0; j < D + 1; j++)
          reader >> dump;
      }
      ...
      throw std::runtime_error(
          "2D files are currently not supported - Aborting!");

    // correct material IDs
    // note: material IDs need to start with 1 and have a 1-increment ID
    // if we ignored some materials, having a small ID, e.g., 1, we need to
    // decrement all other material ids accordingly we start from the largest
    // region to be ignored and process the material set one after another
    for (std::vector<int>::reverse_iterator rit = ignore_materials.rbegin();
         rit != ignore_materials.rend(); ++rit) {
      for (std::vector<int>::iterator it = Materials.begin();
           it != Materials.end(); ++it) {
        if (*it > *rit)
          --(*it);
      }
  ...
  void
  ReadTDR(const std::string &FileName,
          double scale,                                   //=1.,
          std::vector<int> InputTransformationDirections, //=std::vector<int>(),
          std::vector<bool> InputTransformationSigns, //=std::vector<bool>(),
          bool change_input_parity,                   //=false,
    ...
    if (D != geometry.dim)
      msg::print_error("Dimension in parameters file does not match geometry!");
    delete file;
    ...
    for (unsigned int i = 0; i < geometry.nvertices; i++) {
      ...
      for (int j = 0; j < D; j++)
        coords[j] = geometry.vertex[geometry.dim * i + j];
    ...
    int num_elems = 0;
    for (map<string, region_t>::iterator S = geometry.region.begin();
         S != geometry.region.end(); S++)
      num_elems += S->second.nelements;
    ...
    for (map<string, region_t>::iterator S = geometry.region.begin();
      ...
      for (vector<vector<int>>::iterator E = elements.begin();
           E != elements.end(); E++) {
        std::vector<int> e = *E;
        for (int j = 0; j < D + 1; j++)
          Elements[i][j] = e[j];
  ...
  void
  ReadGRD(const std::string &FileName,
          double scale,                                   //=1.,
          std::vector<int> InputTransformationDirections, //=std::vector<int>(),
          std::vector<bool> InputTransformationSigns, //=std::vector<bool>(),
          bool change_input_parity,                   //=false,
    ...
    if (!f)
      msg::print_error("Failed reading geometry file!");
    ...
    for (int i = 0; i < num_elems; i++) {
      for (int j = -1; j < D + 1; j++) {
        ...
        if ((D == 3) && (j == 0)) {
          for (int k = 0; k < D; k++)
            Elements[i][j + k] = Faces[elem_tmp][k];
        } else if ((D == 3) && (j == 1)) {
          int k = 0;
          do {
            Elements[i][3] = Faces[elem_tmp][k++];
          } while ((Elements[i][3] == Elements[i][0]) ||
                   (Elements[i][3] == Elements[i][1]) ||
  ...
  void
  ReadDX(const std::string &FileName,
         double scale,                                   //=1.,
         std::vector<int> InputTransformationDirections, //=std::vector<int>(),
         std::vector<bool> InputTransformationSigns,     //=std::vector<bool>(),
         bool change_input_parity,                       //=false,
  ...
  void
  ReadVTK(const std::string &FileName,
          double scale,                                   //=1.,
          std::vector<int> InputTransformationDirections, //=std::vector<int>(),
          std::vector<bool> InputTransformationSigns, //=std::vector<bool>(),
          bool change_input_parity,                   //=false,
    ...
    std::ifstream f(FileName.c_str());
    if (!f)
      msg::print_error("Could not open geometry file!");
    std::string c;

    // Check if geometry is an unstructured grid as required
    while (std::getline(f, c)) {
      if (c.find("DATASET") != std::string::npos)
        break;
    }
    ...
    while (std::getline(f, c)) {
      if (c.find("POINTS") != std::string::npos)
        break;
    }
    ...
    for (int i = 0; i < num_nodes; i++) {
      ...
      for (int j = 0; j < D; j++) {
        ...
        int shift_size = shift.size();
        if (shift_size > j)
          Nodes[i][j] += shift[j]; // Assign desired shift
        if (InputTransformationSigns[j])
          Nodes[i][j] = -Nodes[i][j]; // Assign sign transformation, if needed
    ...
    while (std::getline(f_ct, c)) {
      if (c.find("CELL_TYPES") == 0)
        break;
    }
    int num_cell_types = atoi(&c[c.find(" ") + 1]);
    // need a cell_type for each cell
    if (num_elems != num_cell_types) {
      msg::print_error("Corrupt input geometry! Number of CELLS and CELL_TYPES "
                       "is different!");
    }
    ...
    while (std::getline(f_m, c)) {
      if (c.find("CELL_DATA") != std::string::npos) {
        std::getline(f_m, c);
        if ((c.find("SCALARS material") != std::string::npos) ||
            (c.find("SCALARS Material") != std::string::npos)) {
          std::getline(f_m, c);
          break;
        }
      }
  ...
  void
  ReadVTU(const std::string &FileName,
          double scale,                                   //=1.,
          std::vector<int> InputTransformationDirections, //=std::vector<int>(),
          std::vector<bool> InputTransformationSigns, //=std::vector<bool>(),
          bool change_input_parity,                   //=false,
  ...
  Read(std::string const &FileName,
    ...
    while (InputTransformationDirections.size() < D)
      InputTransformationDirections.push_back(
          InputTransformationDirections
              .size()); // TODO test if directions are unique
    while (InputTransformationSigns.size() < D)
      InputTransformationSigns.push_back(false);

    if ((InputTransformationDirections[0] + 1) % D !=
        InputTransformationDirections[1])
      change_input_parity = !change_input_parity;

    for (int i = 0; i < D; ++i)
      if (InputTransformationSigns[i])
        change_input_parity = !change_input_parity;
    ...
    if (GeometryFile.find(".tdr") == (GeometryFile.size() - 4)) {
      ReadTDR(GeometryFile, scale, InputTransformationDirections,
              InputTransformationSigns, change_input_parity, shift);
    } else
#endif
        if (FileName.substr(FileName.find_last_of(".") + 1) == "str") {
      ReadSTR(FileName, scale, InputTransformationDirections,
              InputTransformationSigns, change_input_parity, shift,
              ignore_materials);
    } else if (GeometryFile.find(".grd") == (GeometryFile.size() - 4)) {
      ReadGRD(GeometryFile, scale, InputTransformationDirections,
              InputTransformationSigns, change_input_parity, shift);
    } else if (GeometryFile.find(".dx") == (GeometryFile.size() - 3)) {
      ReadDX(GeometryFile, scale, InputTransformationDirections,
             InputTransformationSigns, change_input_parity, shift);
    } else if (GeometryFile.find(".vtk") == (GeometryFile.size() - 4)) {
      ReadVTK(GeometryFile, scale, InputTransformationDirections,
              InputTransformationSigns, change_input_parity, shift);
    } else if (GeometryFile.find(".vtu") == (GeometryFile.size() - 4)) {
      ReadVTU(GeometryFile, scale, InputTransformationDirections,
              InputTransformationSigns, change_input_parity, shift);
    } else {
#ifdef USE_HDF5
      msg::print_error("This software accepts only STR, TDR, GRD, DX and VTK "
                       "geometry files!");
#else
      msg::print_error(
          "This software accepts only STR, GRD, DX and VTK geometry files!");
#endif
    }
  ...
  void WriteVTK(const std::string &FileName) const {
    ...
    for (unsigned int i = 0; i < Nodes.size(); i++) {
      for (int j = 0; j < D; j++)
        f << static_cast<float>(Nodes[i][j]) << " ";
      if (D == 2)
        f << "0. ";
      f << std::endl;
    }
    f << "CELLS " << Elements.size() << " " << ((D + 2) * Elements.size())
      << std::endl;
  ...
  void Write(const std::string &FileName) const { WriteVTK(FileName); }
...
public:
  ...
  unsigned int number_of_nodes() const { return Nodes.size(); }

  unsigned int number_of_elements() const { return Elements.size(); }
  ...
  void Write(std::string FileName) const {
    ...
    f << "object \"positions\" class array type float rank 1 shape " << D
      << " items " << Nodes.size() << " data follows" << std::endl;
    for (unsigned int i = 0; i < Nodes.size(); i++) {
      for (int j = 0; j < D; j++)
        f << static_cast<float>(Nodes[i][j]) << " ";
      f << std::endl;
    ...
    f << "object \"connections\" class array type int rank 1 shape " << D
      << " items " << Elements.size() << " data follows" << std::endl;
    for (unsigned int i = 0; i < Elements.size(); i++) {
      for (int j = 0; j < D; j++)
        f << Elements[i][j] << " ";
      f << std::endl;
  ...
  void WriteVTK(std::string FileName) const {
    ...
    f << "CELLS " << Elements.size() << " " << ((D + 1) * Elements.size())
      << std::endl;
    for (unsigned int i = 0; i < Elements.size(); i++) {
      f << D << " ";
      for (int j = 0; j < D; j++)
  ...
  void ReadVTK(std::string FileName, double scale,
               std::vector<int> InputTransformationDirections,
               std::vector<bool> InputTransformationSigns,
               bool change_input_parity, std::vector<double> shift) {
  ...
  void ReadVTP(std::string FileName, double scale,
               std::vector<int> InputTransformationDirections,
               std::vector<bool> InputTransformationSigns,
               bool change_input_parity, std::vector<double> shift) {
    ...
    while (InputTransformationDirections.size() < D)
      InputTransformationDirections.push_back(
          InputTransformationDirections.size());
    while (InputTransformationSigns.size() < D)
      InputTransformationSigns.push_back(false);
...
template <class MaterialsType>
void GetMaterialNumbers(const MaterialsType &materials,
                        MaterialsType &discreteMaterials) {
  for (auto it = materials.begin(); it != materials.end(); ++it) {
    // if material is not yet in discreteMaterials
    if (std::find(discreteMaterials.begin(), discreteMaterials.end(), *it) ==
        discreteMaterials.end()) {
...
template <int D, class SurfacesType>
void TransformGeometryToSurfaces(const geometry<D> &Geometry,
                                 SurfacesType &Surfaces,
                                 std::bitset<2 * D> remove_flags, double eps,
  ...
  std::vector<unsigned> materialNumbers(materialInts.begin(),
                                        materialInts.end());

  // determine maximum number of materials
  unsigned max_mat = materialNumbers.back();

  typedef std::map<lvlset::vec<unsigned int, D>,
  ...
  for (unsigned int i = 0; i < Geometry.Elements.size(); ++i) {
    ...
    for (int j = 0; j < D + 1; j++) {
      ...
      for (int k = 0; k < D; k++)
        tmp[k] = Geometry.Elements[i][(j + k) % (D + 1)];
      ...
      if ((it != Triangles.end()) && (it->first == tmp)) {
        if (lvlset::Orientation(pts)) {
          if (report_import_errors && it->second.second != max_mat + 1) {
            std::ostringstream oss;
            oss << "Coinciding triangles with same orientation in Element: "
                << i << std::endl;
        ...
        if (lvlset::Orientation(pts)) {
          Triangles.insert(
              it, std::make_pair(
                      tmp, std::make_pair(max_mat + 1, Geometry.Materials[i])));
        } else {
          Triangles.insert(
              it, std::make_pair(
                      tmp, std::make_pair(Geometry.Materials[i], max_mat + 1)));
        }
      }
    }
  }
  ...
  for (auto matIt = materialNumbers.begin(); matIt != materialNumbers.end();
       ++matIt) {
    for (typename triangle_map::iterator it = Triangles.begin();
         it != Triangles.end(); ++it) {
    ...
    std::vector<unsigned int> NodeReplacements(Geometry.Nodes.size(),
                                               undefined_node);
...
template <int D, class SurfacesType>
void TransformHullToSurfaces(const surface<D> &Geometry, SurfacesType &Surfaces,
                             std::bitset<2 * D> remove_flags, double eps,
                             bool report_import_errors) {
  ...
  for (unsigned i = 0; i < Geometry.Elements.size(); ++i) {
    materialElements.push_back(
        std::make_pair(Geometry.Elements[i], Geometry.Materials[i]));
  }

  // elements will now be sorted based on material number
  std::sort(materialElements.begin(), materialElements.end(),
            [](const materialElementType &i, const materialElementType &j) {
              return i.second < j.second;
            });

  // cycle through all available materials and use all triangles of lower
  // materials as well
  for (auto materialIt = materialNumbers.begin();
       materialIt != materialNumbers.end(); ++materialIt) {
    ...
    // upper_bound returns iterator to first element greater than current
    // material therefore elemIterator is essentially our .end() element same
    // goes for elemStart as .begin() element
    auto elemStart = std::lower_bound(
        materialElements.begin(), materialElements.end(), *materialIt,
        [](const materialElementType &i, const int &j) {
          return i.second < j;
        });
    auto elemEnd = std::upper_bound(
        materialElements.begin(), materialElements.end(), *materialIt,
        [](const int &i, const materialElementType &j) {
          return i < j.second;
        });
    ...
    for (auto elemIt = elemStart; elemIt != elemEnd; ++elemIt) {
      ...
      auto degenerateIt = std::find(searchSurface.Elements.begin(),
                                    searchSurface.Elements.end(), element);
      if (degenerateIt != searchSurface.Elements.end()) { // degenerate found
        searchSurface.Elements.erase(degenerateIt);       // remove element
        Surfaces.back().Elements.erase(
            Surfaces.back().Elements.begin() +
            std::distance(searchSurface.Elements.begin(),
                          degenerateIt)); // remove actual element
        continue;                         // ignore current element as well
      } else {
        searchSurface.Elements.push_back(
            element); // push element if it does not exist yet
        Surfaces.back().Elements.push_back(
            elemIt->first); // push element in actual surfaces
      }
    }

    // Take only the required nodes for the new surface
    const unsigned undefinedNode = std::numeric_limits<unsigned int>::max();
    std::vector<unsigned> NodeReplacements(Geometry.Nodes.size(),
                                           undefinedNode);
...
void import_levelsets_from_surface(GridTraitsType &GridProperties,
                                   lvlset::grid_type<GridTraitsType> &grid,
                                   ParameterType &p,
  ...
  std::cout << "The geometry consists of " << p.geometry_files.size()
            << " input surfaces. \n";

  //! surface.ReadVTK(...) reads surface file/s and modifies it/them according
  //! to the user-set parameters
  for (unsigned i = 0; i < p.geometry_files.size(); ++i) {
    ...
    if (p.geometry_files[i].find(".vtk") == (p.geometry_files[i].size() - 4)) {
      surfaces.back().ReadVTK(
          p.geometry_files[i], p.input_scale, p.input_transformation,
          p.input_transformation_signs, p.change_input_parity, p.input_shift);
    } else if (p.geometry_files[i].find(".vtp") ==
               (p.geometry_files[i].size() - 4)) {
      surfaces.back().ReadVTP(
          p.geometry_files[i], p.input_scale, p.input_transformation,
          p.input_transformation_signs, p.change_input_parity, p.input_shift);
    } else {
    ...
    for (int h = 0; h < D; ++h) {
      grid_min[h] = std::min(
          grid_min[h], int(std::ceil(surfaces.back().Min[h] / p.grid_delta -
                                     p.snap_to_boundary_eps)));
      grid_max[h] = std::max(
          grid_max[h], int(std::floor(surfaces.back().Max[h] / p.grid_delta +
                                      p.snap_to_boundary_eps)));
    }
    msg::print_done();
#ifdef VERBOSE
    std::cout << "min = " << (surfaces.back().Min) << "   "
              << "max = " << (surfaces.back().Max) << std::endl;
    std::cout << "min = " << (surfaces.back().Min / p.grid_delta) << "   "
              << "max = " << (surfaces.back().Max / p.grid_delta) << std::endl;
#endif
  }
  ...
  for (typename std::list<surface<D>>::const_iterator it = surfaces.begin();
       it != surfaces.end(); ++it) {
...
template <int D, class GridTraitsType, class ParameterType, class LevelSetType>
void import_levelsets_from_volume(GridTraitsType &GridProperties,
                                  lvlset::grid_type<GridTraitsType> &grid,
                                  ParameterType &p,
                                  std::list<LevelSetType> &LevelSets) {
  int grid_min[D] = {};
  ...
  geometry<D> g;
  // g.Read reads a geometry file and modifies it according to the user-set
  // parameters
  std::cout << "Read geometry input file " << p.geometry_files[0];
  msg::print_start("...");
  g.Read(p.geometry_files[0], p.input_scale, p.input_transformation,
         p.input_transformation_signs, p.change_input_parity,
         p.material_mapping, p.input_shift, p.ignore_materials);
  {
  ...
  std::cout << "min = " << (g.Min) << "   "
            << "max = " << (g.Max) << std::endl;
  std::cout << "min = " << (g.Min / p.grid_delta) << "   "
            << "max = " << (g.Max / p.grid_delta) << std::endl;
#endif
  msg::print_done();
  ...
  //! Transform the input volume geometry to surfaces and interfaces
  //! "TransformGeometryToSurfaces(...)"
  ...
  for (int i = 0; i < D; ++i) {
    if (p.boundary_conditions[i].min == bnc::PERIODIC_BOUNDARY ||
      ...
      remove_flags.set(i);
    } else if (i == p.open_boundary && !p.open_boundary_negative &&
               p.remove_bottom) {
      remove_flags.set(i);
    }
  ...
  TransformGeometryToSurfaces(g, Surfaces, remove_flags,
                              p.grid_delta * p.snap_to_boundary_eps,
                              p.report_import_errors);
  msg::print_done();

  msg::print_start("Distance transformation...");

  //! Initialize each level set with "lvlset::init(...)"
  for (typename SurfacesType::const_iterator it = Surfaces.begin();
       it != Surfaces.end(); ++it) {
    LevelSets.push_back(LevelSetType(grid));
    lvlset::init(LevelSets.back(), *it, p.report_import_errors);
    LevelSets.back().set_levelset_id();
...
/// This function takes a hull mesh, as output by ViennaTS and separates it into
/// different surfaces for each material, applying explicit wrapping, thus
/// conserving thin layers. The respective layers are then converted to level
/// sets
template <int D, class GridTraitsType, class ParameterType, class LevelSetType>
void import_levelsets_from_hull(GridTraitsType &GridProperties,
                                lvlset::grid_type<GridTraitsType> &grid,
                                ParameterType &p,
                                std::list<LevelSetType> &LevelSets) {
  ...
  TransformHullToSurfaces(geometry, surfaces, remove_flags,
                          p.grid_delta * p.snap_to_boundary_eps,
                          p.report_import_errors);
  ...
  for (int h = 0; h < D; ++h) {
    grid_min[h] =
        std::ceil(geometry.Min[h] / p.grid_delta - p.snap_to_boundary_eps);
    grid_max[h] =
        std::floor(geometry.Max[h] / p.grid_delta + p.snap_to_boundary_eps);
  }
#ifdef VERBOSE
  std::cout << "min = " << (geometry.Min) << "   "
            << "max = " << (geometry.Max) << std::endl;
  std::cout << "min = " << (geometry.Min / p.grid_delta) << "   "
            << "max = " << (geometry.Max / p.grid_delta) << std::endl;
#endif
  msg::print_done();
  ...
  // Initialize each level set with "lvlset::init(...) and wrap with material
  // below"
  for (typename std::list<surface<D>>::const_iterator it = surfaces.begin();
       it != surfaces.end(); ++it) {
    LevelSets.push_back(LevelSetType(grid));
