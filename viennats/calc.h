...
public:
  inline double operator[](int i) const { return (i < Dimensions) ? v[i] : 0.; }
...
public:
  ...
  static inline CellRefType UndefinedCellRef() {
    return std::numeric_limits<CellRefType>::max();
  } // return 0;}
  static inline IntLinkType UndefinedLink() {
    return std::numeric_limits<IntLinkType>::max();
  }

  // static const double
  // SurfaceAreaHeuristicLambda=ParameterType::SurfaceAreaHeuristicLambda;
  // static const partition::SplittingModeType
  // PartitionMode=ParameterType::PartitionMode;

  static const int Dimension = ParameterType::Dimension;
};

template <int D, class LS, class NormalVectorVectorClass,
          class DistancesVectorClass>
void SetupCells(const LS &l, geom::cells<D> &Cells,
  ...
  for (typename LS::template const_iterator_cells_filtered<
           typename LS::filter_active>
           it(l);
    ...
    if (cell_contains_disk) {
      ...
      for (unsigned int i = 0; i < (1 << D); i++) {
        //                  if (it.corner(i).pt_id()==1028) //std::cout <<
        //                  "it.corner("<<i<<").pt_id():" <<it.corner(i).pt_id()
        //                  <<std::endl;
  ...
  ////std::cout << "num_cells_equal_signs=" << cell_equal_signs_counter <<
  ///std::endl; /std::cout << "num_cells_contains_disk=" <<
  ///cell_contains_disk_counter << std::endl; /std::cout <<
  ///"num_cells_inserted=" << cell_inserted_counter << std::endl;
}

template <class LS>
void CalculateNormalVectors(
    const LS &l, std::vector<double> &NormalVectors,
    std::vector<double> &DistancesToReceiver, int open_boundary_direction,
    bool is_open_boundary_negative, double ReceptorRadius,
    const lvlset::vec<double, LS::dimensions> &default_directions =
        lvlset::vec<double, LS::dimensions>(0)) {
...
#pragma omp for schedule(static,                                               \
  ...
  for (int p = 0; p <= static_cast<int>(segmentation.size()); ++p) {
    typename LS::point_type begin_v =
        (p == 0) ? l.grid().min_point_index() : segmentation[p - 1];
    typename LS::point_type end_v =
        (p != static_cast<int>(segmentation.size()))
            ? segmentation[p]
            : l.grid().increment_indices(l.grid().max_point_index());

    for (typename LS::template const_iterator_neighbor_filtered<
             typename LS::filter_active, 1>
             it(l, typename LS::filter_active(), begin_v);
         it.indices() < end_v; it.next()) {
      ...
      for (int uu = 1; uu < D; uu++)
        tmp_max = std::max(tmp_max, std::fabs(n[uu]));

      if (tmp_max == 0.) {
        for (int uu = 0; uu < D; uu++)
          n[uu] = t[uu];
        dist = 0.;
      } else {
        double no2 = 0.;
        for (int uu = 0; uu < D; uu++)
          no2 += my::math::pow2(n[uu] / tmp_max);
      ...
      // reduce distance if receptor disk would not completely lie inside of
      // attached voxels

      for (int i = 0; i < D; i++) {
        dist = my::math::Sign(dist) *
               std::max(0., std::min(std::fabs(dist),
                                     (1. - ReceptorRadius *
                                               std::sqrt(std::max(
                                                   0., 1 - n[i] * n[i]))) /
                                         std::fabs(n[i])));
...
template <class LS>
void CalculateCurvatureVectors(const LS &l,
                               std::vector<double> &CurvatureVectors,
                               bool initialized) {
    ...
    for (typename LS::template const_iterator_neighbor_filtered<
      ...
      if (initialized) {
        for (unsigned int i = 0; i < it_neighbors.size(); i++)
          it_neighbors[i].go_to_indices_sequential(it.indices());
      } else {
        for (int i = -1; i <= 1; ++i) {
          for (int j = -1; j <= 1; ++j) {
            for (int k = -1; k <= 1; ++k) {
      ...
      double PhiX =
          (it_neighbors[XpY0Z0].value() - it_neighbors[XmY0Z0].value()) * 0.5;
      double PhiY =
          (it_neighbors[X0YpZ0].value() - it_neighbors[X0YmZ0].value()) * 0.5;
      double PhiZ =
          (it_neighbors[X0Y0Zp].value() - it_neighbors[X0Y0Zm].value()) * 0.5;

      double PhiXX = it_neighbors[XpY0Z0].value() +
                     it_neighbors[XmY0Z0].value() - 2 * it.center().value();
      double PhiYY = it_neighbors[X0YpZ0].value() +
                     it_neighbors[X0YmZ0].value() - 2 * it.center().value();
      double PhiZZ = it_neighbors[X0Y0Zp].value() +
                     it_neighbors[X0Y0Zm].value() - 2 * it.center().value();

      double PhiXY =
          (it_neighbors[XpYpZ0].value() + it_neighbors[XmYmZ0].value() -
           it_neighbors[XpYmZ0].value() - it_neighbors[XmYpZ0].value()) *
          0.25;
      double PhiXZ =
          (it_neighbors[XpY0Zp].value() + it_neighbors[XmY0Zm].value() -
           it_neighbors[XpY0Zm].value() - it_neighbors[XmY0Zp].value()) *
          0.25;
      double PhiYZ =
          (it_neighbors[X0YpZp].value() + it_neighbors[X0YmZm].value() -
           it_neighbors[X0YmZp].value() - it_neighbors[X0YpZm].value()) *
          0.25;
      ...
      double num = 0.5 * PhiX * PhiX * (PhiYY + PhiZZ) -
                   PhiY * PhiZ * PhiYZ + // mean curvature
      ...
      if (denom != 0) {
        *curv = num / (denom * std::sqrt(denom));
        ...
        //                      if ((k>max_curvature) || (k<min_curvature)) s=
        //                      -num/denom;
...
template <int D, class PartitionType> class ClusterPositionType {
public:
  double X[D];
...
// [Josef] Main function where particles are tracked and collision with
// interface is checked!
template <class ModelType, class ParameterType, class PartitionType,
          class LevelSetType>
void CalculateRates(const ModelType &Model, const ParameterType &Parameter,
  ...
  const double further_tracking_distance =
      Parameter.further_tracking_distance; // default is 3
...
#pragma omp parallel
  {
    ...
    // in case of equally distributed flux the number of starting places is
    ...
    const int NumStartingPlaces =
        (ModelType::SpatiallyEqualDistributedFlux)
            ? static_cast<int>(Partition.AreaSize(Parameter.open_boundary))
            : // AXIS
            my_num_threads;
...
#pragma omp for schedule(                                                      \
    dynamic) // Chunks are dynamically assigned to threads on a first-come,
      ...
      for (unsigned int ParticleType = 0;
            ...
            (ModelType::SpatiallyEqualDistributedFlux)
                ...
                : Model.NumberOfParticleClusters[ParticleType] /
                      ...
                      (my_thread_num <
                               static_cast<int>(
                                   Model
                                       .NumberOfParticleClusters[ParticleType] %
                                   my_num_threads)
                           ? 1
                           : 0);
        ...
        for (unsigned int ParticleCounter = 0; ParticleCounter < NumOfParticles;
          ...
          if (Parameter.open_boundary_negative) {
            if (p.Direction[Parameter.open_boundary] <= 0.)
              continue;
          } else {
            if (p.Direction[Parameter.open_boundary] >= 0.)
              continue;
          ...
          if (ModelType::SpatiallyEqualDistributedFlux) {
            ...
            for (int i = 0; i < D; ++i) {
              ...
              if (i != Parameter.open_boundary)
                cp.X[i] += my::stat::RandomNumber();
            }

            cp.Subbox = starting_subbox;

            // determine additional particles, which are necessary to account
            ...
            for (int i = 0; i < D - 1; ++i) {
              ...
              if (dir != Parameter.open_boundary) {
                if ((Parameter.boundary_conditions[dir].min ==
                     bnc::EXTENDED_BOUNDARY) &&
                    (p.Direction[dir] > 0)) {
                  zmax[i] = static_cast<int>(std::ceil(
                      (-std::min(
                           std::fabs(
                               (Partition.Extension(Parameter.open_boundary) *
                                p.Direction[dir]) /
                               p.Direction[Parameter.open_boundary]),
                           static_cast<double>(
                               Parameter.max_extended_starting_position)) -
                       (cp.X[dir] +
                        (cp.Subbox.Min(dir) - Partition.Min(dir)))) /
                      Partition.Extension(dir)));
                  assert(zmax[i] <= 0);
                } else if ((Parameter.boundary_conditions[dir].max ==
                            bnc::EXTENDED_BOUNDARY) &&
                           (p.Direction[dir] < 0)) {
                  zmax[i] =
                      static_cast<int>(std::floor(
                          (std::min(
                               std::fabs((Partition.Extension(
            ...
            while (true) {
              ...
              for (int g = 0; g < D - 1; ++g) {
                ...
                new_cp.X[dir] =
                    cp.X[dir] +
                    static_cast<double>(cp.Subbox.Min(dir) -
                                        Partition.Min(dir)) + // TODO check!!!
                    static_cast<double>(counter[g]) *
                        static_cast<double>(Partition.Extension(dir));
              }
              new_cp.Subbox =
                  Partition.Access(new_cp.X, Parameter.open_boundary,
                                   Parameter.open_boundary_negative);
            ...
            for (int i = 0; i < D; ++i)
              cp.X[i] = StartPosition[i] /
                        Parameter.grid_delta; // scale starting position

            double t = -(cp.X[Parameter.open_boundary] -
                         ((Parameter.open_boundary_negative)
                              ? Partition.Min(Parameter.open_boundary)
                              : Partition.Max(Parameter.open_boundary))) /
                       p.Direction[Parameter.open_boundary];

            // Move cp.X to the top LS surface and update horizontal axis values
            // (not open boundary value)
            for (int dir = 0; dir < D; ++dir) {
              if (dir != Parameter.open_boundary) {
                bool ReverseSign;
                cp.X[dir] = Parameter.boundary_conditions[dir].map_coordinate(
                    Partition.Min(dir), Partition.Max(dir),
                    cp.X[dir] + p.Direction[dir] * t, ReverseSign);
                if (ReverseSign)
                  p.Direction[dir] = -p.Direction[dir];
              }
            }
            cp.Subbox = Partition.Access(cp.X, Parameter.open_boundary,
                                         Parameter.open_boundary_negative);
            // cp.X is now position within subbox after removing the "global
            // components"
          }
          ...
          while (true) {
            ...
            for (int r = 0; r < D; ++r)
              last_surface_cell_indices[r] =
                  Partition.Min(r) - 2; // initialize with invalid indices

            // Iterate through the cells between LS.Max() and LS.Min() until
            // surface reached or particle exits environment
            while (true) {
              ...
              int LeavingDirection =
                  -1; // LeavingDirection : 0,1,2 particle leaves box in x,y,z
                      // direction respectively

              std::bitset<D> PositionStatusPos; // for each direction the bit is
                                                // set if a particle is outside
                                                // (in positive direction) of
                                                // the regular simulation domain
              std::bitset<D> PositionStatusNeg; // for each direction the bit is
                                                // set if a particle is outside
              ...
              for (i = 0; i < D; i++) {
                ...
                // Subbox.Extension(dir) is the length of the subbox in the dir
                // direction Subbox.Min(dir) is the global coordinate (grid
                // points) of the Subbox edge in the min dir direction
                // Subbox.Max(dir) is the global coordinate (grid points) of the
                // Subbox edge in the max dir direction Global coordinate is
                // then found by Subbox.Min(dir)+cp.X[dir]

                // When outside the min extended boundary
                if ((cp.X[i] <= 0) && (Parameter.boundary_conditions[i].min ==
                                       bnc::EXTENDED_BOUNDARY)) {
                  if (p.Direction[i] > 0.) {
                    if (cp.X[i] == 0) {
                    ...
                    if (cp.X[i] < 0 /*-Parameter.DomainExtension*/)
                      break;
                    PositionStatusNeg.set(i);
                  }
                  // When outside the max extended boundary
                } else if ((cp.X[i] >= Subbox.Extension(i)) &&
                           (Parameter.boundary_conditions[i].max ==
                            bnc::EXTENDED_BOUNDARY)) {
                  if (p.Direction[i] < 0.) {
                    ...
                    if (cp.X[i] >
                        Subbox.Extension(i) /*+Parameter.DomainExtension*/)
                      break;
                    PositionStatusPos.set(i);
                  }
                } else {
                  if (p.Direction[i] > 0.) {
                    // t_temp is the variable to determine time to reach
                    // Subbox.Extension(i) from cp.X in p.Direction[i]
                    t_temp = (Subbox.Extension(i) - cp.X[i]) / p.Direction[i];
              ...
              // cp.X remains unchanged at this point, only max_distance_in_box
              // and LeavingDirection are found
              //                              //std::cout << "cp.X4 " << "(" <<
              //                              cp.X[0] << "," << cp.X[1] << ","
              //                              << cp.X[2] << ")\n";
              //                //std::cout << "max_dinstance_in_box: " <<
              //                max_distance_in_box << "\n";
              //                //std::cout << "LeavingDirection: " <<
              //                LeavingDirection << "\n";
              // Now have max_distance_in_box = distance from cp.X to point of
              // exit in the i direction where i is the leaving direction
              // (x,y,z)=(0,1,2)

              if ((i != D) || (LeavingDirection == -1))
                break;

              // When the subbox which received the particle also contains
              // within it the surface boundary
              if (Subbox.ContainsCell()) {
                ...
                // std::cout << "containsCell\n";
                // Calculate the exit direction and distance as before to see if
                // surface is intersected
                //#######################################################
                //# check for surface intersection                      #
                //#######################################################

                //[Josef] This is where the particle is tracked within a subbox
                ...
                if (travelled_distance_from_intersection ==
                  ...
                  for (int i = 0; i < (1 << D); i++) {
                    Rho[i] = SurfaceLevelSet.value2(
                        Cell.Points[((std::bitset<D>(i) | PositionStatusPos) &
                                     (~PositionStatusNeg))
                                        .to_ulong()]);
                  ...
                  if (sgn_count != (1 << D)) {
                    ...
                    if (sgn_count == 0) {
                      ...
                      my::math::DetermineCoefficientsForImplicitRayTracing<D>(
                          cp.X, p.Direction, Rho, &(poly.Coefficients()[0]));
                      //                        std::cout << "cp.X5: (" <<
                      //                        cp.X[0] << "," << cp.X[1] << ","
                      //                        << cp.X[2] << ")\n";
                      relative_distance_to_intersection = my::math::
                          FindFirstTransitionFromPosToNegOfPolynomNewton(
                              0., max_distance_in_box, poly, 1e-6);
                    }

                    if (relative_distance_to_intersection <
                        std::numeric_limits<double>::max()) { // if particle
                                                              // hits surface
                      // std::cout << "hits\n";
                      travelled_distance_from_intersection =
                          -relative_distance_to_intersection;

                      ClusterPositionType new_cp = cp;

                      for (int kk = 0; kk < D; kk++)
                        new_cp.X[kk] +=
                            relative_distance_to_intersection * p.Direction[kk];
                      //                          std::cout << "RDI\n";
                      // p.Direction[kk] is unchanged at this point
                      // new_cp.X contains the coordinates within the Subbox at
                      // the subbox exit point

                      // determine normal vector
                      double tmp_normalvec[3];
                      my::math::CalculateNormal<D>(tmp_normalvec, new_cp.X,
                                                   Rho);
                      if (D == 2)
                        tmp_normalvec[2] = 0.;
                      //                                                std::cout
                      ...
                      for (int w = 1; w < D; ++w)
                        dot += tmp_normalvec[w] * p.Direction[w];

                      if (dot >= 0.) {
                        msg::print_warning("Particle hits negative side of "
                                           "surface! Particle is skipped.");
                        break;
                      }

                      // calculate nearest active grid point to determine
                      // coverages and material (using Manhattan distance for
                      // speedup)
                      unsigned int gp = 0;
                      unsigned int mat = 0;
                      if ((ModelType::CoverageStorageSize > 0) ||
                          (ModelType::ReemissionIsMaterialDependent)) {
                        //                              std::cout << "CSS>0
                        //                              RMD\n";
                        ...
                        for (int g = 0; g < (1 << D); g++) {
                          ...
                          //                                  " << g << "\n";
                          unsigned int tmp_gp =
                              SurfaceLevelSet.active_pt_id(Cell.Points[g]);
                          //                                  std::cout <<
                          //                                  "tmp_gp = " <<
                          //                                  tmp_gp << "\n";
                          //                                  std::cout <<
                      ...
                      //                          std::cout << "Reflection\n";
                      Model.ParticleReflexion(
                          p, ParticleStack, tmp_normalvec,
                          &Coverages[gp * Model.CoverageStorageSize],
                          mat //, D, dot
                      );
                      while (ParticleStack.size() >
                             ParticlePositionsStack.size())
                        ParticlePositionsStack.push(new_cp);
                    }
                  }
                }
                ...
                if (PositionStatusPos.none() && PositionStatusNeg.none()) {
                  ...
                  //#####################################################################
                  //# determine corners which have to be checked for disk
                  //intersections #
                  //#####################################################################
                  ...
                  for (int s = 0; s < D; ++s)
                    last_surface_cell_indices[s] = Subbox.Min(s);

                  //#######################################################
                  //# check for disk intersections                        #
                  //#######################################################

                  //[Josef] This is where the four corners of the box containing
                  //the particle are checked for intersection all 8 neighbors
                  // have to be checked if they are active and their disks are
                  // hit
                  for (int g = 0; g < (1 << D); g++) {
                    ...
                    if (corners.test(g)) {
                      ...
                      if (gp != LevelSetType::INACTIVE) {
                        ...
                        for (int kk = 1; kk < D; kk++)
                          cos -= NormalVectors[gpD + kk] * p.Direction[kk];
                        ...
                        if (cos > 0.) {
                          ...
                          double rel_pos[D];
                          for (int kk = 0; kk < D; kk++)
                            rel_pos[kk] = cp.X[kk] - ((g >> kk) & 1) -
                                          NormalVectors[gpD + kk] *
                                              DistancesToReceiver[gp];
                          // rel_pos holds cp.X not cp_new.X

                          // calculate rel_pos*disk_normal
                          double rel_pos_dot_normal =
                              rel_pos[0] * NormalVectors[gpD];
                          for (int kk = 1; kk < D; kk++)
                            rel_pos_dot_normal +=
                                rel_pos[kk] * NormalVectors[gpD + kk];

                          if (rel_pos_dot_normal <=
                              (further_tracking_distance -
                               travelled_distance_from_intersection) *
                                  cos) {

                            double tmpx = my::math::pow2(
                                rel_pos[0] * cos +
                                p.Direction[0] * rel_pos_dot_normal);
                            for (int kk = 1; kk < D; kk++)
                              tmpx += my::math::pow2(rel_pos[kk] * cos +
                                                     p.Direction[kk] *
                                                         rel_pos_dot_normal);
                            ...
                            if (tmpx <= cos * cos * ReceptorRadius2) {
                              ...
                              for (int kk = 0; kk < D; kk++) {
                                if (kk != Parameter.open_boundary) {
                                  if ((Parameter.boundary_conditions[kk].min ==
                                       bnc::REFLECTIVE_BOUNDARY) ||
                                      (Parameter.boundary_conditions[kk].min ==
                                       bnc::EXTENDED_BOUNDARY)) {
                                    if ((g & (1 << kk)) == 0) {
                              ...
                              for (; Factor > 0; --Factor) {
                                //[Josef] Here, the particle has collided with
                                //the surface so the model's function to deal
                                //with this is called.
                                ...
                                //                                                  mat = PointMaterials[gp];
                                // std::cout << "Collision\n";
                                Model.ParticleCollision(
                                    p, Make3DVector<D>(&NormalVectors[gpD]),
                                    //&NormalVectors[gpD],
              ...
              //######################################################################
              //# check if calculation of particle cluster trajectory can be
              //stopped #
              //######################################################################
              ...
              for (int kk = 0; kk < D; kk++) {
                if (kk != LeavingDirection)
                  cp.X[kk] += p.Direction[kk] * max_distance_in_box;
              }

              if (PositionStatusNeg.test(
                      LeavingDirection)) { // particle enters regular simulation
                                           // domain from negative side
                cp.X[LeavingDirection] = 0;
              } else if (PositionStatusPos.test(
                             LeavingDirection)) { // particle enters regular
                                                  // simulation domain from
                                                  // positive side
                ...
                bool IsDirectionPositive =
                    (p.Direction[LeavingDirection] >= 0.);

                if (Partition.GoToNeighborBox(cp.Subbox, cp.X, LeavingDirection,
                                              IsDirectionPositive)) {

                  if (IsDirectionPositive) {
                    if (Parameter.boundary_conditions[LeavingDirection].max ==
                        bnc::INFINITE_BOUNDARY)
                      break;

                    if (Parameter.boundary_conditions[LeavingDirection].max ==
                        bnc::REFLECTIVE_BOUNDARY)
                      p.Direction[LeavingDirection] =
                          -p.Direction[LeavingDirection];
                    cp.X[LeavingDirection] =
                        cp.Subbox.Extension(LeavingDirection);

                  } else {
                    if (Parameter.boundary_conditions[LeavingDirection].min ==
                        bnc::INFINITE_BOUNDARY)
                      break;
                  ...
                  last_surface_cell_indices[LeavingDirection] =
                      Partition.Min(LeavingDirection) - 2;
                  ...
                  cp.X[LeavingDirection] =
                      (IsDirectionPositive)
                          ...
                          : cp.Subbox.Extension(LeavingDirection);
                  if ((Parameter.boundary_conditions[LeavingDirection].min ==
                       bnc::PERIODIC_BOUNDARY) &&
                      PositionStatusPos.none() && PositionStatusNeg.none()) {
                    if (IsDirectionPositive) {
                      if (old_min >= cp.Subbox.Min(LeavingDirection))
                        last_surface_cell_indices[LeavingDirection] -=
                            Partition.Extension(LeavingDirection);
                    } else {
                      if (old_min <= cp.Subbox.Min(LeavingDirection))
                        last_surface_cell_indices[LeavingDirection] +=
                            Partition.Extension(LeavingDirection);
    ...
    if (Model.OutputFluxes) {
      {
        ...
        for (typename LevelSetType::const_iterator_runs it(SurfaceLevelSet);
             !it.is_finished(); it.next()) {
          if (it.active_pt_id() != LevelSetType::INACTIVE) {
            for (int j = 0; j < D; j++)
              outputfile << (it.start_indices()[j]) << " ";
      ...
      }
      {
        ...
        for (typename LevelSetType::const_iterator_runs it(SurfaceLevelSet);
          ...
          if (it.active_pt_id() != LevelSetType::INACTIVE) {
            for (int j = 0; j < D; j++)
              outputfile << (it.start_indices()[j]) * Parameter.grid_delta
                         << " ";
            outputfile << Rates[it.active_pt_id()] << std::endl;
          }
        }
...
//  template<class ModelType> void UpdateCoverages(const std::vector<double>&
//  Rates, std::vector<double>& Coverages, const ModelType& Model) {
template <class ModelType>
void UpdateCoverages(const std::vector<double> &Rates,
                     std::vector<double> &Coverages, const ModelType &Model,
                     double &time_step) { //, double &current_time) {
