...
const boundary_type INFINITE_BOUNDARY =
    1; // no boundary, the maximum allowed index is only
       // limited by the constant INF_EXTENSION

const boundary_type PERIODIC_BOUNDARY = 2; // periodic boundary conditions

const boundary_type POS_INFINITE_BOUNDARY =
    3; // one-sided reflective boundary conditions,
       // no boundary in positive direction (for positive
       // indices)

const boundary_type NEG_INFINITE_BOUNDARY =
    4; // one-sided reflective boundary conditions,
...
template <class GridTraitsType> class grid_type {
  ...
  //   public:
  //       typedef int index_type;         //the data type used for the grid
  //       indices typedef double coord_type;      //the data type used for the
  //       coordinates
  //
  //       const static int dimensions=2;  //number of dimensions (2 or 3
  //       allowed)
  //
  //       index_type min_index(int dir) const;    //for each grid direction dir
  //       (x=0,y=1,z=2)
  //                                               //the minimum grid index has
  //                                               to be returned
  //       index_type max_index(int dir) const;    //for each grid direction dir
  //       (x=0,y=1,z=2)
  //                                               //the minimum grid index has
  //                                               to be returned
  //
  //        coord_type grid_position(int dir, index_type Index) const; //this
  //        function returns the coordinates
  //                                                                       //of
  //                                                                       the
  //                                                                       grid
  //                                                                       planes
  ...
  //                                                                       below)
  //
  //       boundary_type boundary_condition(int dir) const;    //for each grid
  //       direction "dir" the boundary condition type
  //                                                           //has to be
  //                                                           specified,
  //                                                           returning one of
  //                                                           the
  //                                                           boundary_type-constants
  //                                                           //defined above
  //
  // IMPORTANT: the GridTraitsType has to be copyable, make sure that you define
  // a appropriate copy-constructor if the default copy-constructor does not
  // work for your class
...
private:
  ...
  vec<index_type, D> Min_, Max_,
      Ext_; // Minimum, maximum, extension of whole grid for all grid directions

  vec<boundary_type, D> BoundaryConditions_; // here the boundary conditions for
                                             // all grid directions are stored

  vec<index_type, D> MinGridPointCoord_,
      MaxGridPointCoord_; // effective maximum and minimum grid point
                          ...
                          // MaxGridPointCoord_[k]=Max_[k]-1 holds

  vec<bool, D> parities; // here the parities of the grid are stored, the parity
                         // is false if the grid_position function is strictly
                         // monotonic increasing and true if the grid_position
                         // function is strictly monotonic decreasing
...
public:
  grid_type(const GridTraitsType &gt) : GridTraits(gt) { // constructor
    for (int i = 0; i < D; i++) {
      ...
      if (GridTraits.boundary_condition(i) == PERIODIC_BOUNDARY)
        MaxGridPointCoord_[i]--;
    }
    Ext_ = Max_ - Min_;
    // initialize parities
    for (int i = 0; i < D; i++) {
      parities[i] = (GridTraits.grid_position(i, Min_[i]) >
                     GridTraits.grid_position(i, Max_[i]));
    }
  }

  // copy constructor
  grid_type(const grid_type &gt)
      : GridTraits(gt.GridTraits), Min_(gt.Min_), Max_(gt.Max_), Ext_(gt.Ext_),
        BoundaryConditions_(gt.BoundaryConditions_),
        MinGridPointCoord_(gt.MinGridPointCoord_),
        MaxGridPointCoord_(gt.MaxGridPointCoord_), parities(gt.parities) {}
  // empty constructor
  ...
  index_type grid_extent(int dim) const { return Ext_[dim]; }

  index_type min_grid_index(int dim) const { return Min_[dim]; }

  index_type max_grid_index(int dim) const { return Max_[dim]; }

  inline const vec<index_type, D> &min_grid_index() const { return Min_; }

  inline const vec<index_type, D> &max_grid_index() const { return Max_; }
  ...
  bool is_pos_boundary_infinite(int dim) const {
    // returns true if the boundary is infinite for positive indices in
    // "dim"-direction
  ...
  bool is_neg_boundary_infinite(int dim) const {
    // returns true if the boundary is infinite for negative indices in
    // "dim"-direction
    return ((BoundaryConditions_[dim] == INFINITE_BOUNDARY) ||
            (BoundaryConditions_[dim] == NEG_INFINITE_BOUNDARY));
  }

  template <class V> bool is_cell_member(const V &vec) const {
    for (int i = 0; i < D; ++i) {
      if ((vec[i] < min_grid_index(i)) || (vec[i] >= max_grid_index(i)))
        return false;
    }
    return true;
  }
  ...
  template <class V> inline bool is_at_infinity(const V &v) const {
    ...
    for (int i = 0; i < D; i++) {
      if (math::abs(v[i]) == INF_EXTENSION)
        return true;
    }
    return false;
  }

  inline index_type local_index_2_global_index(int dim,
                                               index_type relative_coord,
                                               int cycles,
    ...
    // if symmetric or periodic boundary conditions are used more than one
    // global index is mapped to the same local index for the local index always
    // (min_point_index<=local index<=max_point_index holds)
    ...
    if (cycles == 0) {
      ...
      if (is_boundary_periodic(dim)) {
        ...
        if ((cycles & 1) == 0) { // if cycles is even
          ...
          return cycles * Ext_[dim] + Max_[dim] + Min_[dim] - relative_coord -
                 offset;
        }
      }
    }
  ...
  inline index_type global_index_2_local_index(int dim,
                                               index_type absolute_coord,
                                               index_type offset,
                                               int &cycles) const {
    // this function transforms a global index to the corresponding local index
    // the global index is the index of an infinite grid in all directions
    // if symmetric or periodic boundary conditions are used more than one
    // global index are mapped to the same local index for the local index
    // always (min_point_index<=local index<=max_point_index holds)
  ...
  inline vec<index_type, D> global_indices_2_local_indices(const V &v) const {
    // this function transforms a global index vector to the corresponding local
    // index vector
  ...
  coord_type local_index_2_local_coordinate(int dir, coord_type c) const {
    ...
    // grid into the real coordinates
    ...
    if (lc != uc) {
      return (grid_position_of_local_index(dir, static_cast<index_type>(lc)) *
                  ((uc - c)) + // TODO
              grid_position_of_local_index(dir, static_cast<index_type>(uc)) *
                  ((c - lc)));
    } else {
      return grid_position_of_local_index(dir, static_cast<index_type>(lc));
    }
  ...
  coord_type global_index_2_global_coordinate(int dir, coord_type c) const {
    ...
    if (lc != uc) {
      return (grid_position_of_global_index(dir, static_cast<index_type>(lc)) *
                  ((uc - c) / (uc - lc)) + // TODO
              grid_position_of_global_index(dir, static_cast<index_type>(uc)) *
                  ((c - lc) / (uc - lc)));
    } else {
      return grid_position_of_global_index(dir, static_cast<index_type>(lc));
    }
  ...
  vec<coord_type, D> global_indices_2_global_coordinates(const V &v) const {
    ...
    for (unsigned i = 0; i < D; ++i)
      tmp[i] = global_index_2_global_coordinate(i, v[i]);
    return tmp;
  }
  ...
  vec<index_type, D> global_coordinates_2_global_indices(const V &v) const {
    vec<index_type, D> tmp;
    for (unsigned i = 0; i < D; ++i)
      tmp[i] = global_coordinate_2_global_index(i, v[i]);
    return tmp;
  }

  coord_type local_coordinate_2_local_index(
      int dir, coord_type c, index_type a,
      index_type b) const { // TODO global/local coordinates
  ...
  /*coord_type global_coordinate_2_global_index(int dim, coord_type
  absolute_coord) const { int cycles=0; if (is_boundary_periodic(dim)) { while
  (absolute_coord<min_local_coordinate(dim)) { cycles--;
          ...
          return local_coordinate_2_local_index(dim,
  absolute_coord)+cycles*Ext_[dim]; } else { do { if
  (absolute_coord<min_local_coordinate(dim)) { cycles--;
      ...
      if ((cycles & 1)==0) {
          return local_coordinate_2_local_index(dim,
  absolute_coord)+cycles*Ext_[dim]; } else { return
  cycles*Ext_[dim]+Max_[dim]+Min_[dim]-local_coordinate_2_local_index(dim,
  absolute_coord);
      }
  }*/
  ...
  coord_type
  grid_position_of_global_index(int dir,
                                index_type offset) const { // TODO check
    ...
    if (((cycles & 1) == 0) || is_boundary_periodic(dir)) {
      return grid_position_of_local_index(dir, offset) +
             cycles * (grid_position_of_local_index(dir, max_grid_index(dir)) -
                       grid_position_of_local_index(dir, min_grid_index(dir)));
    } else {
      return (1 + cycles) *
                 grid_position_of_local_index(dir, max_grid_index(dir)) +
             (1 - cycles) *
                 grid_position_of_local_index(dir, min_grid_index(dir)) -
             grid_position_of_local_index(
                 dir, max_grid_index(dir) + min_grid_index(dir) - offset);
    }
  }
  ...
  template <class V> V increment_indices(V v) const {
    // this function transforms a global index vector to the corresponding local
    // index vector

    int dim = 0;
    for (; dim < D - 1; ++dim) {
      if (v[dim] < max_point_index(dim))
        break;
  ...
  template <class V> V decrement_indices(V v) const {
    // this function transforms a global index vector to the corresponding local
    // index vector

    int dim = 0;
    for (; dim < D - 1; ++dim) {
      if (v[dim] > min_point_index(dim))
        break;
  ...
  template <class V> bool is_border_point(V v) const {
    for (unsigned i = 0; i < D; ++i) {
      if (v[i] <= Min_[i] || v[i] >= Max_[i])
        return true;
    }
    return false;
  }

  // determine whether point is outside of domain in direction other than
  // infinite boundary
  template <class V> bool is_outside_domain(V v) const {
    for (unsigned i = 0; i < D; ++i) {
      if (BoundaryConditions_[i] == INFINITE_BOUNDARY)
        continue;
      if (BoundaryConditions_[i] == PERIODIC_BOUNDARY && v[i] == Max_[i])
        return true;
      if (v[i] < Min_[i] || v[i] > Max_[i])
        return true;
    }
...
const typename grid_type<GridTraitsType>::index_type grid_type<
    GridTraitsType>::INF_EXTENSION =
    std::numeric_limits<typename grid_type<GridTraitsType>::index_type>::max() /
    3; // in this way the addition of 2 indices
