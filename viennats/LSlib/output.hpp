...
#define UINT48_MAX                                                             \
  281474976710655ULL // highest value of 3 unsigned bytes  2^48-1
#define INT48_MAX                                                              \
  140737488355327LL //  highest value of 3 signed   bytes  2^47-1
#define INT48_MIN -140737488355328LL //  lowest  value of 3 signed   bytes -2^47

#define UINT56_MAX                                                             \
  72057594037927935ULL // highest value of 3 unsigned bytes  2^56-1
#define INT56_MAX                                                              \
  36028797018963967LL //  highest value of 3 signed   bytes  2^55-1
#define INT56_MIN                                                              \
  -36028797018963968LL //  lowest  value of 3 signed   bytes -2^55
...
template <int D>
class Surface { // the surface(-inserter) class used by the output functions
                // defined below
public:
  ...
  template <class V> node_ref_type insert_node(const V &v, unsigned int loc) {
    ...
    for (unsigned int i = 0; i < Nodes.size(); i++)
      temp.push_back(vec<float, D>(Nodes[i]));
    Nodes.clear();
...
public:
  int number_of_series() const { return 0; }
  ...
  std::string get_series_label(int series) const { return std::string(); }

  std::string get_series_type(int series) const { return std::string(); }

  bool get_series_output(int series) const { return false; }
};

template <class SizeType, class DataType> class GetActivePointType {
public:
  typedef std::vector<SizeType> result;
};

template <class SizeType> class GetActivePointType<SizeType, DefaultDataType> {
public:
  class result {
  public:
    void push_back(const SizeType &v) const {}

    template <class V> SizeType operator[](const V &x) const { return 0; }
  };
};
...
void write_explicit_surface_opendx(
    const levelset<GridTraitsType, LevelSetTraitsType> &l,
    const std::string &filename, const DataType &Data,
    typename LevelSetTraitsType::value_type eps = 0.) {
  // this function etracts the surface using the marching cubes algorithm and
  // writes it to the specified file using OpenDX-file format the parameter eps
  // is forwarded to the surface extraction function

  const int D = GridTraitsType::dimensions;

  Surface<D> s;

  typename GetActivePointType<typename LevelSetTraitsType::size_type,
                              DataType>::result ActivePointList;
  ...
  //! print positions
  f << "object \"positions\" class array type float rank 1 shape " << D
    << " items " << s.Nodes.size() << " data follows" << std::endl;
  for (unsigned int i = 0; i < s.Nodes.size(); i++) {
    for (int j = 0; j < D; j++)
      f << static_cast<float>(s.Nodes[i][j]) << " ";
    f << std::endl;
  }

  //! print connections
  f << "object \"connections\" class array type int rank 1 shape " << D
    << " items " << s.Elements.size() << " data follows" << std::endl;
  ...
  for (int k = 0; k < Data.number_of_series(); ++k) {
    if (Data.get_series_output(k)) {
      f << "object \"" << Data.get_series_label(k)
        << "_data\" class array type " << Data.get_series_type(k)
        << " rank 0 items " << s.Nodes.size() << " data follows" << std::endl;
      for (unsigned int i = 0; i < s.Nodes.size(); i++) {
  ...
  for (int k = 0; k < Data.number_of_series(); ++k) {
    if (Data.get_series_output(k)) {
      f << "object \"" << Data.get_series_label(k) << "\" class field"
        << std::endl;
      f << "  component \"positions\" value \"positions\"" << std::endl;
      f << "  component \"connections\" value \"connections\"" << std::endl;
      f << "  component \"data\" value \"" << Data.get_series_label(k)
        << "_data\"" << std::endl;
    }
  }
...
void write_explicit_volume_vtk(const LevelSetsType &LevelSets,
                               const CounterType counter, ParameterType &p,
                               double eps = 0.) {
...
void write_explicit_volume_vtk(const LevelSetsType &LevelSets,
                               const CounterType counter, ParameterType &p,
                               const DataType &Data, double eps = 0.) {
  ...
  // check if any output should be done. if none, we do not need to extract the
  // volume
  if (!(p.print_volume_tetra || p.print_volume_hull))
    return;

  // if volumeMesh or hullMesh are not initialised, they will not be created in
  // extract_volume
  if (p.print_volume_tetra)
    volumeMesh = vtkSmartPointer<vtkUnstructuredGrid>::New();
  if (p.print_volume_hull)
    hullMesh = vtkSmartPointer<vtkPolyData>::New();

  // depending on the open boundary, volume output needs to be treated
  // differently if the bottom is removed
  if (p.remove_bottom && (p.open_boundary < (D - 1))) {
...
void write_explicit_levelset(const LevelSetType &l,
                             const std::string &filename) {
  ...
  typename LevelSetType::template const_iterator_neighbor_filtered<
      typename LevelSetType::filter_value, 1>
      itA(l, typename LevelSetType::filter_value(2.0));
  ...
  vtkSmartPointer<vtkFloatArray> polyValues =
      vtkSmartPointer<vtkFloatArray>::New();
  vtkSmartPointer<vtkFloatArray> polyNormals =
      vtkSmartPointer<vtkFloatArray>::New();
...
template <class GridTraitsType, class LevelSetTraitsType, class DataType>
void write_explicit_surface_vtp(
    const levelset<GridTraitsType, LevelSetTraitsType> &l,
    const std::string &filename, const DataType &Data,
    typename LevelSetTraitsType::value_type eps = 0.) {
  ...
  vtkSmartPointer<vtkPoints> polyPoints = vtkSmartPointer<vtkPoints>::New();
  for (unsigned int i = 0; i < s.Nodes.size(); i++) {
    polyPoints->InsertNextPoint(s.Nodes[i][0], s.Nodes[i][1],
                                (D == 2) ? 0 : s.Nodes[i][2]);
  }
  ...
  for (int k = 0; k < Data.number_of_series(); ++k) {
    if (Data.get_series_output(k)) {
      ...
      for (unsigned int i = 0; i < s.Nodes.size(); i++) {
        pointData->InsertNextValue(
            Data.get_series_data_double(ActivePointList[i], k));
      }
...
template <class GridTraitsType, class LevelSetTraitsType, class DataType>
void write_explicit_surface_vtk(
    const levelset<GridTraitsType, LevelSetTraitsType> &l,
    const std::string &filename, const DataType &Data,
    typename LevelSetTraitsType::value_type eps = 0.) {
  // this function etracts the surface using the marching cubes algorithm and
  // writes it to the specified file using vtk-file format the parameter eps is
  // forwarded to the surface extraction function
  ...
  f << "CELLS " << s.Elements.size() << " " << ((D + 1) * s.Elements.size())
    << std::endl;
  for (unsigned int i = 0; i < s.Elements.size(); i++) {
    f << D << " ";
    for (int j = 0; j < D; j++)
  ...
  for (int k = 0; k < Data.number_of_series(); ++k) {
    if (Data.get_series_output(k)) {
      f << "SCALARS " << Data.get_series_label(k) << " "
        << Data.get_series_type(k) << " 1" << std::endl;
...
void write_triangulated_surface_vtk(const TriangulationType &t,
                                    const std::string &filename) {
  // this functions writes a given surface triangulation "t" to the vtk-file
  // "filename" the TriangulationType has to have the same properties as for the
  // levelset-initialization function init (see surface2levelset.hpp)
  ...
  for (typename TriangulationType::element_index_type e = 0;
       e < t.number_of_elements(); e++) {
    for (int k = 0; k < D; k++) {
  ...
  for (typename TriangulationType::element_index_type e = 0;
    ...
    for (int k = 0; k < D; k++) {
      if (node_ids.find(t.element_node_id(e, k)) == node_ids.end()) {
        for (int j = 0; j < D; j++)
          f << static_cast<float>(t.node_coordinate(t.element_node_id(e, k), j))
            << " ";
        node_ids[t.element_node_id(e, k)] = counter++;
        if (D == 2)
          f << "0. ";
  ...
  f << "CELLS " << t.number_of_elements() << " "
    << ((D + 1) * t.number_of_elements()) << std::endl;
  for (unsigned int i = 0; i < t.number_of_elements(); i++) {
    f << D << " ";
    for (int j = 0; j < D; j++)
      f << node_ids[t.element_node_id(i, j)] << " ";
    f << std::endl;
  }
...
template <class TriangulationType>
void write_triangulated_surface_opendx(const TriangulationType &t,
                                       const std::string &filename) {
  // this functions writes a given surface triangulation "t" to the OpenDX (Open
  // Data Explorer)-file "filename" the TriangulationType has to have the same
  // properties as for the levelset-initialization function init (see
  // surface2levelset.hpp)
  ...
  //! print positions
  f << "object \"positions\" class array type float rank 1 shape " << D
    << " items " << counter << " data follows" << std::endl;
  counter = 0;
  for (typename TriangulationType::element_index_type e = 0;
       e < t.number_of_elements(); e++) {
    for (int k = 0; k < D; k++) {
  ...
  //! print connections
  f << "object \"connections\" class array type int rank 1 shape " << D
    << " items " << t.number_of_elements() << " data follows" << std::endl;
  for (unsigned int i = 0; i < t.number_of_elements(); i++) {
    for (int j = 0; j < D; j++)
      f << node_ids[t.element_node_id(i, j)] << " ";
    f << std::endl;
...
template <class GridTraitsType, class LevelSetTraitsType>
void write_levelset_opendx(
    const levelset<GridTraitsType, LevelSetTraitsType> &l, std::string FileName,
    bool only_defined_grid_points, float limit, bool only_signs) {
  // this functions writes all defined grid points including their level set
  // values
  ...
  // if only_defined_grid_points is set to false then also the start and end
  // grid points
  //       of undefined runs are written to file, their values are then set to
  //       +/-limit
  // if only_signs is set to true then only the signs of the grid points are
  // written to file,
  ...
  for (typename LevelSetType::const_iterator_runs it(l); !it.is_finished();
       it.next()) {
    if (only_defined_grid_points)
      if (!it.is_defined())
        continue;
  ...
  f << "object 1 class array type float rank 1 shape " << D << " items " << num
  ...
  f << "object 2 class array type float rank 0 items " << num << " data follows"
    << std::endl;
  for (typename LevelSetType::const_iterator_runs it(l); !it.is_finished();
       it.next()) {
    if (only_defined_grid_points)
...
GridTraitsType get_grid_traits_from_lvst_file(std::string path) {
  // reads the grid information from a levelset file

  std::ifstream fin(path);
  if (!fin.is_open())
    msg::print_error("Could not open the file: " + path);
  char buff[9] = {};
  fin.read(buff, 9);
  // Comparing Identification Bytes
  if (std::string(buff).compare(0, 4, "LvSt"))
    msg::print_error("File is not a levelset file.");
  if (LVST_FILE_VERSION_NUMBER != buff[4] - 48)
    msg::print_error("File version does not match!");
  if (bigEndian() != buff[5] - 48)
    msg::print_warning("File was written in a different byte order than it is "
                       "being read. Results may be incorrect!");
  ...
  // since grid limits can be negative we need a mask to fill the remaining
  // bytes with FF if the sign bit is set (Two's complement)
  ...
  for (int i = dim; i--;) {
    ...
    if (grid_min[i] >> (bytes_per_grid_limit * CHAR_BIT - 1))
      grid_min[i] |= sign_bit_mask;
    fin.read((char *)&grid_max[i], bytes_per_grid_limit);
    if (grid_max[i] >> (bytes_per_grid_limit * CHAR_BIT - 1))
      grid_max[i] |= sign_bit_mask;
...
#endif
  if (!fin.good())
    msg::print_error("Could not read grid properties from file: " + path);
  fin.close();
  return GridTraitsType(grid_min, grid_max, b_conditions, grid_delta);
}

template <class GridTraitsType, class LevelSetTraitsType>
void export_levelset_to_file(levelset<GridTraitsType, LevelSetTraitsType> &ls,
                             const std::string &path,
                             const int &bits_per_distance = 8) {
  /*
  **************************************************************************************************************
  ...
  **************************************************************************************************************
  *    File Header: 8 Bytes     * *
  ******************************** * 4 Bytes   Identification Bytes (LvSt) * 1
  *Byte    File Version Number * 1 Byte    Endianess - Little Endian (0) or Big
  *Endian (1)                                               * 1 Byte Dimension
  *of the Levelset (2 or 3) * 1 Byte    Bits Per Distance *
  **************************************************************************************************************
  *    Grid: 15+ Bytes    * *
  ************************* * 1 Byte    This byte contains the number of bytes
  *used for the grid min and grid max of each dimension.  * NOTE: The following
  *block is repeated for each dimension. * x Bytes   Grid minimum * x Bytes Grid
  *maximum * 1 Byte    Boundary Condition * NOTE: END * 8 Bytes   GridDelta
  *(sizeof double) *
  **************************************************************************************************************
  *    H-RLE Block Header: 14 Bytes    * *
  ************************************** * 1  Byte   This byte contains the
  *number of bytes used for each start index and runtype                  * 1
  *Byte   This byte contains the number of bytes used for each runbreak. * 4
  *Bytes  Number of saved Start Indices * 4  Bytes  Number of saved Runtypes *
  *    4  Bytes  Number of saved Runbreaks *
  ************************** * H-RLE Block Data    * *
  ************************** * Start Indices               - using adaptive
  *number of bytes(delta encoded)                             * Runtypes - using
  *2 bits per runtype (-oo, +oo, defined)                              * Indices
  *of defined runtypes - using adaptive number of bytes(delta encoded) *
  *    Runbreaks                   - using adaptive number of bytes *
  **************************************************************************************************************
  *    Distances Header: 4 Bytes    * *
  *********************************** * 4 Bytes  Number of distances *
  *********************************** * Distances Data               * *
  *********************************** * Distances - using 8 bits per distance
  *(default)                                                         * NOTE:
  *Currently up to 64 bits are supported. When bits_per_distance is 0, 8 bits
  *are used.              *
  **************************************************************************************************************
  */
  ...
  const boundary_type *grid_b_conditions =
      ls.grid().grid_traits().boundary_conditions();
  ...
  for (unsigned int i = 0; i < D; i++) {
    if (minimum > grid_minima[i])
      minimum = grid_minima[i];
    if (maximum < grid_maxima[i])
      maximum = grid_maxima[i];
  }
  // Use bytesize such that all gridMin and gridMax fit into the number of bytes
  if (minimum >= INT8_MIN && maximum <= INT8_MAX)
    bytes_grid_limits = 1;
  else if (minimum >= INT16_MIN && maximum <= INT16_MAX)
    bytes_grid_limits = 2;
  else if (minimum >= INT24_MIN && maximum <= INT24_MAX)
    bytes_grid_limits = 3;
  else if (minimum >= INT32_MIN && maximum <= INT32_MAX)
    bytes_grid_limits = 4;
  else if (minimum >= INT40_MIN && maximum <= INT40_MAX)
    bytes_grid_limits = 5;
  else if (minimum >= INT48_MIN && maximum <= INT48_MAX)
    bytes_grid_limits = 6;
  else if (minimum >= INT56_MIN && maximum <= INT56_MAX)
    bytes_grid_limits = 7;
  else
    bytes_grid_limits = 8;
  ...
  oss << "File Version: " << LVST_FILE_VERSION_NUMBER << std::endl
      << (bigEndian() ? "Big Endian" : "Little Endian") << std::endl
  ...
  double delta = ls.grid().grid_traits().grid_position(
      0, 1); // 1st argument is not used. Returns 1 * GridDelta.
  fout.write((char *)&delta,
             sizeof(double)); // double, aka binary64, has 64 bits(8 bytes) on
                              // all platforms as of the IEEE-754 standard(1985;
                              // newest revision 2008)
#ifdef VERBOSE
  ...
  for (int dim = D - 1; dim >= 0; --dim) {
    ...
    if (tmp <= UINT8_MAX)
      ...
      bytes_per_index = 2;
    else if (tmp <= UINT24_MAX)
      bytes_per_index = 3;
    else if (tmp <= UINT32_MAX)
      bytes_per_index = 4;
    else if (tmp <= UINT40_MAX)
      bytes_per_index = 5;
    else if (tmp <= UINT48_MAX)
      bytes_per_index = 6;
    else if (tmp <= UINT56_MAX)
      bytes_per_index = 7;
    else
      bytes_per_index = 8;

    // bytesize for runbreaks
    // The smallest runbreak is >= grid_min and the largest runbreak is <=
    // grid_max.
    uint8_t bytes_per_runbreak = 1;
    if (grid_minima[dim] >= INT8_MIN && grid_maxima[dim] <= INT8_MAX)
      bytes_per_runbreak = 1;
    else if (grid_minima[dim] >= INT16_MIN && grid_maxima[dim] <= INT16_MAX)
      bytes_per_runbreak = 2;
    else if (grid_minima[dim] >= INT24_MIN && grid_maxima[dim] <= INT24_MAX)
      bytes_per_runbreak = 3;
    else if (grid_minima[dim] >= INT32_MIN && grid_maxima[dim] <= INT32_MAX)
      bytes_per_runbreak = 4;
    else if (grid_minima[dim] >= INT40_MIN && grid_maxima[dim] <= INT40_MAX)
      bytes_per_runbreak = 5;
    else if (grid_minima[dim] >= INT48_MIN && grid_maxima[dim] <= INT48_MAX)
      bytes_per_runbreak = 6;
    else if (grid_minima[dim] >= INT56_MIN && grid_maxima[dim] <= INT56_MAX)
      bytes_per_runbreak = 7;
    else
      bytes_per_runbreak = 8;
    ...
    oss << "Dimension " << dim << ":" << std::endl
        << "    " << (int)bytes_per_index
        << " byte(s) per start index or runtype." << std::endl
        << "    " << (int)bytes_per_runbreak << " byte(s) per runbreak."
        << std::endl
        << "    " << (values_written + 1) << " of " << start_indices.size()
        << " start indices written.";
    msg::print_message_2(oss.str());
#endif

    // write all runtypes to the file, skipping all segments and indices (using
    // 2 bits per runtype)
    count = CHAR_BIT / 2 - 1;
    byte = 0;
    values_written = 0;
    std::vector<size_type> def_run_indices =
        {}; // store all indices of defined runtypes
    for (typename std::vector<size_type>::const_iterator it = runtypes.begin();
         it != runtypes.end(); ++it) {
      if (*it == ls.POS_PT) // 01 - positive undefined runtype
        ...
        byte |= 2 << count * 2;
      else if (!ls.is_defined(*it)) { // skip Segments --> there shouldn't be
                                      // any because of serialize()
    ...
    if (count >= 0 &&
        count <
            CHAR_BIT / 2 -
                1) // if the last byte contains less than 4 runtyes, write it
      fout << byte;
...
#ifdef VERBOSE
    oss.str("");
    oss << "    " << values_written << " of " << runtypes.size()
        << " runtypes written. Defined runtypes: " << def_run_indices.size();
    msg::print_message_2(oss.str());
#endif
    // Write runbreaks
    values_written = 0;
...
#ifdef VERBOSE
    oss.str("");
    oss << "    " << values_written << " of " << runbreaks.size()
        << " runbreaks written.";
    msg::print_message_2(oss.str());
#endif
  }
  ...
  const int num_overflow_bytes =
      bits_per_distance / CHAR_BIT -
      1; //-1 because bpd is mapped to the next higher power of 2; 16/8 = 2 but
         // overflow bytes is only 1
  const int bits_per_byte =
      num_overflow_bytes > 0 ? CHAR_BIT : bits_per_distance;
  const long double value =
      std::pow(2.0L, (long double)bits_per_distance - 1) - 0.5L;
  ...
  /*
  *************************************************************************************
  *  NOTE: In the current implementation x = 1 * Levelset values range from -x
  *.... +x, where x = levelset.num_layers/2           * With n bits we can
  *represent values from 0 .... (2^n -1)                         * -x .... +x |*
  *(2^n-1)/(2x)                                     *
  * -(2^n-1)/2 .... +(2^n-1)/2    |+ (2^n-1)/2 * 0 .... +(2^n-1) *
  *************************************************************************************
  */
  unsigned long long discrete_distance, overflow;
  long double tmp;
  byte = 0;
  for (typename std::vector<value_type>::const_iterator it = distances.begin();
       it != distances.end(); ++it) {
    tmp = *it * value + value + 0.5; //+0.5 for rounding, std::llround returns a
                                     // signed long long and not an unsigned one
    ...
    if (count < 0) {
      ...
      if (count < 0)
        count = 0;
      // write overflow, even if it is 0, so each distance has the same amount
      // of bytes
  ...
  if (!fout.good())
    msg::print_error("Writing to file " + path + " failed.");
  fout.close();

  ls.finalize(2); // sets the segmentation points
  // ls.prune() is called before writing the levelset, therefore we only need to
  // set up the segmentation
  ls.segment(); // parallelize the levelset
}

template <class GridTraitsType, class LevelSetTraitsType>
void import_levelset_from_file(levelset<GridTraitsType, LevelSetTraitsType> &ls,
                               const std::string &path) {
  ...
  typedef typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
      size_type;
  typedef typename levelset<GridTraitsType, LevelSetTraitsType>::index_type
      index_type;
  typedef typename levelset<GridTraitsType, LevelSetTraitsType>::value_type
      value_type;
  ...
  if (std::string(buff).compare(0, 4, "LvSt"))
    msg::print_error("File is not a levelset file.");
  if (LVST_FILE_VERSION_NUMBER != buff[4] - 48)
    msg::print_warning("File version does not match!");
  if (bigEndian() != buff[5] - 48)
    msg::print_warning("File was written in a different byte order than it is "
                       "being read. Results may be incorrect!");
  ...
  /*
  **************************** SKIP GRID PROPERTIES(they have to be read before
  *the levelset) ****************************** Current position:  fin.tellg() *
  *  For each dimension add: 2 * bytes_per_grid_limits + 1 byte (per boundary
  *condition)                                   * Add: the additional number of
  *bytes for the grid delta; sizeof(double) *
  **************************************************************************************************************************
  */
  fin.seekg(int(fin.tellg()) + (bytes_per_grid_limits * 2 + 1) * dim +
            sizeof(double));
#ifdef VERBOSE
  oss << std::endl
  ...
  for (int i = dim; i--;) {
    ...
    start_indices.push_back(
        current); // push the 0, it was not written to the file
    for (unsigned int i = 0; i < num_start_indices - 1;
         i++) { //-1 cause we didnt write the 0
      current = 0;
    ...
    msg::print_message_2(oss.str());
    oss.str("");
    oss << "    " << (int)bytes_per_index
        << " byte(s) per start index and runtype." << std::endl
        << "    " << (int)bytes_per_runbreak << " byte(s) per runbreak."
        << std::endl
        << "    " << (values_read + 1) << " of " << num_start_indices
        << " start indices read."; // first index (0) is not written to file
    msg::print_message_2(oss.str());
#endif
    uint32_t count = 0;
    values_read = 0;
    ...
    std::ifstream tmp_fin(
        path); // temporary stream to read defined runtypes from
    if (!tmp_fin.is_open())
      msg::print_error("Could not open the tmp file: " + path);
    tmp_fin.seekg(
        fin.tellg() +
        (long)
    ...
    for (unsigned int y = 0; y < bytes_to_read; y++) {
      ...
      for (unsigned int z = runtypes_per_byte; z--;) {
        if (values_read == num_runtypes)
          break;
        uInt = byte >> z * 2 & 0x3;
        if (uInt == 1)
          runtypes.push_back(ls.POS_PT);
        else if (uInt == 3)
          runtypes.push_back(ls.NEG_PT);
        else if (uInt == 2)
          runtypes.push_back(ls.UNDEF_PT);
        else if (uInt == 0) {
          current = 0;
          tmp_fin.read((char *)&current,
                       bytes_per_index); // read index of defined runtype
          sum += current;
    ...
    fin.seekg(
        tmp_fin
            .tellg()); // set position of stream to after the defined runtypes
    tmp_fin.close();
#ifdef VERBOSE
    oss.str("");
    oss << "    " << values_read << " of " << num_runtypes
        << " runtypes read. Defined runtypes: " << count;
    msg::print_message_2(oss.str());
#endif
    ...
    /*
    *********************************************************************************************************************************
    * Example: We have 2 bytes and the runbreak is -7, which corresponds to FFF9
    *(Two's complement).                                * Now if we read it into
    *a 4 byte integer we will have 0000 FFF9, which is not interpreted as -7,
    *but as a positive number.     * -7 as a 4 byte integer is: FFFF FFF9. * We
    *need a mask to set the higher bytes to FF if the sign bit is set *
    *********************************************************************************************************************************
    */
    unsigned long long sign_bit_mask = 0;
    ...
    for (unsigned int z = 0; z < num_runbreaks; z++) {
      ...
      if (sInt >> (bytes_per_runbreak * CHAR_BIT - 1))
        sInt |= sign_bit_mask;
...
#ifdef VERBOSE
    oss.str("");
    oss << "    " << values_read << " of " << num_runbreaks << " runbreaks."
        << std::endl;
    msg::print_message_2(oss.str());
#endif
  }
  ...
  const long double value =
      1.0L / (std::pow(2.0L, (long double)bits_per_distance - 1) -
              0.5L); // value ^ -1 to avoid division
  const int count = std::ceil((double)CHAR_BIT / bits_per_distance);
  const int num_overflow_bytes =
      bits_per_distance / CHAR_BIT -
      1; //-1 because bpd is mapped to the next higher power of 2
  const int bits_per_byte =
      num_overflow_bytes > 0 ? CHAR_BIT : bits_per_distance;
  const int num_reads = std::ceil((double)num_distances / count);
  const unsigned char mask =
      0xFF >> (bits_per_distance < CHAR_BIT
                   ? CHAR_BIT - bits_per_distance
                   : 0); // for bits per distance > 8 the mask will be 0xFF
  ...
  for (int i = 0; i < num_reads; i++) {
    fin.read((char *)&byte, 1);
    for (int z = count; z--;) {
      if (values_read == num_distances)
        break; // if distances are odd, skip padding bits
      discrete_distance = 0;
...
#endif
  if (!fin.good())
    msg::print_error("Reading from " + path + " failed.");
  fin.close();

  ls.finalize(2);
  // ls.prune() is called before writing the levelset file, therefore we only
  // need to set up segmentation.
  ls.segment();
  ls.set_levelset_id();
}
