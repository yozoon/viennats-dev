...
// in this file the basic functions and data structures of the level set library
// are defined as data structure hierarchical run-length-encoding (HRLE) as
// described in
//       B.~Houston, M.B.~Nielsen, C.~Batty, O.~Nilsson, K.~Museth,
//       "Hierarchical RLE-Level Set - A Compact and Versatile Deformable
//       Surface Representation", ACM Trans. Graph. 25/1, pp. 151-175, 2006.
// the sparse field level set method, which is used for time integration is
// described in
//       R.T.~Whitaker, A Level-Set Approach to 3D Reconstruction from Range
//       Data J. Comp. Vision 29/3, pp. 203-231, 1998. in this file functions
//       are defined which rebuild the level set function after the active grid
//       points are time integrated the functions for the time integration
//       itself are defined in "timeintegration.hpp"
...
template <class GridTraitsType,
          class LevelSetTraitsType = DefaultLevelSetTraitsType>
class levelset {
  // the class GridTraitsType has to have the properties as described in
  // "grid.hpp"

  // the LevelSetTraitsType defines the used data types within the level
  // set data structure, and has to have the certain type and static constant
  // definitions as shown in the "DefaultLevelSetTraitsType" above
...
private:
  ...
  public:
    ...
    template <class X> allocation_type &operator*=(const X &x) {
      for (int i = 0; i < D; ++i)
        num_values[i] = static_cast<size_type>(num_values[i] * x);
      for (int i = 0; i < D; ++i)
        num_runs[i] = static_cast<size_type>(num_runs[i] * x);
      return *this;
    }
    ...
    template <class X> allocation_type &operator/=(const X &x) {
      for (int i = 0; i < D; ++i)
        num_values[i] = static_cast<size_type>(num_values[i] / x) + 1;
      for (int i = 0; i < D; ++i)
        num_runs[i] = static_cast<size_type>(num_runs[i] / x) + 1;
      return *this;
    }
  ...
  public:
    // the following vectors are used to store the run-lenght-encoded data
    // structure for more details on that data structure see B. Houston, M.B.
    // Nielsen, C. Batty, O. Nilsson, K. Museth, "Hierarchical RLE-Level Set - A
    // Compact and Versatile Deformable Surface Representation", ACM Trans.
    ...
    std::vector<value_type>
        distances; // this vector keeps the level set values of all defined grid
                   // points its size is therefore equal to the number of
                   // defined grid points

    std::vector<size_type>
        active; // an additional vector, which has the same size as the
                // distances-vector in case the defined grid point is active in
                // terms of the sparse field level set method (the level set
                // value is in the range [-0.5,0.5]) this vector keeps the
                // active
                // point id the active point ids are given to the active grid
                // points in lexicographical order starting with index 0
                // therefore the largest active point ID equals the number of
                // active grid points-1 if the grid point is not active, its
                // value is set to the constant INACTIVE

    size_type num_active_points; // num_active_points stores the number of
                                 // active points

    const grid_type2 &Grid; // Grid stores the information about the grid, on
  ...
  public:
    allocation_type get_allocation() const {
      ...
      // num_values[0] is to contain level set values, num_values[i] contains
      // the start indices at the i-th dimension num_runs[i] is to contain the
      // run types at the i-th dimension
      allocation_type a;
    ...
    sub_levelset_type(const grid_type2 &g, const allocation_type &a)
        : num_active_points(0), Grid(g) {
      // Constructor for the sub_levelset_type class
      // Takes a pointer to a grid_type and an allocation_type to reserve the
      // memory required for the start indeces, run types and run breaks arays,
      // as well as the LS values
    ...
    const sub_levelset_type &operator=(const sub_levelset_type &s) {
      for (int i = 0; i < D; ++i)
        start_indices[i] = s.start_indices[i];
      for (int i = 0; i < D; ++i)
    ...
    index_type GetRunStartCoord(int dim, size_type start_indices_pos,
                                size_type run_type_pos) const {
      // returns the start index of the run given by start_indices_pos and
      // run_type_pos see the definition of the HRLE-data structure for more
      // details
    ...
    index_type GetRunEndCoord(int dim, size_type start_indices_pos,
                              size_type run_type_pos) const {
      // returns the end index of the run given by start_indices_pos and
      // run_type_pos NOTE: the end index is not included by the run see the
    ...
    unsigned long int allocated_memory() const {
      ...
      // set function.
      // NOTE: since this function is based on  the sizeof-operator it cannot
      // take the memory needed for the
      //       GridTraitsType (see grid.hpp) into account if dynamic data
      //       structures are used for example Furthermore due to data structure
      //       alignment the result can be somehow inaccurate
    ...
    void push_back_undefined(V start_point, const V &end_point, size_type rt) {
      ...
      if (start_point[D - 1] <= Grid.max_point_index(D - 1))
        push_back_undefined(start_point, rt);
    }
    ...
    template <class V> void push_back_undefined(const V &point, size_type rt) {
      ...
      for (level = 0; level < D; ++level) {
        if (point[level] != Grid.min_point_index(level))
          break;
      }
      ...
      for (dim = D - 1; dim > level; --dim) {
        ...
        if (runtypes[dim].size() ==
            start_indices[dim].back()) { // if there is no run
        ...
        } else if (!levelset::is_defined(
                       runtypes[dim].back())) { // if there is an defined run
          old_sign = runtypes[dim].back();
          if (old_sign == rt)
            return;
          if (runtypes[dim].size() ==
              start_indices[dim].back() + 1) { // if there is a single run
            if (point[dim] == Grid.min_point_index(dim)) {
    ...
    template <class V> void push_back(const V &point, value_type distance) {
      ...
      } else if (!is_defined(runtypes[0].back())) { // if there is an defined
                                                    // run
        old_sign = runtypes[0].back();
        if (runtypes[0].size() ==
            start_indices[0].back() + 1) { // if there is a single run
          if (point[0] == Grid.min_point_index(0)) {
    ...
    void print(std::ostream &out = std::cout) const {
      ...
      for (int dim = D - 1; dim >= 0; --dim) {
        ...
        for (typename std::vector<size_type>::const_iterator it =
                 start_indices[dim].begin();
             it != start_indices[dim].end(); ++it) {
          if (c % 10 == 0)
            out << std::endl;
          ++c;
          out << std::setw(8) << *it;
        ...
        c = 0;
        for (typename std::vector<index_type>::const_iterator it =
                 runbreaks[dim].begin();
  ...
  class sub_levelsets_type {
    // contains the vector of sub_levelset_type items that are used for
    // parallelization

    typedef sub_levelset_type
        *sub_levelset_ptr_type; // pointer to memory containing
                                // sub_levelset_type object

    typedef typename std::vector<sub_levelset_ptr_type>
        sub_levelsets_intern_type;
  ...
  public:
    ...
    sub_levelset_type &operator[](size_type i) { return *(subs[i]); }

    const sub_levelset_type &back() const { return *(subs.back()); }

    sub_levelset_type &back() { return *(subs.back()); }

    void swap(sub_levelsets_type &s) { subs.swap(s.subs); }

    void initialize(size_type i, const grid_type2 &g,
                    allocation_type a = allocation_type()) {
      // initialize the parallelization by implementing i number of
      // sub_levelset_type objects through a sub_levelset_ptr_type object, that
      // each correspond to a parallel core

      for (typename sub_levelsets_intern_type::iterator it = subs.begin();
           it != subs.end(); ++it)
        if (*it != 0)
          delete *it;
...
#pragma omp parallel for schedule(                                             \
    static, 1) // parallelization - Iterations divided into chunks of size 1.
...
#pragma omp parallel for schedule(                                             \
    ...
    sub_levelsets_type(const sub_levelsets_type &s) {
      // builds a new sub_levelsets_type that copies the one passed to the
      // function as s
      subs.resize(s.size());
...
#pragma omp parallel for schedule(                                             \
  ...
  public:
      ...
      void initialize(size_type i, const grid_type2& g, const allocation_type&
  a=allocation_type()) {
...
public:
  ...
  static const size_type
      INACTIVE; // this constant is used for grid points which are not active

  static const value_type POS_VALUE; // this constant is used for positive
                                     // undefined grid points as level set value
  static const value_type NEG_VALUE; // this constant is used for negative
                                     // undefined grid points as level set value

  static const size_type
      POS_PT; // this constant is used as runtype for positive runs
  static const size_type
      NEG_PT; // this constant is used as runtype for negative runs
  static const size_type
      UNDEF_PT; // this constant is used as runtype for uninitialized runs
  static const size_type
      SEGMENT_PT; // this constant is used as runtype for uninitialized runs
  ...
  // are used for certain iterators, to specify at which grid points an iterator
  // should stop

  class filter_all_defined { // stop at all defined grid points
  public:
    template <class C> bool operator()(const C &c) const {
      return c.is_defined();
    }
  };

  class filter_all { // stop at all possible stop points (beginning of each run)
  public: // in case of undefined runs of the data structure, which can consist
          // of more than 1 grid point the iterator will also stop at the
          // beginning of such runs
  ...
  class filter_value { // stop at (defined) grid points which have an absolute
                       // level set value
    value_type limit;  // less than a given limit
  public:
    filter_value(value_type x) : limit(x) {}
    template <class C> bool operator()(const C &c) const {
      return (math::abs(c.value()) <= limit);
    }
  };
...
private:
  ...
  // structure for more details on that data structure see B. Houston, M.B.
  // Nielsen, C. Batty, O. Nilsson, K. Museth, "Hierarchical RLE-Level Set - A
  // Compact and Versatile Deformable Surface Representation", ACM Trans. Graph.
  // 25/1, pp. 151-175, 2006. std::vector<size_type> start_indices[D];
  // std::vector<size_type> runtypes[D];
  // std::vector<index_type> runbreaks[D];
  // std::vector<value_type> distances;          //this vector keeps the level
  // set values of all defined grid points its size is therefore equal to the
  // number of defined grid points

  // std::vector<size_type> active;  //an additional vector, which has the same
  // size as the distances-vector in case the defined grid point is active in
  // terms of the sparse field level set method (the level set value is in the
  // range [-0.5,0.5]) this vector keeps the active point id the active point
  // ids
  // are given to the active grid points in lexicographical order starting with
  // index 0 therefore the largest active point ID eqauls the number of active
  // grid points-1 if the grid point is not active, its value is set to the
  // constant INACTIVE
  ...
  int num_layers; // num_layers keeps the number of layers of which the level
                  // set function currently consists of at least the minimum
                  // valid value is 1, that means that at least all grid points
                  // are explicitly stored (defined grid points) which have a
                  // value in the range [-0.5,0.5] if num_layers=2 then the
                  // level set data structures stores at least all grid points,
                  // which are in the range [-1,1] if num_layers=n at least all
                  // grid points are explicitly stored which are in the range
                  // [-n/2,n/2]

  const grid_type<GridTraitsType>
      &Grid; // Grid stores the information about the grid, on which the level
  ...
  /*template <class V> V GetNextCoordinate(V coords, int& level) const {
      //this functions returns the next indices of the grid in lexicographical
  order
      //first coords[0] is increased, if it is out of range of the grid, it is
  set to the minimum index
  ...
  /*index_type GetRunStartCoord(int dim, size_type start_indices_pos, size_type
  run_type_pos) const {
      //returns the start index of the run given by start_indices_pos and
  run_type_pos
  ...
  index_type GetRunEndCoord(int dim, size_type start_indices_pos, size_type
  run_type_pos) const {
      //returns the end index of the run given by start_indices_pos and
  run_type_pos
  ...
  /*size_type FindInferiorRunTypePos(int dim, size_type start_indices_pos,
  index_type relative_coord) const {
      //this functions finds the right run by looking up the given index
  "relative_coord" in the
      ...
      return
  start+(std::upper_bound(runbreaks[dim].begin()+start_break,runbreaks[dim].begin()+end_break,relative_coord)-(runbreaks[dim].begin()+start_break));
  }*/

public:
  void print_segment_sizes() {
    for (typename sub_levelsets_type::size_type i = 0;
         i != sub_levelsets.size(); ++i) {
  ...
  std::vector<size_type> &start_indices(unsigned int dim,
  ...
  std::vector<index_type> &runbreaks(unsigned int dim,
                                     unsigned int segment = 0) {
  ...
  levelset &export_levelset(const std::string &path,
                            int bits_per_distance = 8) {
  ...
  void swap(levelset<GridTraitsType, LevelSetTraitsType> &l) {
    // this function swaps the level set function with another level set
    // function "l"
    ...
    /*for (typename sub_levelsets_type::iterator
    it=sub_levelsets.begin();it!=sub_levelsets.end();++it) { it->l=this;
    }

    for (typename sub_levelsets_type::iterator
    it=l.sub_levelsets.begin();it!=l.sub_levelsets.end();++it) { it->l=&l;
    }*/
  ...
  size_type number_of_runs(int level, int sub) const {
    // this function returns the number of runs for the given sub_levelset and
    // given level
  ...
  index_type
  get_runbreak(int dim, int runbreak = std::numeric_limits<int>::max()) const {
  ...
  index_type get_max_runbreak(int dim) const {
    ...
    for (unsigned int i = 1; i < number_of_segments(); ++i)
      maxBreak = std::max(maxBreak, sub_levelsets[i].getMaxRunBreak(dim));
    return maxBreak;
  }
  ...
  index_type get_min_runbreak(int dim) const {
    ...
    for (unsigned int i = 1; i < number_of_segments(); ++i)
      minBreak = std::min(minBreak, sub_levelsets[i].getMinRunBreak(dim));
    return minBreak;
  }
  ...
  const unsigned get_levelset_id() const { return levelset_id; }
  ...
  template <class CoordType> size_type pt_id2(const CoordType &coords) const {
    // this function returns the ID of a defined grid point given by its local
    // indices "coords" or in case of undefined grid points it returns POS_PT or
    // NEG_PT here, local indices means that the indices must be contained by
    // the grid see also the function pt_id below

    // size_type sub=std::upper_bound(segmentation.begin(), segmentation.end(),
    // coords)-segmentation.begin();

    // size_type distances_pos(0);     //start_indices_pos

    int sub =
        std::upper_bound(segmentation.begin(), segmentation.end(), coords) -
        segmentation.begin();
    ...
    for (int level = D - 1; level >= 0; --level) {
      ...
      typename std::vector<index_type>::const_iterator start_breaks =
          sub_levelsets[sub].runbreaks[level].begin() + (run_type_pos - id);
      typename std::vector<index_type>::const_iterator end_breaks =
          sub_levelsets[sub].runbreaks[level].begin() +
          (sub_levelsets[sub].GetStartIndex(level, id + 1) - (id + 1));
      typename std::vector<index_type>::const_iterator pos_breaks =
          std::upper_bound(start_breaks, end_breaks, coords[level]);
  ...
  template <class CoordType> size_type pt_id(const CoordType &coords) const {
    // this function returns the ID of a grid point given by its global indices
    // "coords" or in case of undefined grid points it returns POS_PT or NEG_PT
    // here, global indices means that also indices can be passed to this
    // function, which are not included by the grid, or in other words the
    // boundary conditions are taken into account to get the right grid point
  ...
  const value_type &
  value(size_type pt_id) const { // this function returns the level set value
    if (pt_id == POS_PT)
      return POS_VALUE; // of a grid point given by its pt_id
    if (pt_id == NEG_PT)
      return NEG_VALUE;
    return value2(pt_id);
  }
  ...
  const value_type &
  value(int sub,
        size_type pt_id) const { // this function returns the level set value
    if (pt_id == POS_PT)
      return POS_VALUE; // of a grid point given by its pt_id
    if (pt_id == NEG_PT)
      return NEG_VALUE;
  ...
  const value_type &value2(size_type pt_id) const {
    typename std::vector<size_type>::const_iterator it =
        std::upper_bound(pt_id_offsets.begin(), pt_id_offsets.end(), pt_id) - 1;
  ...
  void set_value(
      size_type pt_id,
      value_type v) { // this function sets the level set value of a grid point
    typename std::vector<size_type>::const_iterator it =
        std::upper_bound(pt_id_offsets.begin(), pt_id_offsets.end(), pt_id) - 1;
  ...
  const value_type &
  value(const CoordType &c) const { // this function returns the level set value
  ...
  static sign_type sign(value_type value) { // this function returns the sign
    return (sign_type(math::signbit(value))
                ? NEG_SIGN
                : POS_SIGN); // of a given level set value "value"
  }

  sign_type sign(size_type pt_id)
      const { // this function returns the sign of a grid point given by
    if (pt_id == POS_PT)
      return POS_SIGN; // its point-ID, and also checks if the point-ID is a
                       // positive or
    if (pt_id == NEG_PT)
      return NEG_SIGN; // negative run
    // shfdhsfhdskjhgf assert(is_defined(pt_id));
    return sign_type(sign(value2(pt_id)));
  }

  sign_type sign2(size_type pt_id)
      const { // this function returns the sign of a grid point given by
  ...
  static bool
  is_defined(size_type r) { // returns if the grid point given by the "pt_id"
    return (r < UNDEF_PT);  // is a defined grid point
  }

  size_type active_pt_id(size_type pt_id)
      const { // returns the active_pt_id for a grid point given by the
    if (is_defined(
            pt_id)) { //"pt_id". If it is an undefined (pt_id=POS_PT or NEG_PT)
  ...
  bool
  is_active(size_type pt_id) { // returns if the grid point given by the "pt_id"
    return (active_pt_id(pt_id) != INACTIVE); // is an active grid point
  }

  size_type active_pt_id2(size_type pt_id)
      const { // this function assumes that pt_id is a valid point-ID of
    ...
    if (a != INACTIVE)
      a += active_pt_id_offsets[sub];

    return a;
  }

  /*bool is_active(local_pt_id_type pt_id) const {             //returns if the
  grid point given by its ID "pt_id" return (active_pt_id(pt_id)!=INACTIVE);
  //is an active grid point, using the function active_pt_id
  }

  bool is_active2(local_pt_id_type pt_id) const {            //returns if the
  grid point given by its ID "pt_id" return (active_pt_id2(pt_id)!=INACTIVE);
  //is an active grid point, using the function active_pt_id
  }*/

  //#########################################################

  // void clear(const points_type & =points_type());    //this function deletes
  // all defined grid points and assigns to the whole grid the sign given by
  // "sign" (an undefined run of the specified sign is created containing all
  // grid points of the grid) by default, all grid points are initialized to
  // positive

  void initialize(const points_type & = points_type(),
                  const allocation_type & = allocation_type());

  void finalize(int);

  allocation_type get_allocation() const {
    // allocation_type allocates the requried sizes to num_values and num_runs
    // for all the sub_levelsets members num_values[0] is to contain level set
  ...
  levelset(const grid_type2 &, const PointsType &point_defs);
  // this is a constructor, creating a new level set from a sorted list of
  // index/level set value-pairs

  levelset(const levelset &l)
      : segmentation(l.segmentation), sub_levelsets(l.sub_levelsets),
        num_layers(l.num_layers), Grid(l.Grid),
        active_pt_id_offsets(l.active_pt_id_offsets),
        pt_id_offsets(l.pt_id_offsets) {

    // for (typename sub_levelsets_type::iterator it = sub_levelsets.begin();
    // it!=sub_levelsets.end(); ++it) {
    //     it->l=this;
    // }
    // sub_levelsets.update_levelset_ref(*this);
  }
  ...
  // initializes a plain levelset with its normal pointing into the given axis
  // direction
  ...
  /*const levelset & operator=(const levelset& l) {
       ...
       Grid=l.Grid;
       for (typename sub_levelsets_type::iterator it = sub_levelsets.begin();
  it!=sub_levelsets.end(); ++it) { it->l=this;
       }
  ...
  void deep_copy(const levelset &l) {
    ...
    // since Grid is a const reference to the underlying grid, it has to be the
    // same
    assert(&Grid == &l.Grid);
  }

  template <class V>
  void push_back(int sub, const V &point, value_type distance) {
  ...
  template <class V>
  void push_back_undefined(int sub, const V &point, size_type rt) {
  ...
  template <class V> int find_sub_levelset(const V &point) {
    return std::upper_bound(segmentation.begin(), segmentation.end(), point) -
           segmentation.begin();
  }
  ...
  vec<index_type, D> find_coordinate_of_pt(size_type pt) const {
    ...
    const int sub =
        (std::upper_bound(pt_id_offsets.begin() + 1, pt_id_offsets.end(), pt) -
         (pt_id_offsets.begin() + 1));
    ...
    for (int g = 0; g < D; ++g) {
      ...
      while (!is_defined(s.runtypes[g][min]))
        ++min;
      while (!is_defined(s.runtypes[g][max]))
        --max;

      while (min != max) {
        size_type mid = (max + min + 1) / 2;
        while (!is_defined(s.runtypes[g][mid]))
          ++mid;
      ...
      pt = std::upper_bound(s.start_indices[g].begin() + 1,
                            s.start_indices[g].end(), min) -
           (s.start_indices[g].begin() + 1);
  ...
  points_type get_new_segmentation() const {
    ...
    for (unsigned int g = 0; g < static_cast<unsigned int>(n - 1); ++g) {
      ...
      if (sum != n_pts)
        tmp_segmentation.push_back(find_coordinate_of_pt(sum));
    }

    if (n_pts == 0)
      assert(tmp_segmentation.size() == 0);
  ...
  template <class V> void push_back(const V &point, value_type distance) {
    // this function adds a new defined grid point to the level set function
    // the indices of the new grid point "indices" must be greater
    // (lexiographically greater) than the the indices of the current "last"
    // grid point
  ...
  void rebuild(); // this function is used to rebuild the level set function
                  // after time integration, after rebuilding the level set
                  // function will consist of two layers of defined grid points
                  // (number_of_layers()==2)

  template <
      class DataType> // this function does the same as the Rebuild()-function
  void
  rebuild(const DataType
              &Data); // however, it allows the user to transfer user-data
                      // asigned to active grid points, to the new active grid
                      // points the size of the array Data must be equal to the
                      // number of active grid points after the call of this
                      // functions the size of Data is again equal to the number
                      // of active grid points after rebuilding DataSize defines
                      // how many consecutive entries of the array Data are
                      // assigned to each active grid point
  ...
  /*void set_number_of_layers(int x) {                  //this function sets the
  number of layers
  ...
  void
  expand(int end_width,
         int start_width); // this function expands the level set function
                           //"start_width" is the number of layers of the level
                           // set function which should be used, then the level
                           // set function is expanded where "end_width" defines
                           // the final number of layers HINT:
                           // start_width<=number_of_layers() must be fulfilled

  void expand(int end_width) { // this function expands the level set function
    expand(end_width,
           number_of_layers()); // to the given number of layers "end_width"
  }

private:
  class max_wrapper {
  public:
    template <class G> G operator()(G a, G b) const { return std::max(a, b); }
  };

  class min_wrapper {
  public:
    template <class G> G operator()(G a, G b) const { return std::min(a, b); }
  };
...
public:
  void min(const levelset &l) { minmax(l, min_wrapper()); }

  void max(const levelset &l) { minmax(l, max_wrapper()); }

  void reduce(int width); // this function reduces the level set function
                          // to the given number of layers (>=1)

  void segment(); // segments the levelset in the available sub_levelsets to
                  // balance correctly

  void
  prune(); // removes all grid points, which do not have at least one opposite
           // signed neighbour and distributes points across sub_levelsets to
           // approximately balance the number of points across threads

  void add_voxel_corners(); // this function expands the level set functions to
                            // 5 layers, in addition it adds defined grid
                            // points, so that each voxel (grid cell) which has
                            // at least one active grid point as corner, has all
                            // its corner grid points defined

  void add_voxel_corners2();

  template <class PointsType>
  void insert_points(
      const PointsType &point_defs); // this functions clears the level set
                                     // function and setups the function from
                                     // new from a sorted list "point_defs" of
                                     // index/level set value-pairs

  // iterator declarations
  // iterators are used to process the data sequentially and to obtain linear
  // scaling algorithms

  class const_iterator_base;

  class const_iterator_runs; // this iterator is the main iterator, on which all
                             // other iterators base on this iterator iterates
                             // over all runs and stops at every start index of
                             // each run this run can be defined or undefined,
                             // if it is a defined run it corresponds to a
                             // single defined grid point

  class const_iterator_runs_offset; // this iterator is derived from the
                                    // const_iterator_runs class and allows to
                                    // iterate over the level set function with
                                    // a given offset-index with this iterator
                                    // it is possible to create
                                    // neighbor-iterators, which consist of a
                                    // const_iterator_runs and a
                                    // const_iterator_runs_offset-iterator for
                                    // each neighbor grid point a movement of
                                    // the neighbor-iterator corresponds to a
                                    // synchronous movement of all individual
                                    // iterators. In this way linear scaling
                                    // algorithms can be obtained

  template <class F, int NumN>
  class const_iterator_neighbor_filtered; // this neighbor-iterator allows to
                                          // access up to "NumN"-neighbors in
                                          // all positive and negative axis
                                          // directions the filter "F" defines
                                          // where the iterator for the center
                                          // should stop

  template <class F>
  class const_iterator_cells_filtered; // this iterator consist of 2^Dimensions
                                       // const_iterator_runs_offset-iterators
                                       // one for each corner of a grid cell
                                       // all these 2^Dimensions iterators are
                                       // synchronously moved whenever the
                                       // filter is fulfilled for one of these
                                       // iterators, the
                                       // const_iterator_cells_filtered iterator
                                       // stops

  class const_iterator_neighbor; // this neighbor iterator consists of
                                 // 2*Dimenions const_iterator_runs_offset for
                                 // the neighbors and a
                                 // const_iterator_runs-iterator for the center
                                 // whenever one of these (2*Dimensions+1)
                                 // iterators reach a defined grid point the
                                 // const_iterator_neighbor iterator stops this
                                 // iterator is internally used for rebuilding
                                 // and expanding the level set function

  class neighbor_stencil;
  class star_stencil;

private:
  template <class I> void next(I &) const;

  template <class I> void previous(I &) const;

  template <class V, class I> void go_to_indices(const V &, I &) const;

  template <class V, class I> void go_to_indices(int, const V &, I &) const;
...
const typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
    levelset<GridTraitsType, LevelSetTraitsType>::SEGMENT_PT =
        std::numeric_limits<size_type>::max() -
        (lvlset_omp_max_num_threads - 1);
template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
    levelset<GridTraitsType, LevelSetTraitsType>::POS_PT =
        std::numeric_limits<size_type>::max() -
        (lvlset_omp_max_num_threads + 0);
template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
    levelset<GridTraitsType, LevelSetTraitsType>::NEG_PT =
        std::numeric_limits<size_type>::max() -
        (lvlset_omp_max_num_threads + 1);
template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
    levelset<GridTraitsType, LevelSetTraitsType>::UNDEF_PT =
        std::numeric_limits<size_type>::max() -
        (lvlset_omp_max_num_threads + 2);

template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::value_type
    levelset<GridTraitsType, LevelSetTraitsType>::POS_VALUE =
        std::numeric_limits<value_type>::max();
template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::value_type
    levelset<GridTraitsType, LevelSetTraitsType>::NEG_VALUE =
        -std::numeric_limits<value_type>::max();

template <class GridTraitsType, class LevelSetTraitsType>
const typename levelset<GridTraitsType, LevelSetTraitsType>::size_type
    levelset<GridTraitsType, LevelSetTraitsType>::INACTIVE =
        std::numeric_limits<size_type>::max();

template <class GridTraitsType, class LevelSetTraitsType>
unsigned levelset<GridTraitsType, LevelSetTraitsType>::top_levelset_id = 0;

template <class GridTraitsType, class LevelSetTraitsType>
template <class PointsType>
void levelset<GridTraitsType, LevelSetTraitsType>::insert_points(
  ...
  if (point_defs.front().first != Grid.min_point_index()) {
    push_back_undefined(0, Grid.min_point_index(),
                        (sign_type(sign(point_defs.front().second)) == POS_SIGN)
                            ? POS_PT
                            : NEG_PT); // TODO
  }

  typename PointsType::const_iterator begin_pt_defs =
      point_defs.begin(); //+starts[t_num];
  typename PointsType::const_iterator end_pt_defs =
      point_defs.end(); // point_defs.begin()+starts[t_num+1];
  ...
  while (it_pt_defs != end_pt_defs) {
    ...
    if (math::abs(it_pt_defs->second) >=
        std::numeric_limits<
            typename PointsType::value_type::second_type>::max()) {
      push_back_undefined(
          0, it_pt_defs->first,
          (sign_type(sign(it_pt_defs->second)) == POS_SIGN) ? POS_PT : NEG_PT);
    } else {
    ...
    for (int q = 0; q < D; q++) {
      ...
      tmp[q]++;
      if (tmp[q] > Grid.max_point_index(q))
        continue;
      for (int r = 0; r < q; ++r)
        tmp[r] = Grid.min_point_index(r);
...
template <class GridTraitsType, class LevelSetTraitsType>
template <class PointsType>
levelset<GridTraitsType, LevelSetTraitsType>::levelset(
    const grid_type2 &g, const PointsType &point_defs)
    : Grid(g) {
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::initialize(
    const points_type &p, const allocation_type &a) {
  ...
  for (typename sub_levelsets_type::size_type i = 1; i < sub_levelsets.size();
       ++i) {

    sub_levelset_type &s = sub_levelsets[i];

    s.push_back_undefined(Grid.min_point_index(),
                          Grid.decrement_indices(segmentation[0]), SEGMENT_PT);

    for (typename sub_levelsets_type::size_type j = 1; j < i; ++j) {
      s.push_back_undefined(segmentation[j - 1],
                            Grid.decrement_indices(segmentation[j]),
                            SEGMENT_PT + j);
    }
  }
}
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::finalize(int num_lay) {

  // finalize sub-levelsets
  for (typename sub_levelsets_type::size_type i = 0;
       i < sub_levelsets.size() - 1; ++i) {

    sub_levelset_type &s = sub_levelsets[i];

    for (typename sub_levelsets_type::size_type j = i + 1;
         j < segmentation.size(); ++j) {
      s.push_back_undefined(segmentation[j - 1],
                            Grid.decrement_indices(segmentation[j]),
                            SEGMENT_PT + j);
    }
    s.push_back_undefined(segmentation.back(), Grid.max_point_index(),
                          SEGMENT_PT + segmentation.size());
  }
  ...
  for (typename sub_levelsets_type::size_type i = 0;
    ...
    active_pt_id_offsets.push_back(active_pt_id_offsets.back() +
                                   sub_levelsets[i].num_active_pts());
  }
...
template <class GridTraitsType, class LevelSetTraitsType>
levelset<GridTraitsType, LevelSetTraitsType>::levelset(const grid_type2 &g)
    : Grid(g) {
...
template <class GridTraitsType, class LevelSetTraitsType>
levelset<GridTraitsType, LevelSetTraitsType>::levelset(
    const grid_type2 &g, value_type c, int direction,
    bool is_direction_negative)
    : segmentation(points_type()), Grid(g) {
  ...
  if (min_[direction] == max_[direction])
    --(min_[direction]);
  ...
  while (true) {
    ...
    if (it[direction] == min_[direction]) {
      ...
      sub_levelsets[0].push_back_undefined(t, (is_direction_negative) ? POS_PT
                                                                      : NEG_PT);
      ...
      sub_levelsets[0].push_back_undefined(
          it, (is_direction_negative) ? NEG_PT : POS_PT);
    }
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::segment() {
...
#pragma omp parallel num_threads(                                              \
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
  {
    int p = 0;
#ifdef _OPENMP
    ...
    const_iterator_runs it(old_lvlset, (p == 0) ? grid().min_point_index()
                                                : segmentation[p - 1]);
    vec<index_type, D> end_v = (p != static_cast<int>(segmentation.size()))
                                   ? segmentation[p]
                                   : grid().max_point_index();

    for (; it.start_indices() < end_v; it.next()) {
      if (it.is_defined()) {
        s.push_back(it.start_indices(), it.value());
      } else {
        s.push_back_undefined(it.start_indices(),
                              (it.sign() == POS_SIGN) ? POS_PT : NEG_PT);
      }
    }
  }
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::prune() {
  // removes all grid points, which do not have at least one opposite signed
  // neighbour and distributes points across sub_levelsets to approximately
  // balance the number of points
  levelset<GridTraitsType, LevelSetTraitsType> old_lvlset(
      grid()); // create new Level-Set
  swap(old_lvlset);
...
#pragma omp parallel num_threads(                                              \
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
  {
    ...
    const_iterator_neighbor_filtered<filter_all, 1> it(
        old_lvlset, filter_all(),
        (p == 0) ? grid().min_point_index() : segmentation[p - 1]);
    vec<index_type, D> end_v =
        (p != static_cast<int>(segmentation.size()))
            ? segmentation[p]
            : grid().increment_indices(grid().max_point_index());
    ...
    for (; it.indices() < end_v; it.next()) {
      ...
      if (it.center().is_defined()) {
        ...
        for (; i < 2 * D; i++) {
          if (it.neighbor(i).sign() != sgn)
            break;
        }
        if (i != 2 * D) {
          s.push_back(it.indices(), it.center().value());
        } else {
          s.push_back_undefined(it.indices(),
                                (sgn == POS_SIGN) ? POS_PT : NEG_PT);
        }
      } else {
        s.push_back_undefined(
            it.indices(), (it.center().sign() == POS_SIGN) ? POS_PT : NEG_PT);
      }
    }
  }
...
void levelset<GridTraitsType, LevelSetTraitsType>::rebuild(
    const DataType &data) {
  ...
  initialize(old_lvlset.get_new_segmentation(),
             old_lvlset.get_allocation() * (2. / old_lvlset.num_layers));
...
#pragma omp parallel num_threads(                                              \
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
  {
    ...
    {
      const size_type tmp = old_lvlset.num_active_pts() *
                            (allocation_factor / sub_levelsets.size()); // TODO
    ...
    const_iterator_neighbor it(old_lvlset, (p == 0) ? grid().min_point_index()
                                                    : segmentation[p - 1]);
    vec<index_type, D> end_v =
        (p != static_cast<int>(segmentation.size()))
            ? segmentation[p]
    ...
    for (; it.start_indices() != end_v; it.next()) {
      ...
      if (it.center().is_active()) { // if the center is an active grid point
        ...
        int k = 0;
        for (; k < 2 * D; k++)
          if (it.neighbor(k).sign() != it.center().sign())
            break;
        ...
        if (k != 2 * D) { // if there is at least one neighbor of opposite sign
          ...
          if (it.center().value2() > 0.5) {
            int j = 0;
            for (; j < 2 * D; j++) {
              if (it.neighbor(j).is_active())
                if (it.neighbor(j).value2() < -0.5)
                  break;
            }

            if (j == 2 * D) {
              s.push_back(it.start_indices(), it.center().value2());
            } else { // if there is at least one active grid point, which is <
                     // -0.5
          ...
          s.push_back_undefined(it.start_indices(),
                                (it.center().sign() == POS_SIGN) ? POS_PT
                                                                 : NEG_PT);
        }
        ...
        if (it.center().sign() == POS_SIGN) {
          ...
          for (int i = 0; i < 2 * D; i++) {
            if (it.neighbor(i).is_active() &&
                (it.neighbor(i).sign() == NEG_SIGN)) {
...
#pragma omp                                                                    \
    barrier // wait until all other threads in section reach the same point.
#pragma omp                                                                    \
    single // section of code that must be run by a single available thread.
    {
      finalize(2);
      data.set_size(num_active_pts());
    }

    for (size_type i = 0; i < s.num_active_pts(); ++i)
      data.copy_data(i + active_pt_id_offsets[p], data_transfer[i]);
  }
}
...
template <class GridTraitsType, class LevelSetTraitsType>
template <class BinaryType>
void levelset<GridTraitsType, LevelSetTraitsType>::minmax(
    const levelset<GridTraitsType, LevelSetTraitsType> &lB,
    const BinaryType &min_or_max) {
  ...
  //       this function calculates the minimum or maximum of two level set
  //       functions with an optimal complexity O(lA.num_pts()+lB.num_pts())
  ...
  initialize(lA.get_new_segmentation(),
             lA.get_allocation() + lB.get_allocation()); // TODO
    ...
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
  {
    ...
    // for (int p=0;p<static_cast<int>(sub_levelsets.size());++p) {
    {
      ...
      while (pos < end_v) {
        ...
        if (math::abs(d) <
            std::numeric_limits<typename LevelSetType::value_type>::max()) {
          s.push_back(pos, d);
        } else {
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::reduce(int width) {
  ...
  initialize(old_lvlset.get_new_segmentation(),
             old_lvlset.get_allocation() *
                 std::min(1., (static_cast<double>(width) /
                               old_lvlset.number_of_layers())));
  // initialize();
...
#pragma omp parallel num_threads(                                              \
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
  {
    ...
    // for (int p=0;p<static_cast<int>(sub_levelsets.size());++p) {
    {
      ...
      for (const_iterator_runs it(old_lvlset, begin_v);
           it.start_indices() < end_v; it.next()) {
  ...
  segment();
  /*std::string err= misc::test(*this);     //check level set function //TODO if
  (err.size()) {                   //if inconsistent print out error message
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::serialize() {
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::expand(int end_width,
                                                          int start_width) {
  ...
  for (int a = 0; a < num_required_cycles; ++a) {
    ...
    initialize(old_lvlset.get_new_segmentation(),
               old_lvlset.get_allocation() *
                   (static_cast<double>(num_layers) / old_lvlset.num_layers));
    // tmp+=my::time::GetTime();
    // std::cout << "time1 = " << tmp;
...
#pragma omp parallel num_threads(                                              \
    sub_levelsets.size()) // use num_threads(sub_levelsets.size()) threads
    {
      ...
      // for (int p=0;p<static_cast<int>(sub_levelsets.size());++p) {
      {
        ...
        for (const_iterator_neighbor it(old_lvlset, begin_v);
             it.start_indices() != end_v; it.next()) {
          ...
          if (math::abs(it.center().value()) <= total_limit) {
            ...
            if (it.center().sign() == POS_SIGN) {
              value_type distance = POS_VALUE;
              for (int i = 0; i < 2 * D; i++) {
                // if (it.neighbor(i).sign()>0) //shfdhsfhdskjhgf
                // assert(it.neighbor(i).value()!=0);
                distance =
                    std::min(distance, it.neighbor(i).value() + value_type(1));
              }
              if (distance <= limit) {
                s.push_back(it.start_indices(), distance);
              } else {
              ...
              for (int i = 0; i < 2 * D; i++) {
                ...
                // assert(it.neighbor(i).value()!=0);
                distance =
                    std::max(distance, it.neighbor(i).value() - value_type(1));
              }
              if (distance >= -limit) {
                s.push_back(it.start_indices(), distance);
              } else {
    ...
    // tmp=-my::time::GetTime();
    finalize(
        std::max(start_width, end_width + 2 * (1 + a - num_required_cycles)));
    // tmp+=my::time::GetTime();
...
/*
//this function expands the level set functions to 5 layers, in addition it adds
defined grid points, so that
//each voxel (grid cell) which has at least one active grid point as corner, has
all its corner grid points defined
*/

template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::add_voxel_corners() {
  ...
  // shfdhsfhdskjhgf
  // assert(old_lvlset.sub_levelsets.size()==old_lvlset.active_pt_id_offsets.size());
  ...
  while (!it_mid.is_finished()) {
    ...
    if (it_mid.is_defined()) {
      ...
      if (k != (1 << D)) {
        ...
        value_type distance;
        for (int m = 0; m < 2 * D; ++m)
          ix[m].go_to_indices_sequential(coords);
        if (it_mid.sign() == POS_SIGN) {
          distance = POS_VALUE;
          for (int m = 0; m < 2 * D; ++m)
            distance = std::min(distance, ix[m].value() + value_type(1));
        } else {
          distance = NEG_VALUE;
          for (int m = 0; m < 2 * D; ++m)
            distance = std::max(distance, ix[m].value() - value_type(1));
        }
        s.push_back(coords, distance);
      } else {
        s.push_back_undefined(coords,
                              (it_mid.sign() == POS_SIGN) ? POS_PT : NEG_PT);
      }
    }
...
template <class GridTraitsType, class LevelSetTraitsType>
void levelset<GridTraitsType, LevelSetTraitsType>::add_voxel_corners2() {
...
#pragma omp for schedule(static,                                               \
                         1) // parallelization - Iterations divided into chunks
                            // of size 1. Each chunk is assigned to a thread
    for (int p = 0; p < static_cast<int>(sub_levelsets.size()); ++p) {
          ...
          if (it.center().sign() == POS_SIGN) {
            ...
            for (int i = 0; i < 2 * D; i++) {
              if (it.neighbor(i).is_defined()) {
                if (it.neighbor(i).value2() <= 2.5)
                  x.set(i % D);
                // if (it.neighbor(i).sign()>0) //shfdhsfhdskjhgf
                // assert(it.neighbor(i).value()!=0);
                distance =
                    std::min(distance, it.neighbor(i).value2() + value_type(1));
              }
            }
            ...
            for (int i = 0; i < 2 * D; i++) {
              if (it.neighbor(i).is_defined()) {
                ...
                // assert(it.neighbor(i).value()!=0);
                distance =
                    std::max(distance, it.neighbor(i).value2() - value_type(1));
              }
            }
            if ((distance >= -3.5) && (x.count() == D)) {
              s.push_back(it.start_indices(), distance);
...
template <class GridTraitsType, class LevelSetTraitsType>
template <class F, int NumN>
class levelset<GridTraitsType,
               LevelSetTraitsType>::const_iterator_neighbor_filtered {
  // this neighbor-iterator allows to access up to "NumN"-neighbors in all
  // positive and negative axis directions the filter "F" defines where the
  // iterator for the center should stop

  const_iterator_runs it_mid; // the iterator for the center
  std::vector<const_iterator_runs_offset> it_neighbors; // the neighbor
                                                        // iterators
  const F filter; // the filter for the center grid point
  const levelset<GridTraitsType, LevelSetTraitsType>
      &l; // reference to the level set function

public:
  typedef levelset<GridTraitsType, LevelSetTraitsType> levelset_type;

  const levelset_type &get_levelset() const { return l; }

  const_iterator_neighbor_filtered(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx, const F &f = F())
      : it_mid(lx), filter(f), l(lx) {
    // constructor, initializes the iterator and sets to the first position
    // fulfilling the "filter"-criterion
    ...
    for (int i = 0; i < 2 * D; i++) {
      ...
      for (int j = 0; j < NumN; j++) {
        if (i < D)
          ...
          tv[i - D]--;
        it_neighbors.push_back(
            const_iterator_runs_offset(lx, tv, it_mid.start_indices()));
      }
    }
  }
  ...
  template <class V>
  const_iterator_neighbor_filtered(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx, const F &f,
      const V &v)
      : it_mid(lx, v), filter(f), l(lx) {
    // constructor, initializes the iterator and sets to the first position
    // fulfilling the "filter"-criterion
  ...
  void next() { // move to the next run fulfilling "filter"
    ...
    if (!it_mid.is_finished()) {
      for (int i = 0; i < 2 * D * NumN; i++)
        it_neighbors[i].go_to_indices_sequential(it_mid.start_indices());
    }
  }

  const const_iterator_runs_offset &neighbor(int direction,
                                             int item = 0) const {
  ...
  index_type indices(int dir)
      const { // returns the index of the center for the given axis direction
    return it_mid.start_indices(dir);
  }

  value_type gradient(int dir) const {
    // returns the derivation in respect to the real coordinates for the given
    // axis direction this function requires that the level set function
    // consists at least of 3 layers of grid points, which means that all
    // neighbor grid points of active grid points are defined

    const value_type pos =
        l.Grid.grid_position_of_local_index(dir, indices(dir));
    const value_type d_p =
        l.Grid.grid_position_of_global_index(dir, indices(dir) + 1) - pos;
    const value_type d_n =
        l.Grid.grid_position_of_global_index(dir, indices(dir) - 1) - pos;
    ...
    return ((d_p / d_n) * (phi_p - phi_0) - (d_n / d_p) * (phi_n - phi_0)) /
           (d_n - d_p);
  }
  ...
  value_type gradient2(
      int dir) const { // returns the derivation of the level set function in
                       // respect to the index for the given axis direction
    const value_type phi_p = neighbor(dir, 0).value();
    const value_type phi_n = neighbor(dir + D, 0).value();
...
template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_base {
protected:
...
public:
  ...
  const_iterator_base(const levelset &lx)
      : l(lx), abs_coords(l.grid().min_point_index()),
        end_abs_coords(l.grid().max_point_index()), r_level(D), s_level(D),
        sub(0) {
  ...
  const vec<index_type, D> &start_indices() const { return abs_coords; }
  ...
  const vec<index_type, D> &end_indices() const { return end_abs_coords; }
  ...
  int get_level() const { return s_level; }

  /*int get_sub() const {
      return sub;
  }*/

  size_type current_run_code() const { return start_indices_pos[r_level]; }
  ...
  size_type get_segment_num() const { return sub; }
  ...
  size_type active_pt_id2() const { // TODO
    ...
    // shfdhsfhdskjhgf assert(sub>=0);
    // shfdhsfhdskjhgf
    // assert(l.sub_levelsets.size()==l.active_pt_id_offsets.size());
    // shfdhsfhdskjhgf assert(current_run_code()>=0);
    // shfdhsfhdskjhgf
    // assert(current_run_code()<l.sub_levelsets[sub].distances.size());

    // shfdhsfhdskjhgf
    // assert(l.sub_levelsets[sub].active.size()>current_run_code());
  ...
  sign_type sign_p() const { return l.sign(value_p()); }
};

template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_runs
    : public levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_base {
  ...
  // run this run can be defined or undefined, if it is a defined run it
  // corresponds to a single defined grid point

  using levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_base::l;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::r_level;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::s_level;
  using levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_base::sub;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::abs_coords;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::end_abs_coords;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::start_run_abs_coords;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::end_run_abs_coords;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::run_type_pos;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::start_indices_pos;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::go_up_AB;
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::go_up_BA;

public:
  using levelset<GridTraitsType,
                 LevelSetTraitsType>::const_iterator_base::is_finished;
...
private:
  ...
  template <class V, class I>
  friend void
  levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices(const V &,
                                                              I &) const;

  template <class V, class I>
  friend void
  levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices(int, const V &,
                                                              I &) const;

  template <class V, class I>
  friend void
  levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices_sequential(
      const V &, I &) const;
...
protected:
  ...
  bool go_down_BA(index_type c) {
    ...
    start_indices_pos[s_level - 1] =
        sl.runtypes[r_level][run_type_pos[r_level]];
  ...
  bool go_next_A() {
    if (s_level == r_level) {
      // if
      // (start_indices_pos[s_level]<l.runtypes[r_level][run_type_pos[r_level]]+(end_run_coords[r_level]-start_run_coords[r_level]))
      // {
  ...
  bool go_next_B() {
    // shfdhsfhdskjhgf assert(s_level==r_level+1);
    // shfdhsfhdskjhgf
    // assert(end_run_abs_coords[r_level]<=l.grid().max_point_index(r_level));
    ...
    if (end_run_abs_coords[r_level] != l.grid().max_point_index(r_level)) {
      ...
      end_run_abs_coords[r_level] = sl.GetRunEndCoord(
          r_level, start_indices_pos[s_level], run_type_pos[r_level]);
      return true;
    } else {
      return false;
    }
  ...
  bool go_previous_B() {
    // shfdhsfhdskjhgf assert(s_level==r_level+1);
    // shfdhsfhdskjhgf
    // assert(start_run_abs_coords[r_level]>=l.grid().min_point_index(r_level));
    ...
    if (start_run_abs_coords[r_level] != l.grid().min_point_index(r_level)) {
      ...
      start_run_abs_coords[r_level] = sl.GetRunStartCoord(
          r_level, start_indices_pos[s_level], run_type_pos[r_level]);
      return true;
    } else {
      return false;
...
public:
  ...
  const_iterator_runs(const levelset &lx, bool reverse = false)
      : const_iterator_base(lx) {
    if (reverse) {
  ...
  template <class V>
  const_iterator_runs(const levelset &lx, const V &v)
      : const_iterator_base(lx) {
    go_to_indices(v);
  }
  ...
  // end_run_abs_coords[r_level] in go_next_B, it would make more sense to check
  // is_finished first, do nothing if it is and return false
  void next() { l.next(*this); }

  void previous() { l.previous(*this); }

  template <class V> void go_to_indices(const V &v) {
    l.go_to_indices(v, *this);
  }

  template <class V> void go_to_indices_sequential(const V &v) {
...
void levelset<GridTraitsType, LevelSetTraitsType>::next(I &it) const {
  while (true) {
    ...
    if (it.r_level == D) {
      it.abs_coords =
          it.l.grid().increment_indices(it.l.grid().max_point_index());
  ...
  if (s != it.sub) {
    ...
    /*if (s>0) {
        ...
        //shfdhsfhdskjhgf
    assert(it.l.grid().min_point_index()==it.start_indices());
    }*/
  }
}
...
void levelset<GridTraitsType, LevelSetTraitsType>::previous(I &it) const {
  while (true) {
    ...
    if (it.r_level == D) {
      it.abs_coords =
          it.l.grid().decrement_indices(it.l.grid().min_point_index());
      it.end_abs_coords = it.abs_coords; // TODO
      return;
    }
  }
  ...
  if (s != it.sub) {
    ...
    /*if (s<it.l.segmentation.size()) {
        //shfdhsfhdskjhgf
    assert(it.l.grid().decrement_indices(it.l.segmentation[s])==it.end_indices());
    } else {
        //shfdhsfhdskjhgf
    assert(it.l.grid().max_point_index()==it.end_indices());
    }*/
  }
}
...
template <class GridTraitsType, class LevelSetTraitsType>
template <class V, class I>
void levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices(int sub,
                                                                 const V &v,
                                                                 I &it) const {
  it.r_level = D;
  it.s_level = D;
...
template <class GridTraitsType, class LevelSetTraitsType>
template <class V, class I>
void levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices(const V &v,
                                                                 I &it) const {
  go_to_indices(0, v, it); // TODO
  int s = it.current_segment();
  if (s != 0)
    go_to_indices(s, v, it);
}

template <class GridTraitsType, class LevelSetTraitsType>
template <class V, class I>
void levelset<GridTraitsType, LevelSetTraitsType>::go_to_indices_sequential(
    const V &v, I &it) const {
...
template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_runs_offset
    : public levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_base {
  // this iterator is the main iterator, on which all other iterators base on
  // this iterator iterates over all runs and stops at every start index of each
  // run this run can be defined or undefined, if it is a defined run it
...
protected:
  void go_down_AB(index_type abs_c) { // X         //find right run_type_pos
    ...
    int cycles = 0;
    index_type rel_c = l.grid().global_index_2_local_index(
    ...
    // shfdhsfhdskjhgf
    // assert(start_run_rel_coords[r_level]>=l.grid().min_point_index(r_level));
    // shfdhsfhdskjhgf assert(start_run_rel_coords[r_level]<=rel_c);
    // shfdhsfhdskjhgf assert(rel_c<=end_run_rel_coords[r_level]);
    // shfdhsfhdskjhgf
    // assert(l.grid().max_point_index(r_level)>=end_run_rel_coords[r_level]);
    ...
    if (l.grid().is_boundary_periodic(r_level)) {
      ...
      start_run_abs_coords[r_level] =
          std::max(l.grid().local_index_2_global_index(r_level, rel_s, cycles,
                                                       offset[r_level]),
                   l.grid().min_point_index(r_level));
      end_run_abs_coords[r_level] =
          std::min(l.grid().local_index_2_global_index(r_level, rel_e, cycles,
                                                       offset[r_level]),
    ...
    // shfdhsfhdskjhgf
    // assert(l.grid().max_point_index(r_level)>=end_run_abs_coords[r_level]);

    // shfdhsfhdskjhgf assert(s_level>=1);
    // shfdhsfhdskjhgf assert(s_level==r_level+1);

    if (end_run_abs_coords[r_level] - start_run_abs_coords[r_level] >
        end_run_rel_coords[r_level] - start_run_rel_coords[r_level]) {
      // shfdhsfhdskjhgf assert(!(l.grid().is_boundary_periodic(r_level)));
      // shfdhsfhdskjhgf
      // assert(end_run_rel_coords[r_level]==l.grid().max_point_index(r_level)
      // || start_run_rel_coords[r_level]==l.grid().min_point_index(r_level));
    }

    // shfdhsfhdskjhgf
    // assert(l.grid().global_index_2_local_index(r_level,start_run_abs_coords[r_level],
    // offset[r_level])>=start_run_rel_coords[r_level]); shfdhsfhdskjhgf
    // assert(l.grid().global_index_2_local_index(r_level,start_run_abs_coords[r_level],
    // offset[r_level])<=end_run_rel_coords[r_level]);

    // shfdhsfhdskjhgf
    // assert(l.grid().global_index_2_local_index(r_level,end_run_abs_coords[r_level],
    // offset[r_level])>=start_run_rel_coords[r_level]); shfdhsfhdskjhgf
    // assert(l.grid().global_index_2_local_index(r_level,end_run_abs_coords[r_level],
    // offset[r_level])<=end_run_rel_coords[r_level]);
  }
  ...
  bool go_down_BA(index_type abs_c) { // X
    ...
    if (l.is_defined(start_indices_pos[s_level - 1])) {
      ...
      index_type rel_c =
          l.grid().global_index_2_local_index(r_level, abs_c, offset[r_level]);
  ...
  bool go_next_A() { // X
    ...
    if (s_level == r_level) {
      ...
      // shfdhsfhdskjhgf
      // assert(abs_coords[s_level]<=end_run_abs_coords[r_level]);
      // shfdhsfhdskjhgf
      // assert(abs_coords[s_level]>=start_run_abs_coords[r_level]);
      ...
      if (abs_coords[s_level] < end_run_abs_coords[r_level]) {
        ...
        if (l.grid().is_boundary_periodic(r_level)) {
          // shfdhsfhdskjhgf
          // assert(rel_coords[s_level]!=l.grid().max_point_index(s_level));
          ++rel_coords[s_level];
          ++start_indices_pos[s_level];
        } else {
  ...
  bool go_previous_A() {
    ...
    if (s_level == r_level) {
      ...
      if (abs_coords[s_level] > start_run_abs_coords[r_level]) {
        ...
        if (l.grid().is_boundary_periodic(r_level)) {
          // shfdhsfhdskjhgf
          // assert(rel_coords[s_level]!=l.grid().min_point_index(s_level));
          --rel_coords[s_level];
          --start_indices_pos[s_level];
        } else {
          if (rel_coords[s_level] == l.grid().max_point_index(s_level)) {
  ...
  bool go_next_B() {
    ...
    // shfdhsfhdskjhgf assert(run_type_pos[r_level]>=sl.GetStartIndex(r_level,
    // start_indices_pos[s_level])); shfdhsfhdskjhgf
    // assert(run_type_pos[r_level]<sl.GetStartIndex(r_level,
    // start_indices_pos[s_level]+1));

    if (end_run_abs_coords[r_level] != l.grid().max_point_index(r_level)) {

      if (l.grid().is_boundary_periodic(
              r_level)) { // periodic boundary conditions

        if (end_run_rel_coords[r_level] == l.grid().max_point_index(r_level)) {
          run_type_pos[r_level] =
              sl.GetStartIndex(r_level, start_indices_pos[s_level]);
          start_run_rel_coords[r_level] = l.grid().min_point_index(r_level);
        } else {
          ++run_type_pos[r_level];
      ...
      // assert(start_run_abs_coords[r_level]>=l.grid().min_point_index(r_level));
      // shfdhsfhdskjhgf
      // assert(start_run_abs_coords[r_level]<=end_run_abs_coords[r_level]);
      // shfdhsfhdskjhgf
      // assert(l.grid().max_point_index(r_level)>=end_run_abs_coords[r_level]);
    ...
    if (start_run_abs_coords[r_level] != l.grid().min_point_index(r_level)) {
      ...
      if (l.grid().is_boundary_periodic(
        ...
        if (start_run_rel_coords[r_level] ==
            l.grid().min_point_index(r_level)) {
          run_type_pos[r_level] =
              sl.GetStartIndex(r_level, start_indices_pos[s_level] + 1) - 1;
          end_run_rel_coords[r_level] = l.grid().max_point_index(r_level);
        } else {
          --run_type_pos[r_level];
          end_run_rel_coords[r_level] = start_run_rel_coords[r_level] - 1;
...
public:
  ...
  template <class V>
  const_iterator_runs_offset(const levelset &lx, const V &o,
                             bool reverse = false)
      : const_iterator_base(lx), offset(o) {
    if (reverse) {
      go_to_indices(l.grid().max_point_index());
    } else {
      go_to_indices(l.grid().min_point_index());
  ...
  const_iterator_runs_offset(const levelset &lx, const V1 &o, const V2 &v)
      : const_iterator_base(lx), offset(o) {
    go_to_indices(v);
  }
  ...
  const vec<index_type, D> get_offset() const { return offset; }
};
...
/*A star stencil is used for calculation of spatial derivatives.
  ...
  First order difference or WENO schemes can be chosen when a star stencil is
  constructed. Provides convenient methods such as gradient or normal vector.
*/
template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::star_stencil {

  const levelset<GridTraitsType, LevelSetTraitsType> &l;
  typename levelset<GridTraitsType,
                    LevelSetTraitsType>::const_iterator_runs_offset it_center;
...
public:
  star_stencil(const levelset<GridTraitsType, LevelSetTraitsType> &lx,
               const typename levelset<GridTraitsType, LevelSetTraitsType>::
                   const_iterator_runs_offset &it_mid,
    ...
    for (int i = 0; i < 2 * D; ++i) {
      ...
      for (int j = 0; j < order; ++j) {
        if (i < D)
          ...
          tv[i - D]--;
        it_neighbors.push_back(
            const_iterator_runs_offset(l, tv, it_center.start_indices()));
      }
    }
  }
  ...
  const const_iterator_runs_offset &
  center() const { // access to the center grid point
    return it_center;
  }

  const vec<index_type, D> &get_center_offset() const { return offset; }
  ...
  const vec<value_type, D> getDx() const {
    ...
    for (int i = 0; i < D; ++i) {
      dx_all[i] = l.Grid.grid_position_of_global_index(i, indices(i) + 1) -
                  l.Grid.grid_position_of_local_index(i, indices(i));
    }
    return dx_all;
  }
  ...
  value_type inline phi_x_p(int dir) const {
    ...
    if (dscheme == differentiation_scheme::FIRST_ORDER) {
      ...
      if (dscheme == differentiation_scheme::WENO3) {
        ...
        return my::math::weno3<value_type>(0, phi_n, phi_0, phi_p, phi_pp, dx,
                                           true);
        ...
        return my::math::weno5<value_type>(0, phi_nn, phi_n, phi_0, phi_p,
                                           phi_pp, phi_ppp, dx, true);
  ...
  // (d phi / d x)^- .. left-sided derivative along axis x = {x,y,z}, given by
  // axis index dir NOTE we assume uniform grid here
  value_type inline phi_x_n(int dir) const {
    ...
    if (dscheme == differentiation_scheme::FIRST_ORDER) {
      ...
      if (dscheme == differentiation_scheme::WENO3) {
        ...
        return my::math::weno3<value_type>(phi_nn, phi_n, phi_0, phi_p, 0, dx,
                                           false);
        ...
        return my::math::weno5<value_type>(phi_nnn, phi_nn, phi_n, phi_0, phi_p,
                                           phi_pp, 0, dx, false);
...
// NOTE Sparse field is assumed to be properly expanded (with respect to given
// stencil order in constructor)
...
*/
template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::neighbor_stencil {
...
public:
  neighbor_stencil(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx,
      const typename levelset<GridTraitsType,
                              LevelSetTraitsType>::const_iterator_runs &it_mid,
      const int order)
      : l(lx), stencil_order(order) {

    int num_stencil_points =
        std::pow(2 * stencil_order + 1,
                 D); // for neighborhood including diagonal neighbors
    ...
    // they are of the form [-3, -3], [-2, -3] [-1, -3] [0, -3] [1, -3] [2, -3],
    // [3, -3], ...
    for (int i = 0; i < num_stencil_points; ++i) {
      vec<index_type, D> tmp;
      for (int d = 0; d < D; ++d) {
        tmp[d] = index_type((int)(i / std::pow(2 * stencil_order + 1, d)) %
                                (2 * stencil_order + 1) -
                            stencil_order);
      }

      offsets.push_back(tmp);
      it_stencil_points.push_back(
          const_iterator_runs_offset(l, offsets[i], it_mid.start_indices()));
  ...
  void print_point(size_t index, std::ostream &out = std::cout) const {
    out << "Offset: " << it_stencil_points[index].get_offset()
        << ", start_indices(): "
        << it_stencil_points[index].start_indices() +
               it_stencil_points[index].get_offset()
  ...
  int indexByOffset(
      vec<index_type, D>
          offset) { // returns index of the star stencil with a certain offset
    return std::distance(offsets.begin(),
                         std::find(offsets.begin(), offsets.end(), offset));
  }

  std::vector<star_stencil>
  star_stencils(const differentiation_scheme ds =
                    differentiation_scheme::FIRST_ORDER) const {
  ...
  int get_center_index() const { return center_index; }
};

template <class GridTraitsType, class LevelSetTraitsType>
class levelset<GridTraitsType, LevelSetTraitsType>::const_iterator_neighbor {

  // this neighbor iterator consists of 2*Dimensions const_iterator_runs_offset
  // for the neighbors and a
  // const_iterator_runs-iterator for the center
  // whenever one of these (2*Dimensions+1) iterators reach a defined grid point
  // the const_iterator_neighbor iterator stops
  // this iterator is internally used for rebuilding and expanding the level set
  // function
...
public:
  template <class V>
  const_iterator_neighbor(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx, const V &v)
      : l(lx), start_coords(v), it_mid(lx, v) {
  ...
  const_iterator_neighbor(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx)
      : l(lx), start_coords(lx.grid().min_point_index()), it_mid(lx) {
  ...
  const const_iterator_runs &center() const { return it_mid; }

  bool is_finished() const { return center().is_finished(); }

  const vec<index_type, D> &start_indices() const { return start_coords; }

  index_type start_indices(int dir) const { return start_coords[dir]; }
};

template <class GridTraitsType, class LevelSetTraitsType>
template <class F>
class levelset<GridTraitsType,
               LevelSetTraitsType>::const_iterator_cells_filtered {
  ...
  // this iterator is used for surface extraction using the marching cubes
  // algorithm
  ...
  bool is_defined() const {
    ...
    for (int i = 0; i < D; i++) {
      if ((!l.grid().is_boundary_periodic(i)) &&
          (coords[i] == l.grid().max_point_index(i)))
        return false;
    }

    for (int i = 0; i < (1 << D); i++)
      if (filter(it_corners[i]))
        return true;
...
public:
  ...
  const_iterator_cells_filtered(
      const levelset<GridTraitsType, LevelSetTraitsType> &lx, const F &f = F())
      : l(lx), coords(l.grid().min_point_index()), filter(f) {

    // set all iterators = Min
    for (int i = 0; i < (1 << D); i++) {
      it_corners.push_back(
          const_iterator_runs_offset(lx, get_corner<D, index_type>(i)));
    }
  ...
  const vec<index_type, D> &indices() const { return coords; }

  index_type indices(int dir) const { return coords[dir]; }
};
