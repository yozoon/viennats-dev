...
public:
  template <class V> void push_back(const V &v) const {}
};
...
void extract(const LevelSetType &l,    // the level set function
             ...
             typename LevelSetType::value_type eps =
                 0. // eps can be used to avoid degenerated triangles
...
template <class LevelSetType, class SurfaceInserterType,
          class ActivePointListType>
void extract(
    const LevelSetType &l,    // the level set function
    SurfaceInserterType &srf, // the surface inserter type
    typename LevelSetType::value_type
        eps, // eps can be used to avoid degenerated triangles
    ActivePointListType
        &pt_lst // returns for each node of the surface mesh the nearest active
                // grid point which can occur for the marching cubes algorithm
                // the gridedge-surface intersection points are always set
                // at least a distance eps
                // away from both grid points of that grid edge
) {
  ...
  // IMPORTANT: this function requires that the level set function consists of
  // at least two layers of defined grid points
  //            this can be checked using the level set member function
  //            number_of_layers() the level set function can be expanded to 2
  //            layers of defined grid points using the member function
  //            expand(new_number_of_layers) with a value for
  //            new_numer_of_layers>=2
  //
  // the SurfaceInserterType has to be a class with following type definitions
  // and member functions
  //
  // class SurfaceInserterType {
  // public:
  //
  //   typedef unsigned int node_ref_type;     //here as example an unsigned int
  //   is used for referencing the nodes
  //
  //   node_ref_type insert_node(const LevelSetType::value_type*);  //the
  //   coordinates of each new node is passed to
  //                                                                //this
  //                                                                function. So
  //                                                                the user can
  //                                                                decide how
  ...
  //                                                                the new node
  //
  //   void insert_element(const node_ref_type*);                   //the node
  //   references of a new element are passed to this function
  //                                                                //In this
  //                                                                function the
  //                                                                user defines
  //                                                                how the
  ...
  assert(l.number_of_layers() >= 2); // test if level set function consists of
                                     // at least 2 layers of defined grid points
  ...
  // typedef typename std::vector<typename std::deque<node_ref_type> >
  // node_container2;
  ...
  for (typename LevelSetType::template const_iterator_cells_filtered<
           typename LevelSetType::filter_all>
           it_c(l);
    ...
    for (int u = 0; u < D; u++) {
      while (!nodes[u].empty() &&
             nodes[u].begin()->first < vec<index_type, D>(it_c.indices()))
        nodes[u].erase(nodes[u].begin());
    }

    unsigned int signs = 0;
    for (int i = 0; i < (1 << D); i++) {
      if (it_c.corner(i).sign() == POS_SIGN)
        signs |= (1 << i);
    }
    ...
    for (const int *Triangles = marching_cubes::polygonize<D>(signs);
      ...
      for (int n = 0; n < D; n++) {
        ...
        if (it != nodes[dir].end()) {
          ...
          for (int z = 0; z < D; z++) {
            if (z != dir) {
              cc[z] = value_type(it_c.indices(z) +
                                 get_corner<D, index_type>(p0)[z]);
            } else {
              value_type d0, d1;
              ...
              if (d0 == -d1) { // includes case where d0=d1=0
                ...
                if (math::abs(d0) <= math::abs(d1)) {
                  pt_lst.push_back(it_c.corner(p0).active_pt_id());
                  cc[z] = static_cast<value_type>(it_c.indices(z)) +
                          (d0 / (d0 - d1));
                } else {
                  pt_lst.push_back(it_c.corner(p1).active_pt_id());
                  cc[z] = static_cast<value_type>(it_c.indices(z) + 1) -
                          (d1 / (d1 - d0));
                }
              }
      ...
      if (parity)
        std::swap(nod_numbers[0],
                  nod_numbers[1]); // change the orientation of the elements in
                                   // dependence of the grid parity
...
///
///     This function takes an empty LevelSet and fills it with a box with lower
///     corner at start and upper corner at end
///
...
void make_box(
  ...
  for (int i = 0; i < D; ++i)
    num_points +=
        4.2 * std::abs(start[i] - end[i]) *
        ((D == 3) ? std::abs(start[(i + 1) % D] - end[(i + 1) % D]) : 1);
  points.reserve(num_points); // resize to stop many reallocs 4.2 instead of 4
                              // for extra points
  ...
  for (int i = 0; i < D; ++i) {
    ...
    int kmin = (D == 3) ? start[z] : 0,
        kmax = (D == 3) ? end[z] : 2; // in 2D, make loop run only once
  ...
  typename std::vector<std::pair<vec<index_type, D>, value_type>>::iterator it =
