...
template <class value_type> int GetStatusFromDistance(value_type value) {
  ...
  if (value >= 0.) {
    return (value <= static_cast<value_type>(x) + static_cast<value_type>(0.5))
               ? x
               : x + 1;
  } else {
    return (value >= static_cast<value_type>(x) - static_cast<value_type>(0.5))
...
template <class LevelSetType> std::string test(const LevelSetType &l) {
  ...
  for (typename LevelSetType::template const_iterator_neighbor_filtered<
           typename LevelSetType::filter_all, 1>
           it(l);
    ...
    if (it.center().is_defined()) {
      for (int i = 0; i < 2 * D; ++i) {
        ...
        if (it.neighbor(i).is_defined()) {
          if (math::abs(GetStatusFromDistance(it.center().value()) -
                        GetStatusFromDistance(it.neighbor(i).value())) > 1) {
            oss << "The defined point " << it.center().start_indices()
                << " has an inconsistent defined neighbor in direction " << i
                << "!" << std::endl;
            oss.precision(24);
            oss << "Value center point: " << it.center().value()
                << "  Value neighbor point: " << it.neighbor(i).value()
                << std::endl;
          }
        } else {
      ...
      for (int i = 0; i < 2 * D; ++i) {
        ...
        if (!it.neighbor(i).is_defined()) {
          if (it.center().sign() != it.neighbor(i).sign()) {
            oss << "The undefined run from " << it.center().start_indices()
                << " to " << it.center().end_indices()
...
void Write(const levelset<GridTraitsType, LevelSetTraitsType>& l, std::string
FileName, int option, float limit) {
    ...
    for (typename LevelSetType::const_iterator_runs it(l);
        ...
        if (it.start_coordinates()!=it.end_coordinates()) if
(!l.Grid.IsPositionInfinite(it.end_coordinates())) num++;
    }
...
<<std::endl; for (typename LevelSetType::const_iterator_runs it(l);
            !it.IsAtEnd();it.GoToNextRun()) {
        if (option==0) if (!it.IsDefined()) continue;
        ...
        if (!l.Grid.IsPositionInfinite(it.start_coordinates())) f << dist <<
std::endl; if (it.start_coordinates()!=it.end_coordinates()) if
(!l.Grid.IsPositionInfinite(it.end_coordinates())) f << dist << std::endl;
    }
