...
/// and parallel to axis "dir" (x=0, y=1, z=2) intersects the surface element
/// given by the nodes "c" additionally the normal-vector "SurfaceNormalVector"
/// of this surface element has to be given if there is an intersection this
/// function returns true, otherwise false the intersection coordinate is
/// returned by "intersection"
template <class T>
int calculate_gridline_triangle_intersection(vec<T, 3> Point,
                                             const vec<T, 3> *c, int dir,
                                             T &intersection) {
...
/// this function checks if a line containing the point with coordinates "Point"
/// and parallel to axis "dir" (x=0, y=1) intersects the surface element given
/// by the nodes "c" additionally the normal-vector "SurfaceNormalVector" of
/// this surface element has to be given if there is an intersection this
/// function returns true, otherwise false the intersection coordinate is
/// returned by "intersection"
template <class T>
int calculate_gridline_triangle_intersection(vec<T, 2> Point,
                                             const vec<T, 2> *c, int dir,
                                             T &intersection) {
  ...
  if ((!inside_pos && inside_neg) || (inside_pos && !inside_neg)) {
    ...
    intersection =
        c[k][dir] + (c[(k + 1) % 2][dir] - c[k][dir]) * (A[(k + 1) % 2] / sum);
...
void init(
    LevelSetType &l, // the level set function which should be initialized
    ...
    typename LevelSetType::value_type eps_sign =
        1e-6, //"eps_sign" is used to determine
              // the sign of the distance to the surface
    typename LevelSetType::value_type eps_boundary =
        1e-5, //"eps_boundary" defines the range at grid boundaries
              // within which all nodes snap exactly to the boundary coordinates
    typename LevelSetType::value_type eps_distance =
        1e-4, //"eps_distance" ensures that all grid points are initialized
              // which are are on a grid segment which is intersected by the
              // surface element
...

  {
    typedef typename std::vector<
        std::pair<vec<index_type, D>, std::pair<value_type, value_type>>>
        point_vector;
    point_vector points;
    ...
    for (element_index_type e = 0; e < srf.number_of_elements(); e++) {
      ...
      for (int dim = 0; dim < D; dim++) {
        ...
        for (int q = 0; q < D; q++) {
          ...
          if (math::abs(c[q][dim] - l.grid().min_local_coordinate(dim)) <
              boundary_eps_min[dim])
            c[q][dim] = l.grid().min_local_coordinate(dim);
          if (math::abs(c[q][dim] - l.grid().max_local_coordinate(dim)) <
              boundary_eps_max[dim])
      ...
      for (int z = 0; z < D; z++) { // for each axis direction do
        ...
        for (typename box<index_type, D - 1>::iterator it_bb(bb);
             !it_bb.is_finished(); it_bb++) {
          ...
          vec<value_type, D> p;
          for (int k = 1; k < D; k++)
            p[(k + z) % D] = l.grid().grid_position_of_global_index(
                (k + z) % D, it_b[(k + z) % D]);

          value_type intersection;
          int intersection_status =
              calculate_gridline_triangle_intersection(p, c, z, intersection);

          if (intersection_status != 0) {
            // if there is an intersection

            if (intersection < min_c[z])
              assert(0); // TODO
            if (intersection > max_c[z])
              assert(0); // TODO
            intersection = std::max(intersection, min_c[z]);
            intersection = std::min(intersection, max_c[z]);

            if (intersection > l.grid().max_local_coordinate(z))
              continue;
            if (intersection < l.grid().min_local_coordinate(z))
              continue;

            value_type intersection2 =
                l.grid().local_coordinate_2_local_index(z, intersection);

            index_type floor = static_cast<index_type>(
                std::floor(intersection2 - eps_distance));
            index_type ceil = static_cast<index_type>(
                std::ceil(intersection2 + eps_distance));
            ...
            for (it_b[z] = floor; it_b[z] <= ceil; ++(it_b[z])) {
              ...
              points.push_back(
                  std::make_pair(l.grid().global_indices_2_local_indices(it_b),
  ...
  if (report_import_errors) {
    ...
    if (err.size()) {                // if inconsistent print out error message
      std::cout << "Initialization of level set function from triangulated "
                   "surface failed!"
                << std::endl;
#ifdef VERBOSE
      ...
      std::cout << "Level set points have been written to importError.vtp"
                << std::endl;
      abort();
    }
  }

  l.prune(); // remove active grid point which have no opposite signed neighbor
             // grid point
