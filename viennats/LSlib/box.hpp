...
public:
  ...
  public:
    ...
    bool is_finished() const { return (pos[D - 1] > b.xMax[D - 1]); }

    const vec<T, D> &operator*() const { return pos; }
  };

  /// Returns vector of lowest point of box in each dimension.
  const vec<T, D> &min() const { return xMin; }
  /// Returns vector of highest point of box in each dimension.
  const vec<T, D> &max() const { return xMax; }
};

/// Sets xMin, xMax to the lowest and highest value of the two vectors for each
/// dimension respectively.
template <class T, int D>
inline box<T, D>::box(const vec<T, D> &idx0, const vec<T, D> &idx1) {
  xMin = Min(idx0, idx1);
  xMax = Max(idx0, idx1);
}
...
template <class T, int D>
inline box<T, D> &box<T, D>::operator=(const box<T, D> &b) {
  xMin = b.xMin;
  xMax = b.xMax;
  return *this;
}
