...
LS2RectiLinearGrid(const LevelSetType &LevelSet, const double LSOffset = 0.,
  ...
  for (unsigned i = 0; i < D; ++i) {
    ...
    if (LevelSet.grid().boundary_conditions(i) == lvlset::INFINITE_BOUNDARY) {
      ...
      gridMin = std::min(LevelSet.get_min_runbreak(i), infiniteMinimum) -
  ...
  // direction, so we can register a jump in the open boundary direction when it
  // occurs, so we can fix the LS value as follows: if remove_bottom==true, we
  // need to flip the sign otherwise the sign stays the same
  int currentOpenIndex =
      it_l.end_indices()[(openJumpDirection < D) ? openJumpDirection : 0];
  ...
  while ((pointId < rgrid->GetNumberOfPoints())) {
    ...
    // create index vector
    vec<typename LevelSetType::index_type, D> indices(
        LevelSet.grid().global_coordinates_2_global_indices(p));
    ...
    if (LevelSet.grid().is_outside_domain(indices)) {
      ...
      LSValue = (it_l.is_finished()) ? (LevelSetType::POS_VALUE)
                                     : it_l.value() + LSOffset;
    ...
    if (it_l.is_finished()) { // when iterator is done just fill all the
                              // remaining points
      ++pointId;
    } else {
  ...
  // now need to go through again to fix border points, this is done by mapping
  // existing points onto the points outside of the domain according to the
  ...
  if (fixBorderPoints) {
    ...
    while ((pointId < rgrid->GetNumberOfPoints())) {
      if (signedDistances->GetValue(pointId) ==
        ...
        vec<typename LevelSetType::index_type, D> localIndices =
            LevelSet.grid().global_indices_2_local_indices(indices);
...
/// This function removes duplicate points and agjusts the pointIDs in the cells
...
void removeDuplicatePoints(vtkSmartPointer<vtkUnstructuredGrid> &ugrid,
  ...
  // TODO transfer point data as well (do with "InsertTuples (vtkIdList *dstIds,
  // vtkIdList *srcIds, vtkAbstractArray *source) override)" of vtkDataArray
  // class)
...
// since the top levelset wraps all lower ones, all levelsets below it are used
// to cut this mesh explicitly. When all the cuts are made, material numbers are
// assigned and the tetra meshes written to one single file
...
void extract_volume(const LevelSetsType &LevelSets,
  ...
  assert(LevelSets.rbegin()->number_of_layers() >=
  ...
  clipper->SetInputData(LS2RectiLinearGrid<removeBottom>(
      *(LevelSets.rbegin()), 0, totalMinimum, totalMaximum)); // last LS
  clipper->InsideOutOn();
  clipper->Update();
  ...
  for (typename LevelSetsType::const_reverse_iterator it = ++LevelSets.rbegin();
    ...
    vtkSmartPointer<vtkRectilinearGrid> rgrid =
        vtkSmartPointer<vtkRectilinearGrid>::New();
    rgrid = LS2RectiLinearGrid<removeBottom, 1>(
        *it, -1e-4 * counter, totalMinimum,
        totalMaximum); // number of extra grid points outside and LSOffset
    ...
    // inverse for the next layer clipping Use vtkTabelBasedClipDataSet to slice
    // the grid
  ...
  for (unsigned i = 0; i < materialMeshes.size(); ++i) {
    ...
    materialNumberArray->SetName("Material");
    for (unsigned j = 0;
         j < materialMeshes[materialMeshes.size() - 1 - i]->GetNumberOfCells();
         ++j) {
    ...
    // if hull mesh should be exported, create hull for each layer and put them
    // together
  ...
  if (volumeMesh.GetPointer() != 0) {
    ...
    removeDuplicatePoints(volumeMesh,
                          1e-3 * LevelSets.front().grid().grid_delta());
  ...
  if (hullMesh.GetPointer() != 0) {
    ...
    removeDuplicatePoints(hullMesh,
                          1e-3 * LevelSets.front().grid().grid_delta());
