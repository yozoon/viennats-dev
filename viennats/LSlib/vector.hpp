...
inline vec<T, D>::vec(const vec<T1, D1> &v) {
...
template <class T, int D>
template <class V>
inline vec<T, D> &vec<T, D>::operator=(const V &v) {
  for (int i = 0; i < D; i++)
    x[i] = v[i];
  return *this;
}
...
template <class T, int D>
template <class V>
inline vec<T, D> vec<T, D>::operator-(const V &v) const {
  vec<T, D> w;
  for (int i = 0; i < D; i++)
    w.x[i] = x[i] - v[i];
  return w;
...
template <class T, int D>
template <class V>
inline bool vec<T, D>::operator==(const V &v) const {
  for (int i = D - 1; i >= 0; i--)
    if (x[i] != v[i])
      return false;
  return true;
...
template <class T, int D>
inline vec<T, D> Min(const vec<T, D> &v1, const vec<T, D> &v2) {
  vec<T, D> v;
  for (int i = 0; i < D; i++)
...
template <class T, int D>
inline vec<T, D> Max(const vec<T, D> &v1, const vec<T, D> &v2) {
  vec<T, D> v;
  for (int i = 0; i < D; i++)
...
template <class T, int D>
inline T Element_Max(const vec<T, D> &v1, const vec<T, D> &v2) {
  vec<T, D> v;
  for (int i = 0; i < D; i++)
    v[i] = std::max(v1[i], v2[i]);
  return v;
...
inline T Norm2(const vec<T, D> &v) { // squared l2 norm TODO name is misleading
  return dot(v, v);
}
...
// template <class V, class T,int D> inline vec<T,D> operator+(const V& v0,const
// vec<T,D>& v1) {
//     return v1.operator+(v0);
// }
...
template <class T, int D> inline void vec<T, D>::sort() { std::sort(x, x + D); }

template <class T, int D> inline void vec<T, D>::reverse_sort() {
  std::sort(x, x + D, std::greater<T>());
}

template <int D> inline int Parity(const vec<int, D> &v) { return Norm(v) % 2; }
...
template <class T, int D>
int compare(const vec<T, D> &v1, const vec<T, D> &v2) {
  for (int i = D - 1; i >= 0; --i) {
    if (v1[i] > v2[i])
      return 1;
    if (v1[i] < v2[i])
...
/*template <class T, int D> inline vec<T,D> vec<T,D>::ReplaceNth(int i,const T&
val) const { vec<T,D> v(x); v[i]=val; return v;
}*/
} // namespace lvlset
