...
double read_double(const H5Object &g, const string name) {
  ...
  if (a.getTypeClass() != H5T_FLOAT)
    mythrow("Wrong class " << typeid(a.getTypeClass()).name()
                           << " in atrribute");
  a.read(a.getDataType(), &i);
  return i;
}
...
struct tdr_geometry {
  ...
  void read_vertex(const DataSet &vert) {
    ...
    if (dim > 1)
      mtype2.insertMember("y", HOFFSET(coord2_t, x[1]),
                          PredType::NATIVE_DOUBLE);
    if (dim > 2)
      mtype2.insertMember("z", HOFFSET(coord2_t, x[2]),
                          PredType::NATIVE_DOUBLE);
    /*
  ...
  void read_elements(region_t &region, const DataSet &elem) {
    ...
    while (elct < dims[0]) {
      ...
      case 5:
        ...
        break;
      default:
        mythrow("Element type " << el[elct - 1] << " in region " << region.name
                                << " not known");
      }
    }
    delete[] el;
  ...
  void read_region(const int regnr, const Group &reg) {
    ...
    case 1: // we have a nD-1 region (contact... drain, source, ...)
      ...
      break;
    case 2:
      material = "Interface" + std::to_string(read_int(reg, "bulk 0")) +
                 std::to_string(read_int(reg, "bulk 1"));
      break;
      return;
    }
  ...
  region_t &find_region(int regnr) {
    for (map<string, region_t>::iterator R = region.begin(); R != region.end();
         R++)
  ...
  void read_values(dataset_t &dataset, const DataSet &values) {
    ...
    if (dataset.nvalues != dims[0] || ndims != 1)
      mythrow("Dataset " << dataset.name << " should have " << dataset.nvalues
                         << " values, but has " << dims[0] << " with dimension "
                         << ndims);
  ...
  void read_geometry(const Group &geometry) {
    ...
    switch (typ) {
    case 0:
      break; // found a geometry with type 0
    case 1:
      break;
    default:
      mythrow(__LINE__ << ": Unknown type " << typ << " in geometry");
    }
