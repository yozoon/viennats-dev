...
public:
  ...
  TiO2_ALD(const std::string &Parameters) {
    ...
    bool b =
        parse(
            ...
            *((str_p("direction") >> '=' >> '{' >>
              ...
              //(str_p("sticking_TTIP")        >> '='  >>
              //real_p[assign_a(sticking[0])]  >> ';') | (str_p("sticking_H2O")
              //>> '='  >> real_p[assign_a(sticking[1])]  >> ';') |
              (str_p("end_probability") >> '=' >>
              ...
              (str_p("molecular_thickness") >> '=' >>
               real_p[assign_a(molecular_thickness)] >> ';')),
    ...
    NumberOfParticleClusters[0] =
        (FluxTTIP > 0.) ? num_particles : 0; // TTIP flux
    NumberOfParticleClusters[1] = (FluxH2O > 0.) ? num_particles : 0; // H2O
                                                                      // flux
    //                    std::cout<< << "\n";
    //                    time_step=0.001;
    //                    this->setTimes(1.);
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    //                     k2_pyrolysis*kdes_TTIP*Coverages0 +
    //                     k_hydrolysis*Cov0_H2O*Coverages1
    //                    Velocity =
    //                    0.;//std::max(Coverages[1]*Coverages[0]*Cov0_TTIP*Cov0_H2O/k_hydrolysis
    //                    + Coverages[1]*Coverages[0]/Cov0_H2O,0.); std::cout <<
    //                    "Velocity = " << Velocity << "\n";
    Velocity =
        molecular_thickness *
        std::max(Coverages[0] * Coverages[3] + Coverages[1] * Coverages[2], 0.);
  }
  ...
  void
  UpdateCoverage(double *Coverages, const double *Rates,
                 double &delta_time) const { //, double &CurrentTime) const {
    // delta_time=1e-5;
    //                    std::cout << "    Coverages[0] = " << Coverages[0] <<
    //                    ", "; std::cout << "    Coverages[1] = " <<
    //                    Coverages[1] << "\n";
    const double Coverages0 = Coverages[0] * Cov0_TTIP;
    const double Coverages1 = Coverages[1] * Cov0_H2O;

    const double C_TTIP =
        (Rates[0] * sticking[0] //*(1.-Coverages[0])
         - kdes_TTIP * Coverages0 - k2_pyrolysis * Coverages0 * Coverages0 -
         k_hydrolysis * Coverages0 * Coverages1) /
        Cov0_TTIP;
    //                                C_TTIP /= Cov0_TTIP;
    const double t_TTIP = std::min(0.01 / std::abs(C_TTIP), 0.001);
    const double C_H2O =
        (Rates[1] * sticking[1] //*(1.-Coverages[1])
         - kdes_H2O * Coverages1 - k_hydrolysis * Coverages0 * Coverages1) /
        Cov0_H2O;
    //                                C_H2O /= Cov0_H2O;
    ...
    //                    ", "; std::cout << "1 - Coverages[1] = " <<
    //                    Coverages[1] << "\n";

    //                    const double A0 = Rates[0]/Cov0_TTIP;    // 2e3
    //                    const double B0 = kdes_TTIP +
    //                    k2_pyrolysis*kdes_TTIP*Coverages0 +
    //                    k_hydrolysis*Cov0_H2O*Coverages1 + A0; // 7.803e11
    //
    //                    const double A1 = Rates[1]/Cov0_H2O;     // 2.427e4
    //                    const double B1 = kdes_H2O +
    //                    k_hydrolysis*Cov0_TTIP*Coverages0 + A1;   // 8.574e13
    //                    delta_time = std::min(error/B0,error/B1);
    //                    //time = 0.001;
    //
    //                    Coverages[0] = std::max(A0/B0 + (Coverages0 -
    //                    A0/B0)*std::exp(-B0*(delta_time)),0.);   // 2.563e-9
    //                    Coverages[1] = std::max(A1/B1 + (Coverages1 -
    //                    A1/B1)*std::exp(-B1*(delta_time)),0.);   // 2.830e-10
  }
  ...
  void ParticleCollision(const PT &p, const NormVecType &NormalVector,
    ...
    Rates[p.Type] +=
        p.Flux * p.Probability * std::max(1. - Coverages[p.Type], 0.);
    ...
    /*
                        switch(p.Type) {
                            case 0: {   //TTIP
    //                            const double
    StickingProbability=std::max(s_TTIP*(1.-Coverages[0]),0.);
                                Rates[0]+=p.Flux*p.Probability;//StickingProbability;
                                break;
                            }
                            case 1: {   //H2O
    //                            const double
    StickingProbability=std::max(s_H2O*(1.-Coverages[1]),0.);
  ...
  void ParticleReflexion(
    ...
    //			}
    //                            const double
    //                            new_probability=p.Probability*(1-StickingProbability);
    ...
    /*
                        switch(p.Type) {
                            case 0: {   //TTIP
    //                            const double
    StickingProbability=std::max(s_TTIP*(1.-Coverages[0]),0.); if
    (Coverages[0]>0.) {
    ...
    //                            const double
    new_probability=p.Probability*(1-StickingProbability); if
    (new_probability>=end_probability) { particle_stack.push(p); PT&
    p_new=particle_stack.top(); p_new.Probability=new_probability;
                                    my::stat::Cosine1DistributedRandomDirection(NormalVector,p_new.Direction);
                                }
                                break;
                            }
    ...
    StickingProbability=std::max(s_H2O*(1.-Coverages[1]),0.);
    //                            const double
    new_probability=p.Probability*(1-StickingProbability); if (Coverages[1]>0.)
    {
...
//        static const bool ReemissionIsMaterialDependent=true;
//
//        TiO2_ALD_H2O (const std::string & Parameters, const double
//        ProcessTime) {
//
//		    using namespace boost::spirit::classic;
//
...
//
//            if (!b) msg::print_error("Failed interpreting process
//            parameters!");
//
//		    unsigned int num_particles=static_cast<unsigned
//int>(Accuracy);
//
...
//
//		template <class VecType>
//		void CalculateVelocity(double &Velocity, const VecType&
//NormalVector, const double *Coverages, const double *Rates, int Material, bool
//Connected, bool Visible) const {
...
//		}
//
//		template <class PT> void ParticleGeneration(PT& p, int
//ParticleType, double ProcessTime, double* Position) const {
//
//                    my::stat::Cosine1DistributedRandomDirection(StartDirection,p.Direction);
//                    p.Probability=1.;
//                    p.Flux=1.;
...
//                    double n_N=1.2;
//
//                    Rates[0]+=std::max(C_T_N  *
//                    std::pow(Coverages[2]-Rates[0],         n_N),0.);
//                    Rates[1]+=std::max(C_TiN_N*
//                    std::pow(Coverages[4]+Rates[0]-Rates[1],n_N),0.);
//
//
//		}
//
...
//			int Material,
//			int D,
//			double dot // dot product between the incoming particle
//direction and the normal vector 			) const {
//
//			double new_probability;
...
double const TiO2_ALD::Cov0_H2O = 1.5e15;  // /cm2
                                           //        double const
//        TiO2_ALD::kdes_TTIP=2.0e10*std::exp(-114*TiO2_ALD::kJm_eV/(TiO2_ALD::k_B*TiO2_ALD::temperature));
//        // /s double const
//        TiO2_ALD::kdes_H2O=5.0e9*std::exp(-81*TiO2_ALD::kJm_eV/(TiO2_ALD::k_B*TiO2_ALD::temperature));
//        // /s double const
//        TiO2_ALD::k2_pyrolysis=1.3e3*std::exp(-185*TiO2_ALD::kJm_eV/(TiO2_ALD::k_B*TiO2_ALD::temperature));
//        // cm2/s double const
//        TiO2_ALD::k_hydrolysis=4.7e-7*std::exp(-86*TiO2_ALD::kJm_eV/(TiO2_ALD::k_B*TiO2_ALD::temperature));
//        // cm2/s

double const TiO2_ALD::kdes_TTIP =
    2.0e10 * std::exp(-114000 / (TiO2_ALD::gas * TiO2_ALD::temperature)); // /s
double const TiO2_ALD::kdes_H2O =
    5.0e9 * std::exp(-81000 / (TiO2_ALD::gas * TiO2_ALD::temperature)); // /s
double const TiO2_ALD::k2_pyrolysis =
    1.3e3 *
    std::exp(-185000 / (TiO2_ALD::gas * TiO2_ALD::temperature)); // cm2/s
double const TiO2_ALD::k_hydrolysis =
    4.7e-7 *
    std::exp(-86000 / (TiO2_ALD::gas * TiO2_ALD::temperature)); // cm2/s
