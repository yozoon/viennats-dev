...
template <class PPT> class Cl2_CH4PlasmaEtching {
...
public:
  ...
  Cl2_CH4PlasmaEtching(
      typename std::list<PPT>::const_iterator p) { //, int stacked_material) {
    ...
    bool b =
        parse(p->ModelParameters.begin(), p->ModelParameters.end(),
              *((str_p("direction") >> '=' >> '{' >>
                ...
                (str_p("min_ion_energy") >> '=' >>
                 real_p[assign_a(min_ion_energy)] >> ';') |
                (str_p("delta_ion_energy") >> '=' >>
                 real_p[assign_a(delta_ion_energy)] >> ';')),
    ...
    NumberOfParticleClusters[0] = (FluxIon > 0.) ? num_particles : 0; // Ion
                                                                      // flux
    NumberOfParticleClusters[1] =
        (FluxO > 0.) ? num_particles : 0; // Oxygen flux
    NumberOfParticleClusters[2] =
        (FluxCl > 0.) ? num_particles : 0; // Fluorine flux
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    if (Material <= AddLayer) { // Si
      Velocity = (-(k_Sigma_Si * Coverages[1] / 4. + Rates[0] +
                    Rates[1] * Coverages[1]) /
                  rho_TiN); // in cm/s      //TODO
      // if (Coverages[1]>1) std::cout << "Coverages[1]= " << Coverages[1] << ",
      // Rates[0]= " << Rates[0] << ", Rates[1]= " << Rates[1] << "\n";
      //} else if (Material==2) {
      // Velocity=(-Rates[5]*Coverages[1]/rho_SiO2);
      // //in cm/s     //TODO
    } else
      Velocity = 0.;
    // assert(0);
  ...
  static void UpdateCoverage(double *Coverages, const double *Rates) {
    ...
    Coverages[1] =
        Rates[4] /
        (Rates[4] + (k_Sigma_Si + 2 * Rates[1]) *
                        (1. + Rates[3] / (beta_Sigma_Si + Rates[2])));

    Coverages[0] =
        Rates[3] /
        (Rates[3] + (beta_Sigma_Si + Rates[2]) *
                        (1. + Rates[4] / (k_Sigma_Si + 2 * Rates[1])));
  }
  ...
  void ParticleGeneration(PT &p, int ParticleType, double ProcessTime,
    ...
    switch (ParticleType) {
    case 0: // ION
      my::stat::CosineNDistributedRandomDirection(ion_exponent, StartDirection,
                                                  p.Direction);
      p.Probability = 1;
  ...
  void ParticleCollision(const PT &p, const NormVecType &NormalVector,
    ...
    case 0: { // ION
      ...
      if (cos > 0.5)
        ...
        f_Si_phi = std::max(3. - 6. * angle / my::math::Pi, 0.);
      if (cos > 0.5)
        f_O_phi = 1.;
      else
        f_O_phi = std::max(3. - 6. * angle / my::math::Pi, 0.);
      // if (cos>0.5) f_p_phi=1.; else f_p_phi=3.-6.*angle/my::math::Pi;
      f_p_phi = 1.;
      // if (cos>0.5) f_p_phi=1.; else f_p_phi=std::asin(cos)/std::asin(0.5);
      // //TODO if (cos>0.5) f_SiO2_phi=1.; else
      // f_SiO2_phi=3.-6.*angle/my::math::Pi;
      f_SiO2_phi = 1.;

      const double sqrtE = std::sqrt(p.Energy);
      const double Y_p = A_p * std::max(sqrtE - std::sqrt(Eth_p), 0.) * f_p_phi;
      const double Y_Si =
          A_Si * std::max(sqrtE - std::sqrt(Eth_Si), 0.) * f_Si_phi;
      const double Y_O = A_O * std::max(sqrtE - std::sqrt(Eth_O), 0.) * f_O_phi;
      const double Y_SiO2 =
          A_SiO2 * std::max(sqrtE - std::sqrt(Eth_SiO2), 0.) * f_SiO2_phi;
    ...
    case 1: { // OXYGEN
      const double StickingProbability =
          std::max(Gamma_O * (1. - Coverages[0] - Coverages[1]), 0.);
      Rates[3] += p.Flux * StickingProbability;
      break;
    }
    case 2: { // FLUORINE
      const double StickingProbability =
          std::max(Gamma_F * (1. - Coverages[0] - Coverages[1]), 0.);
  ...
  void ParticleReflexion(
    ...
    case 0: { // ION
      ...
      const double A =
          1. / (1. + (n_l / n_r) * (my::stat::Pi1_2 / Phi_inflect - 1.));

      const double dot =
          (NormalVector[0] * p.Direction[0] + NormalVector[1] * p.Direction[1] +
           NormalVector[2] * p.Direction[2]);
      ...
      if (Phi >= Phi_inflect) {
        Eref_peak = Eref_max *
                    (1 - (1 - A) * std::pow((my::stat::Pi1_2 - Phi) /
                                                (my::stat::Pi1_2 - Phi_inflect),
                                            n_r));
      } else {
      ...
      do {
        NewEnergy =
            TempEnergy +
            (std::min((p.Energy - TempEnergy), TempEnergy) + 0.05 * p.Energy) *
      ...
      if (NewEnergy >= MinEnergy) {
        ...
        my::stat::CosAngleDistributedRandomDirection(
            my::stat::Pi1_2 - std::min(Phi, Phi_min), ReflectedDirection,
            p_new.Direction);
      }
    ...
    case 1: { // OXYGEN
      ...
      if (new_probability >= end_probability) {
        ...
        my::stat::Cosine1DistributedRandomDirection(NormalVector,
                                                    p_new.Direction);
      }
      break;
    }
