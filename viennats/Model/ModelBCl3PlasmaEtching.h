...
public:
  ...
  BCl3PlasmaEtching(const std::string &Parameters) { //, int stacked_material) {
    ...
    NumberOfParticleClusters[2] =
        (FluxBCl3 > 0.) ? num_particles : 0; // Fluorine flux
  }
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    // return;
    if (Material == 2) { // HfO2
      Velocity = (-(k_Sigma_HfO2 * Coverages[1] / 4. + Rates[0] +
                    Rates[1] * Coverages[1]) /
                  rho_HfO2); // in cm/s      //TODO
      if (Coverages[1] > 1)
  ...
  static void UpdateCoverage(double *Coverages, const double *Rates) {
    ...
    Coverages[1] =
        Rates[4] /
        (Rates[4] + (k_Sigma_HfO2 + 2 * Rates[1]) *
                        (1. + Rates[3] / (beta_Sigma_HfO2 + Rates[2])));

    Coverages[0] =
        Rates[3] /
        (Rates[3] + (beta_Sigma_HfO2 + Rates[2]) *
                        (1. + Rates[4] / (k_Sigma_HfO2 + 2 * Rates[1])));
  }
  ...
  void ParticleCollision(const PT &p, const NormVecType &NormalVector,
    ...
    case 0: { // ION
      ...
      if (cos > 0.5)
        ...
        f_HfO2_phi = std::max(3. - 6. * angle / my::math::Pi, 0.);
      if (cos > 0.5)
        f_O_phi = 1.;
      else
      ...
      const double Y_HfO2 =
          A_HfO2 * std::max(sqrtE - std::sqrt(Eth_HfO2), 0.) * f_HfO2_phi;
