...
public:
  ...
  SimpleDeposition(const std::string &Parameters)
      : StartAngleDistribution(1.), ReemittedAngleDistribution(1.) {
    ...
    bool b =
        parse(Parameters.begin(), Parameters.end(),
              *((str_p("direction") >> '=' >> '{' >>
                ...
                (str_p("sticking_probabilities") >> '=' >> '{' >>
                 (real_p[push_back_a(StickingProbabilities)] % ',') >> '}' >>
                 ';') |
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    double Yield =
        (Material < static_cast<int>(Yields.size())) ? Yields[Material] : 0;
  ...
  void ParticleReflexion(
    ...
    double StickingProbability =
        (Material < static_cast<int>(StickingProbabilities.size()))
            ? StickingProbabilities[Material]
            : 1.0;
