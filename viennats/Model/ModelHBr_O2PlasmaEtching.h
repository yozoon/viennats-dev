...
template <class PPT> class HBr_O2_PlasmaEtching {
...
public:
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    if (Coverages[1] > 1) { // Deposition
      ...
      if (Material <= AddLayer) { //}<(int)ActiveLayers.size()){//Material<=2){//my::stat::AnyElement<int>(ActiveLayers,
                                  //Material)) {	//Si
        Velocity = -0.1 *
