...
public:
  ...
  WetEtching(const std::string &Parameters) {
    ...
    bool b = parse(Parameters.begin(), Parameters.end(),
                   *((str_p("direction100") >> '=' >> '{' >>
                     ...
                     (str_p("rates100") >> '=' >> '{' >>
                      (real_p[push_back_a(r100)] % ',') >> '}' >> ';') |
                     (str_p("rates110") >> '=' >> '{' >>
                      (real_p[push_back_a(r110)] % ',') >> '}' >> ';') |
                     (str_p("rates111") >> '=' >> '{' >>
                      (real_p[push_back_a(r111)] % ',') >> '}' >> ';') |
                     (str_p("rates311") >> '=' >> '{' >>
                      (real_p[push_back_a(r311)] % ',') >> '}' >> ';')),
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    Velocity = my::math::fourRateInterpolation<double, 3>(
        nv, direction100, direction010, r100[Material], r110[Material],
        r111[Material], r311[Material]);
  }
