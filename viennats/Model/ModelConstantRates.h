...
public:
  ...
  ConstantRates(const std::string &Parameters, bool masked = false)
      : CalculateConnectivities(true), CalculateVisibilities(true),
        CalculateNormalVectors(true), IncludeVectorRates(true) {
    ...
    bool b =
        parse(Parameters.begin(), Parameters.end(),
              *((str_p("direction") >> '=' >> '{' >>
                ...
                (str_p("constant_rates") >> '=' >> '{' >>
                 (real_p[push_back_a(constant_rates)] % ',') >> '}' >> ';') |
                (str_p("isotropic_rates") >> '=' >> '{' >>
                 (real_p[push_back_a(isotropic_rates)] % ',') >> '}' >> ';') |
                (str_p("directional_rates") >> '=' >> '{' >>
                 (real_p[push_back_a(directional_rates)] % ',') >> '}' >> ';') |
                (str_p("vector_rates") >> '=' >> '{' >>
                 (real_p[push_back_a(vector_rates)] % ',') >> '}' >> ';') |
                (str_p("bidirectional_rates") >> '=' >>
                 ((str_p("true") |
                   str_p("false"))[assign_bool(bidirectionalRates)]) >>
                 ';')
    ...
    if (isotropic_rates.size() == 0)
      CalculateConnectivities = false;
    ...
    if (vector_rates.size() == 0)
      IncludeVectorRates = false;

    DirEmpty = true;
    for (int i = 0; i < 3; i++)
      if (StartDirection[i] != 0)
        DirEmpty = false;
    if (!DirEmpty) {
      double start_norm = 0;
      for (int i = 0; i < 3; ++i)
        start_norm += StartDirection[i] * StartDirection[i];
      start_norm = std::sqrt(start_norm);
      for (int i = 0; i < 3; ++i)
        StartDirection[i] /= start_norm;
    } // else DirEmpty=0;
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    double isotropic_rate =
        (Material < static_cast<int>(isotropic_rates.size()))
            ? isotropic_rates[Material]
            : 0;
    double directional_rate =
        (Material < static_cast<int>(directional_rates.size()))
            ? directional_rates[Material]
            : 0;
    double constant_rate = (Material < static_cast<int>(constant_rates.size()))
                               ? constant_rates[Material]
                               : 0;

    Velocity = constant_rate;

    if (connected)
      Velocity += isotropic_rate;

    if ((visible) && (directional_rate != 0)) {
      double dot = 0.;
      for (int i = 0; i < 3; ++i)
        dot -= StartDirection[i] * NormalVector[i];
      if (bidirectionalRates)
        Velocity += directional_rate * std::abs(dot);
      else
...
//	const unsigned int
//ConstantRates::NumberOfParticleClusters[ConstantRates::NumberOfParticleTypes]={};
