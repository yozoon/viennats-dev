...
// "Two precursor model for low pressure chemical vapor deposition of silicon
// dioxide from tetraethylorthosilicate", J. Vac. Sci. technol. B 11 (3), 1993
...
public:
  ...
  TwoSpeciesDeposition(const std::string &Parameters) {
        ...
        parse(Parameters.begin(), Parameters.end(),
              *((str_p("direction") >> '=' >> '{' >>
                ...
                (str_p("rate_main") >> '=' >> real_p[assign_a(rate_main)] >>
                 ';') |
                (str_p("rate_intermediate") >> '=' >>
                 real_p[assign_a(rate_intermediate)] >> ';') |
                // (str_p("flux_main")  >> '='  >> real_p[assign_a(flux_main)]
                // >> ';') | (str_p("flux_intermediate")  >> '='  >>
                // real_p[assign_a(flux_intermediate)]  >> ';') |
                (str_p("sticking_main") >> '=' >>
                 real_p[assign_a(sticking_main)] >> ';') |
                (str_p("sticking_intermediate") >> '=' >>
                 real_p[assign_a(sticking_intermediate)] >> ';') |
    ...
    //            if (flux_intermediate > 0) rate_intermediate =
    //            flux_intermediate/rho_SiO2;
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    //?                    Velocity = rate_main*Rates[0] +
    //rate_intermediate*Rates[1];
...
//	double const TwoSpeciesDeposition::rho_SiO2 = 2.3e22; 	//in atoms/cm^3
////2.6e22
