...
public:
  ...
  FIB(const std::string &Parameters) {
    ...
    bool b =
        parse(
            ...
            *((str_p("FWHM") >> '=' >> real_p[assign_a(FWHM)] >> ';') |
              ...
              (str_p("num_reemitted_particles") >> '=' >>
               int_p[assign_a(num_reemitted_particles)] >> ';') |
              (str_p("target_density") >> '=' >>
               real_p[assign_a(target_density)] >> ';') |
              (str_p("stop_criterion") >> '=' >>
               real_p[assign_a(end_probability)] >> ';') |
              (str_p("sticking_probability") >> '=' >>
               real_p[assign_a(sticking_probability)] >> ';') |
              (str_p("position") >> '=' >> '{' >>
               real_p[assign_a(StartPosition[0])] >> "," >>
               real_p[assign_a(StartPosition[1])] >> "," >>
               real_p[assign_a(StartPosition[2])] >> '}' >> ';') |
              (str_p("direction") >> '=' >> '{' >>
               real_p[assign_a(StartDirection[0])] >> "," >>
               real_p[assign_a(StartDirection[1])] >> "," >>
               real_p[assign_a(StartDirection[2])] >> '}' >> ';') |
              (str_p("sputter_yield_file") >> '=' >> '\"' >>
               *((~ch_p('\"'))[push_back_a(SputterYieldFile)]) >> '\"' >> ';') |
              (str_p("num_simulated_particles") >> '=' >>
               int_p[assign_a(num_simulated_particles)] >> ';')),
    ...
    double norm = std::sqrt(StartDirection[0] * StartDirection[0] +
                            StartDirection[1] * StartDirection[1] +
                            StartDirection[2] * StartDirection[2]);
    assert(norm > 0.);
    StartDirection[0] /= norm;
    StartDirection[1] /= norm;
    StartDirection[2] /= norm;

    std::cout << StartDirection[0] << " " << StartDirection[1] << " "
              << StartDirection[2] << std::endl;
  ...
  void CalculateVelocity(double &Velocity, const NormVecType NormalVector,
  ...
  void DefineAngles(const ParameterType& Parameter, const PartitionType&
  Partition, double &theta_min, double &theta_max) const {
  }

  template <class PT, class ParameterType, class PartitionType>
  void ParticleGeneration(PT& p, int ParticleType, double ProcessTime, double*
  Position, const ParameterType& Parameter, const PartitionType& Partition,
...
*/
  ...
  void ParticleGeneration(PT &p, int ParticleType, double ProcessTime,
    ...
    */
    for (int i = 0; i < 3; ++i)
      p.Direction[i] = StartDirection[i];

    // PickRandomPointOnUnitCircle(cosphi, sinphi);
    //       	r2=1;

    my::stat::NormalDistributedStartPosition(StartPosition, StartDirection,
                                             FWHM, Position);
  ...
  void ParticleCollision(const PT &p, const NormVecType &NormalVector,
    ...
    if (p.direct) {
      double dot = NormalVector[0] * p.Direction[0] +
      ...
      if (use_yamamura) {
        ...
        Rates[0] +=
            p.Flux * my::math::Interpolate(&SputterYieldX[0], &SputterYieldY[0],
                                           SputterYieldX.size(), cos);
      }
    } else {
    ...
    if (calculate_redeposition) {
      if (p.direct) {
        ...
        if (use_yamamura) {
          ...
          new_probability = my::math::Interpolate(
              &SputterYieldX[0], &SputterYieldY[0], SputterYieldX.size(), cos);
        }
