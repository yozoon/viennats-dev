...
public:
  ...
  CFx_Deposition(const std::string &Parameters) {
    ...
    NumberOfParticleClusters[0] =
        (IonFlux > 0.) ? static_cast<unsigned int>(Accuracy) : 0;
    NumberOfParticleClusters[1] =
        (CFxFlux > 0.) ? static_cast<unsigned int>(Accuracy) : 0;
  }
  ...
  void ParticleGeneration(PT &p, int ParticleType, double ProcessTime,
    ...
    switch (ParticleType) {
    case 0: { // Ion
      my::stat::CosineNDistributedRandomDirection(IonAngleDistribution,
    ...
    case 1: { // CFx
      my::stat::CosineNDistributedRandomDirection(CFxAngleDistribution,
                                                  StartDirection, p.Direction);
      p.Probability = 1.;
      p.Flux = CFxFlux;
      p.Type = 1;
