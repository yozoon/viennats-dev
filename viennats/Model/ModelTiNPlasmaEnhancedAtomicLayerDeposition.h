...
public:
    ...
    bool b =
        parse(
            ...
            *((str_p("direction") >> '=' >> '{' >>
              ...
              //                            (str_p("m_Plasma")  >> '='  >>
              //                            real_p[assign_a(m_Plasma)] >> ';') |
              //                            (str_p("m_TDMAT")  >> '='  >>
              //                            real_p[assign_a(m_TDMAT)] >> ';') |
  ...
  void CalculateVelocity(double &Velocity, const VecType &NormalVector,
    ...
    //                    Velocity =
    //                    molecular_thickness*std::max(Coverages[5]/m_Plasma +
    //                    Coverages[11]/m_TDMAT,0.);
  }
  ...
  //{ 		static void UpdateCoverage(double *Coverages, const double *Rates, double
  //&delta_time, double &CurrentTime) {
  void
  UpdateCoverage(double *Coverages, const double *Rates,
    ...
    //                        << "\n"; std::cout << "Coverages[(6*Type+10)%12] =
    //                        " << Coverages[(6*Type+10)%12] << "\n"; std::cout
    //                        << "Coverages0_6 = " << Coverages0_6 << "\n";
    ...
    Coverages[6 * Type + 0] =
        std::max(Coverages[(6 * Type + 8) % 12] -
                     std::pow(std::max(std::pow(Coverages[(6 * Type + 8) % 12] -
                                                    Coverages0_6,
                                                1. - n[Type]) -
                                           (1 - n[Type]) * Rates[0] *
                                               C[2 * Type + 0] * delta_time,
                                       0.),
                              1. / (1. - n[Type])),
                 0.);
    ...
    Coverages[6 * Type + 1] =
        Coverages1_7 +
        Rates[0] * C[2 * Type + 1] * delta_time *
            std::max(std::pow(std::max(Coverages[(6 * Type + 10) % 12] +
                                           Coverages0_6 - Coverages1_7,
                                       0.),
                              n[Type]),
                     0.);
    ...
    Coverages[6 * Type + 2] =
        Coverages[(6 * Type + 9) % 12] + Coverages[6 * Type + 1];
    // NH3 or TDMAT after a TDMAT or NH3 step
    Coverages[6 * Type + 3] =
        Coverages[(6 * Type + 8) % 12] - Coverages[6 * Type + 0];
    // TiN after a TDMAT or NH3 step
    Coverages[6 * Type + 4] = Coverages[(6 * Type + 10) % 12] +
                              Coverages[6 * Type + 0] - Coverages[6 * Type + 1];
    // Change in coverage for the relevant species
    Coverages[6 * Type + 5] =
        std::max((Coverages[6 * Type + 0] - Coverages0_6) / delta_time, 0.);
  }
