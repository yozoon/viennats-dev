...
template <class PPT> class CalculateFlux {
...
public:
  ...
  CalculateFlux(typename std::list<PPT>::const_iterator p, int D)
      : StartAngleDistribution(1.), ReemittedAngleDistribution(1.), dim(D) {
    ...
    bool b =
        parse(
            p->ModelParameters.begin(), p->ModelParameters.end(),
            *((str_p("direction") >> '=' >> '{' >>
               real_p[assign_a(StartDirection[0])] >> "," >>
               real_p[assign_a(StartDirection[1])] >> "," >>
               real_p[assign_a(StartDirection[2])] >> '}' >> ';') |
              (str_p("reflection_model") >> '=' >>
               (str_p("DIFFUSIVE_SINGLE")[parser_actors::assign_enum<
                    ReflectionModelType>(ReflectionModel, DIFFUSIVE_SINGLE)] |
                str_p("DIFFUSIVE_MULTIPLE")[parser_actors::assign_enum<
                    ReflectionModelType>(ReflectionModel, DIFFUSIVE_MULTIPLE)] |
                str_p(
                    "SPECULAR")[parser_actors::assign_enum<ReflectionModelType>(
                    ReflectionModel, SPECULAR)] |
                str_p("DIFFUSIVE_COMBINED")[parser_actors::assign_enum<
                    ReflectionModelType>(ReflectionModel, DIFFUSIVE_COMBINED)] |
                str_p("NONE")[parser_actors::assign_enum<ReflectionModelType>(
                    ReflectionModel, NONE)]) >>
               ';') |
              (str_p("reflection_materials") >> '=' >> '{' >>
               (int_p[push_back_a(reflection_materials_temp)] % ',') >> '}' >>
               ';') |
              (str_p("reflection_diffusive_upperbound") >> '=' >>
               int_p[assign_a(reflection_diffusive_upper_bound)] >> ';') |
              //                            (str_p("reference_start")  >> '=' >>
              //                            real_p[assign_a(reference_start)] >>
              //                            ';') |
              (str_p("flux") >> '=' >> real_p[assign_a(TotalFlux)] >> ';') |
              (str_p("sticking_probability") >> '=' >>
               real_p[assign_a(StickingProbability)] >> ';') |
              (str_p("start_angle_distribution") >> '=' >>
               real_p[assign_a(StartAngleDistribution)] >> ';') |
              (str_p("reemitted_angle_distribution") >> '=' >>
               real_p[assign_a(ReemittedAngleDistribution)] >> ';') |
    ...
    std::copy(
        reflection_materials_temp.begin(), reflection_materials_temp.end(),
        std::inserter(reflection_materials, reflection_materials.begin()));
    ...
    NumberOfParticleClusters[0] =
        (TotalFlux > 0.) ? static_cast<unsigned int>(Accuracy) : 0;
  }
  ...
  void ParticleGeneration(PT &p, int ParticleType, double ProcessTime,
    ...
    my::stat::CosineNDistributedRandomDirection(StartAngleDistribution,
  ...
  void ParticleReflexion(
    ...
    if (ReflectionModel == NONE)
      return;

    // perform the reflection if either the material ID is member of the
    // materials which is suppsoed to reflect or if the material list is empty,
    // which has the implicit meaning that all materials should reflect.
    //
    if ((reflection_materials.find(Material) != reflection_materials.end()) ||
        reflection_materials.empty()) {
      // this is the old approach, generate one new particle and assign a random
      // direction to it
      //
      if (ReflectionModel == DIFFUSIVE_SINGLE) {
        ...
        if (new_probability >= end_probability) {
          ...
          my::stat::CosineNDistributedRandomDirection(
              ReemittedAngleDistribution, NormalVector, p_new.Direction);
        }
      }
      // diffusive reflection: generate several particles and assign random
      // directions to them
      //
      else if (ReflectionModel == DIFFUSIVE_MULTIPLE) {
        //          double
        //          new_probability=(p.Probability*(1.-StickingProbability))/(1.*reflection_diffusive_upper_bound);
        double new_probability = p.Probability * (1. - StickingProbability);
        if (new_probability >= end_probability) {
          //            std::cout << particle_stack.size() << " vs. ";
          // evenly distribute the probability among the newly generated
          // particles
          ...
          for (int i = 0; i < reflection_diffusive_upper_bound; i++) {
            ...
            my::stat::CosineNDistributedRandomDirection(
                ReemittedAngleDistribution, NormalVector, p_new.Direction);
            //              std::cout << p_new.Direction[0] << " "<<
            //              p_new.Direction[0] << " " << std::endl;
          }
          //            exit(0);
          //            std::cout << particle_stack.size() << std::endl;
        }
      }
      // specular reflection: generate one new particle and assign the reflected
      // angle to it
      //
        ...
        if (new_probability >= end_probability) {
          ...
          // assign the new particle the reflection vector as direction: R = V -
          // 2N<V,N> http://mathworld.wolfram.com/Reflection.html

          const double dot = (NormalVector[0] * p.Direction[0] +
                              NormalVector[1] * p.Direction[1] +
                              NormalVector[2] * p.Direction[2]);
          for (int d = 0; d < 3; ++d) {
        ...
        if (new_probability >= end_probability) {
          ...
          if (p.Probability >= 0.1)
            reflection_diffusive_upper_bound_temp =
                reflection_diffusive_upper_bound;
        ...
        std::cerr << "Reflection model is not supported! - Aborting .."
                  << std::endl;
        exit(-1);
      }
    }
